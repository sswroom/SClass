...
//-------------------------------------------------------------------------------
...
#if defined(TEST_IMAGE) && defined(TEST_THREAD)
	{
		...
		else
		{
			...
			NEW_CLASS(efs, IO::FileStream(sbuff, IO::FileStream::FileMode::Create, IO::FileStream::FileShare::DenyNone, IO::FileStream::BufferType::NoWriteBuffer));
		...
		else if (key == 'y' || key == 'Y')
		{
			...
			else
			{
				...
				else
				{
					...
					cli = Net::HTTPClient::CreateConnect(sockf, 0, (const UTF8Char*)"http://sswroom.no-ip.org:5080/benchmark/upload", "POST", false);
