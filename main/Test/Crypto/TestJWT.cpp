#include "Stdafx.h"
#include "MyMemory.h"
#include "Core/Core.h"
#include "Crypto/Cert/X509PubKey.h"
#include "Crypto/Cert/X509PrivKey.h"
#include "Crypto/Token/JWTHandler.h"
#include "Crypto/Token/JWToken.h"
#include "IO/FileStream.h"
#include "IO/Path.h"
#include "Net/OSSocketFactory.h"
#include "Net/SSLEngineFactory.h"
#include "Parser/FileParser/X509Parser.h"
#include "Text/StringBuilderUTF8.h"
#include "Text/TextBinEnc/Base64Enc.h"

Int32 MyMain(NN<Core::ProgControl> progCtrl)
{
	Text::CStringNN keyB64 = CSTR("");
	Text::StringBuilderUTF8 sb;
	NN<Crypto::Token::JWTHandler> jwt;
	Net::OSSocketFactory sockf(false);
	Net::TCPClientFactory clif(sockf);
	Optional<Net::SSLEngine> ssl = Net::SSLEngineFactory::Create(clif, true);
	NEW_CLASSNN(jwt, Crypto::Token::JWTHandler(ssl, Crypto::Token::JWSignature::Algorithm::HS256, UTF8STRC("your-256-bit-secret"), Crypto::Cert::X509Key::KeyType::Unknown));

	Crypto::Token::JWTParam param;
	NN<Text::String> s = Text::String::New(UTF8STRC("1234567890"));
	param.SetSubject(s.Ptr());
	s->Release();
	param.SetIssuedAt(1516239022);
	Data::StringMapObj<UnsafeArrayOpt<const UTF8Char>> payload;
	payload.Put(CSTR("name"), (const UTF8Char*)"John Doe");
	jwt->Generate(sb, payload, param);
	if (!sb.Equals(UTF8STRC("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoiSm9obiBEb2UiLCJzdWIiOiIxMjM0NTY3ODkwIiwiaWF0IjoxNTE2MjM5MDIyfQ.3uGPWYtY_HtIcBGz4eUmTtcjZ4HnJZK9Z2uhx0Ks4n8")))
	{
		jwt.Delete();
		ssl.Delete();
		return 1;	
	}
	jwt.Delete();
	
	NN<Crypto::Token::JWToken> token;
	if (!Crypto::Token::JWToken::Parse(CSTR("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoiSm9obiBEb2UiLCJzdWIiOiIxMjM0NTY3ODkwIiwiaWF0IjoxNTE2MjM5MDIyfQ.3uGPWYtY_HtIcBGz4eUmTtcjZ4HnJZK9Z2uhx0Ks4n8"), nullptr).SetTo(token))
	{
		ssl.Delete();
		return 2;
	}
	if (!token->SignatureValid(ssl, UTF8STRC("your-256-bit-secret"), Crypto::Cert::X509Key::KeyType::Unknown))
	{
		token.Delete();
		ssl.Delete();
		return 3;
	}
	NN<Data::StringMapObj<Text::String*>> result;
	if (!token->ParsePayload(param, false, nullptr).SetTo(result))
	{
		token.Delete();
		ssl.Delete();
		return 4;
	}
	if (!s.Set(result->Get(CSTR("name"))) || !s->Equals(UTF8STRC("John Doe")))
	{
		token->FreeResult(result);
		token.Delete();
		ssl.Delete();
		return 5;
	}
	if (!param.GetSubject().SetTo(s) || !s->Equals(UTF8STRC("1234567890")))
	{
		token->FreeResult(result);
		token.Delete();
		ssl.Delete();
		return 6;
	}
	if (param.GetIssuedAt() != 1516239022)
	{
		token->FreeResult(result);
		token.Delete();
		ssl.Delete();
		return 7;
	}
	token->FreeResult(result);
	token.Delete();

	Text::TextBinEnc::Base64Enc b64;
	UInt8 keyBuff[4096];
	UIntOS keySize;
	keySize = b64.DecodeBin(keyB64, keyBuff);
	if (keySize == 0)
	{
		ssl.Delete();
		return 8;
	}
	else
	{
		Optional<Crypto::Cert::X509Key> key = nullptr;
		NN<Crypto::Cert::X509Key> nnkey;
		NN<Text::String> s = Text::String::New(CSTR("JWTRSAPub.key"));
		NN<Crypto::Cert::X509File> x509;// = Parser::FileParser::X509Parser::ParseBuff(Data::ByteArray(keyBuff, keySize), s);
		NEW_CLASSNN(x509, Crypto::Cert::X509PubKey(s, Data::ByteArrayR(keyBuff, keySize)));
		s->Release();
		if (x509->GetFileType() == Crypto::Cert::X509File::FileType::PrivateKey)
		{
			NN<Crypto::Cert::X509PrivKey> privKey = NN<Crypto::Cert::X509PrivKey>::ConvertFrom(x509);
			key = privKey->CreateKey();
		}
		else if (x509->GetFileType() == Crypto::Cert::X509File::FileType::PublicKey)
		{
			NN<Crypto::Cert::X509PubKey> pubKey = NN<Crypto::Cert::X509PubKey>::ConvertFrom(x509);
			key = pubKey->CreateKey();
		}
		else if (x509->GetFileType() == Crypto::Cert::X509File::FileType::Key)
		{
			key = NN<Crypto::Cert::X509Key>::ConvertFrom(x509->Clone());
		}
		else if (x509->GetFileType() == Crypto::Cert::X509File::FileType::Cert)
		{
			NN<Crypto::Cert::X509Cert> cert = NN<Crypto::Cert::X509Cert>::ConvertFrom(x509);
			key = cert->GetNewPublicKey();
		}
		if (key.IsNull())
		{
			x509.Delete();
			ssl.Delete();
			return 10;
		}
		x509.Delete();
		if (key.SetTo(nnkey) && (nnkey->GetKeyType() == Crypto::Cert::X509Key::KeyType::RSA || nnkey->GetKeyType() == Crypto::Cert::X509Key::KeyType::RSAPublic))
		{
			if (Crypto::Token::JWToken::Parse(CSTR("eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImlhdCI6MTUxNjIzOTAyMn0.NHVaYe26MbtOYhSKkoKYdFVomg4i8ZJd8_-RU8VNbftc4TSMb4bXP3l3YlNWACwyXPGffz5aXHc6lty1Y2t4SWRqGteragsVdZufDn5BlnJl9pdR_kdVFUsra2rWKEofkZeIC4yWytE58sMIihvo9H1ScmmVwBcQP6XETqYd0aSHp1gOa9RdUPDvoXQ5oqygTqVtxaDr6wUFKrKItgBMzWIdNZ6y7O9E0DhEPTbE9rfBo6KTFsHAZnMg4k68CDp2woYIaXbmYTWcvbzIuHO7_37GT79XdIwkm95QJ7hYC9RiwrV7mesbY4PAahERJawntho0my942XheVLmGwLMBkQ"), nullptr).SetTo(token))
			{
				if (!token->SignatureValid(ssl, nnkey->GetASN1Buff(), nnkey->GetASN1BuffSize(), nnkey->GetKeyType()))
				{
					token.Delete();
					key.Delete();
					ssl.Delete();
					return 11;
				}
				token.Delete();
			}
		}
		key.Delete();
	}
	ssl.Delete();
	return 0;
}
