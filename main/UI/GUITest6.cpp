...
Int32 MyMain(NN<Core::ProgControl> progCtrl)
{
	...
	if (progCtrl->CreateGUICore(progCtrl).SetTo(core))
	{
		...
		if (tv->InsertItem(nullptr, nullptr, CSTR("Root Item"), 0).SetTo(item1))
		{
			if (tv->InsertItem(item1, nullptr, CSTR("Item 1"), 0).SetTo(item2))
			{
				tv->InsertItem(item2, nullptr, CSTR("Item 1-1"), 0);
				tv->ExpandItem(item2);
			}
			tv->ExpandItem(item1);
			if (tv->InsertItem(item1, nullptr, CSTR("Item 2"), 0).SetTo(item2))
			{
				tv->InsertItem(item2, nullptr, CSTR("Item 2-1"), 0);
				tv->InsertItem(item2, nullptr, CSTR("Item 2-2"), 0);
				tv->ExpandItem(item2);
			}
		}
