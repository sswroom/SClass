...
namespace Crypto
{
	namespace Hash
	{
		...
		private:
			...
			void CalcHash(UInt32 cost, UnsafeArray<const UInt8> salt, Text::CStringNN password, UnsafeArray<UInt8> hashBuff) const;
		public:
			Bcrypt();
			virtual ~Bcrypt();
			...
			Bool GenHash(NN<Text::StringBuilderUTF8> sb, UInt32 cost, UnsafeArray<const UInt8> salt, Text::CStringNN password) const;
		};
	}
}
#endif
