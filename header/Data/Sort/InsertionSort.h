...
namespace Data
{
	namespace Sort
	{
		...
		{
		public:
			template <class T> static void SortB(T *arr, NN<Data::Comparator<T>> comparator, OSInt firstIndex, OSInt lastIndex);
		};
	}
}

template <class T> void Data::Sort::InsertionSort::SortB(T *arr, NN<Data::Comparator<T>> comparator, OSInt left, OSInt right)
{
	OSInt i;
	OSInt j;
	OSInt k;
