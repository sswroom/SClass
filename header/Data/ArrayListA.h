...
namespace Data
{
	...
	template <class T> UOSInt ArrayListA<T>::AddAll(const ReadingList<T> *arr)
	{
	...
	template <class T> UOSInt ArrayListA<T>::IndexOf(T val)
	{
		UOSInt i = objCnt;
		while (i-- > 0)
			if (arr[i] == val)
	...
	template <class T> ArrayListA<T> *ArrayListA<T>::Clone()
	{
		ArrayListA<T> *newArr = new ArrayListA<T>(this->capacity);
		newArr->AddAll(this);
		return newArr;
	}
	...
	template <class T> UOSInt ArrayListA<T>::GetCapacity()
	{
	...
	template <class T> UOSInt ArrayListA<T>::GetRange(T *outArr, UOSInt Index, UOSInt cnt)
	{
	...
	template <class T> T* ArrayListA<T>::GetArray(UOSInt *arraySize)
	{
