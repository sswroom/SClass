...
namespace Data
{
	...
	template <class T> T SyncArrayMap<T>::Put(const UTF8Char *key, T val)
	{
	...
	template <class T> T SyncArrayMap<T>::Get(const UTF8Char *key)
	{
		Sync::MutexUsage mutUsage(this->mut);
		OSInt i;
		i = this->keys->SortedIndexOf(key);
	...
	template <class T> T SyncArrayMap<T>::Remove(const UTF8Char *key)
	{
		Sync::MutexUsage mutUsage(this->mut);
		OSInt i;
		i = this->keys->SortedIndexOf(key);
	...
	template <class T> const UTF8Char *SyncArrayMap<T>::GetKey(OSInt index)
	{
		Sync::MutexUsage mutUsage(this->mut);
		return this->keys->GetItem(index);
	}

	template <class T> void SyncArrayMap<T>::Clear()
	{
		Sync::MutexUsage mutUsage(this->mut);
		OSInt i;
