...
namespace Data
{
	...
	private:
		...
		UOSInt cnt;
	public:
		FastStringNativeKeyIterator(UnsafeArray<const FastStringNativeItem<T>> arr, UOSInt cnt)
		{
			this->arr = arr;
			this->cnt = cnt;
		}
	...
	template <class T> void FastStringMapNative<T>::Insert(UOSInt index, UInt32 hash, NN<Text::String> s, T val)
	{
		if (index > this->cnt)
		{
			index = this->cnt;
	...
	template <class T> UOSInt FastStringMapNative<T>::GetCount() const
	{
		return this->cnt;
	}

	template <class T> T FastStringMapNative<T>::GetItem(UOSInt index) const
	{
		if (index >= this->cnt)
		{
			return 0;
	...
	template <class T> Optional<Text::String> FastStringMapNative<T>::GetKey(UOSInt index) const
	{
		if (index >= this->cnt)
		{
			return nullptr;
	...
	template <class T> OSInt FastStringMapNative<T>::IndexOf(UInt32 hash, UnsafeArray<const UTF8Char> s, UOSInt len) const
	{
		OSInt i;
		OSInt j;
		OSInt k;
...
/*				if (this->items[k].s->Equals(s, len))
	...
	template <class T> OSInt FastStringMapNative<T>::IndexOf(NN<Text::String> s) const
	{
		UInt32 hash = this->crc.CalcDirect(s->v, s->leng);
		return IndexOf(hash, s->v, s->leng);
	}

	template <class T> OSInt FastStringMapNative<T>::IndexOfC(Text::CStringNN s) const
	{
		UInt32 hash = this->crc.CalcDirect(s.v, s.leng);
		return IndexOf(hash, s.v, s.leng);
	}
	...
	template <class T> T FastStringMapNative<T>::RemoveNN(NN<Text::String> key)
	{
		...
		if (index >= 0)
		{
			...
			{
				MemCopyO(&this->items[index], &this->items[index + 1], sizeof(FastStringNativeItem<T>) * (this->cnt - (UOSInt)index));
			}
			return oldVal;
		}
		return 0;
	...
	template <class T> T FastStringMapNative<T>::RemoveAt(UOSInt index)
	{
		if (index < this->cnt)
		{
			T oldVal = this->items[index].val;
	...
	template <class T> UInt32 FastStringMapNative<T>::CalcHash(UnsafeArray<const UTF8Char> s, UOSInt len) const
	{
