...
namespace Data
{
	...
	protected:
		...
		ByteArrayBase(UnsafeArray<T> buff, UIntOS buffSize) : DataArray<T>(buff, buffSize)
		{
		}
	...
	public:
		...
		UInt16 ReadMU16(UIntOS index) const
		{
			this->CheckError(index + 2);
			return ReadMUInt16(&this->buff[index]);
		}

		Int16 ReadNI16(UIntOS index) const
		{
			this->CheckError(index + 2);
			return ReadNInt16(&this->buff[index]);
		}

		UInt16 ReadNU16(UIntOS index) const
		{
			this->CheckError(index + 2);
		...
		UInt32 ReadMU24(UIntOS index) const
		{
			this->CheckError(index + 3);
			return ReadMUInt24(&this->buff[index]);
		}

		Int32 ReadNI24(UIntOS index) const
		{
			this->CheckError(index + 3);
			return ReadNInt24(&this->buff[index]);
		}

		UInt32 ReadNU24(UIntOS index) const
		{
			this->CheckError(index + 3);
		...
		UInt32 ReadMU32(UIntOS index) const
		{
			this->CheckError(index + 4);
			return ReadMUInt32(&this->buff[index]);
		}

		Int32 ReadNI32(UIntOS index) const
		{
			this->CheckError(index + 4);
			return ReadNInt32(&this->buff[index]);
		}

		UInt32 ReadNU32(UIntOS index) const
		{
			this->CheckError(index + 4);
		...
		UInt64 ReadMU64(UIntOS index) const
		{
			this->CheckError(index + 8);
			return ReadMUInt64(&this->buff[index]);
		}

		Int64 ReadNI64(UIntOS index) const
		{
			this->CheckError(index + 8);
			return ReadNInt64(&this->buff[index]);
		}

		UInt64 ReadNU64(UIntOS index) const
		{
			this->CheckError(index + 8);
