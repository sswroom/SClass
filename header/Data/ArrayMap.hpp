...
namespace Data
{
	...
	template <class T, class V> T ArrayMap<T, V>::GetKey(UIntOS index) const
	{
		return this->keys->GetItem(index);
	}

	template <class T, class V> IntOS ArrayMap<T, V>::GetIndex(T key) const
	{
	...
	template <class T, class V> void ArrayMap<T, V>::AllocSize(UIntOS cnt)
	{
	...
	template <class T, class V> UIntOS ArrayMap<T, V>::GetCount() const
	{
		return this->vals.GetCount();
	}

	template <class T, class V> V ArrayMap<T, V>::GetItem(UIntOS index) const
	{
	...
	template <class T, class V> UnsafeArray<V> ArrayMap<T, V>::ToArray(OutParam<UIntOS> objCnt)
	{
		UIntOS cnt;
