...
namespace Net
{
	...
	{
	private:
		static UIntOS DecryptEncryptedKey(NN<Net::SSLEngine> ssl, NN<Crypto::Cert::X509Key> key, NN<Text::XMLReader> reader, NN<Text::StringBuilderUTF8> sbResult, UnsafeArray<UInt8> keyBuff);
		static UIntOS ParseKeyInfo(NN<Net::SSLEngine> ssl, NN<Crypto::Cert::X509Key> key, NN<Text::XMLReader> reader, NN<Text::StringBuilderUTF8> sbResult, UnsafeArray<UInt8> keyBuff);
