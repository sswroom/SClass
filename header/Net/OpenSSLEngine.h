...
namespace Net
{
	...
	public:
		...
		virtual Bool GenerateCert(Text::CString country, Text::CString company, Text::CStringNN commonName, OutParam<NN<Crypto::Cert::X509Cert>> certASN1, OutParam<NN<Crypto::Cert::X509File>> keyASN1, UIntOS keyLength);
		virtual Optional<Crypto::Cert::X509Key> GenerateRSAKey(UIntOS keyLength);
		virtual Optional<Crypto::Cert::X509Key> GenerateECDSAKey(Crypto::Cert::X509File::ECName name);
		virtual Bool Signature(NN<Crypto::Cert::X509Key> key, Crypto::Hash::HashType hashType, Data::ByteArrayR payload, UnsafeArray<UInt8> signData, OutParam<UIntOS> signLen);
		...
		virtual UIntOS Encrypt(NN<Crypto::Cert::X509Key> key, UnsafeArray<UInt8> encData, Data::ByteArrayR payload, Crypto::Encrypt::RSACipher::Padding rsaPadding);
		virtual UIntOS Decrypt(NN<Crypto::Cert::X509Key> key, UnsafeArray<UInt8> decData, Data::ByteArrayR payload, Crypto::Encrypt::RSACipher::Padding rsaPadding);

		UIntOS RSAPublicDecrypt(NN<Crypto::Cert::X509Key> key, UnsafeArray<UInt8> decData, Data::ByteArrayR payload, Crypto::Encrypt::RSACipher::Padding rsaPadding);
	};
}
#endif
