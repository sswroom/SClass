...
namespace Net
{
	namespace JMeter
	{
		...
		public:
			...
			~JMeterIteration() { UIntOS i = listeners.GetCount(); while (i-- > 0) listeners.GetItemNoCheck(i)->IterationEnd(NNTHIS, listenerData.GetItem(i)); }
			...
			void HTTPBegin(NN<HTTPClient> cli) const { UIntOS i = listeners.GetCount(); while (i-- > 0) listeners.GetItemNoCheck(i)->HTTPBegin(NNTHIS, listenerData.GetItem(i), cli); }
			void HTTPEnd(NN<HTTPClient> cli, Bool succ) const { UIntOS i = listeners.GetCount(); while (i-- > 0) listeners.GetItemNoCheck(i)->HTTPEnd(NNTHIS, listenerData.GetItem(i), cli, succ); }
		};
	}
}
#endif
