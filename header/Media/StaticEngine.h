...
namespace Media
{
	...
	public:
		StaticPen(UInt32 color, Double thick, UnsafeArrayOpt<const UInt8> pattern, UIntOS nPattern);
		virtual ~StaticPen();
	...
	public:
		StaticDrawImage(NN<StaticEngine> eng, Math::Size2D<UIntOS> dispSize, Int32 fourcc, Int32 bpp, Media::PixelFormat pf, IntOS maxSize, NN<const Media::ColorProfile> color, Media::ColorProfile::YUVType yuvType, Media::AlphaType atype, Media::YCOffset ycOfst);
...
/*		virtual Bool DrawLine(Double x1, Double y1, Double x2, Double y2, NN<DrawPen> p) = 0;
		virtual Bool DrawPolylineI(UnsafeArray<const Int32> points, IntOS nPoints, NN<DrawPen> p) = 0;
		virtual Bool DrawPolygonI(UnsafeArray<const Int32> points, IntOS nPoints, Optional<DrawPen> p, Optional<DrawBrush> b) = 0;
		virtual Bool DrawPolyPolygonI(UnsafeArray<const Int32> points, UnsafeArray<const UInt32> pointCnt, IntOS nPointCnt, Optional<DrawPen> p, Optional<DrawBrush> b) = 0;
		virtual Bool DrawPolyline(UnsafeArray<const Math::Coord2DDbl> points, IntOS nPoints, NN<DrawPen> p) = 0;
		virtual Bool DrawPolygon(UnsafeArray<const Math::Coord2DDbl> points, IntOS nPoints, Optional<DrawPen> p, Optional<DrawBrush> b) = 0;
		virtual Bool DrawPolyPolygon(UnsafeArray<const Math::Coord2DDbl> points, UnsafeArray<const UInt32> pointCnt, IntOS nPointCnt, Optional<DrawPen> p, Optional<DrawBrush> b) = 0;
