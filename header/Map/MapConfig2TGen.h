...
namespace Map
{
	...
	private:
		...
		static void DrawString(NN<Media::DrawImage> img, NN<MapLayerStyle> lyrs, NN<Map::MapView> view, UnsafeArray<Optional<Data::ArrayListNN<MapFontStyle>>> fonts, UnsafeArray<MapLabels2> labels, UIntOS maxLabels, UnsafeArray<UIntOS> labelCnt, OutParam<Bool> isLayerEmpty);
		static UIntOS NewLabel(UnsafeArray<MapLabels2> labels, UIntOS maxLabel, UnsafeArray<UIntOS> labelCnt, Int32 priority);
		static Bool AddLabel(UnsafeArray<MapLabels2> labels, UIntOS maxLabel, UnsafeArray<UIntOS> labelCnt, Text::CStringNN label, UIntOS nPoints, UnsafeArray<Math::Coord2DDbl> points, Int32 priority, Int32 recType, UInt32 fntStyle, UInt32 flag, NN<Map::MapView> view, Double xOfst, Double yOfst);
		static void SwapLabel(UnsafeArray<MapLabels2> mapLabels, UIntOS index, UIntOS index2);
		static Bool LabelOverlapped(UnsafeArray<Math::RectAreaDbl> points, UIntOS nPoints, Math::RectAreaDbl rect);
		static void DrawLabels(NN<Media::DrawImage> img, UnsafeArray<MapLabels2> labels, UIntOS maxLabel, UnsafeArray<UIntOS> labelCnt, NN<Map::MapView> view, UnsafeArray<Optional<Data::ArrayListNN<MapFontStyle>>> fonts, NN<Media::DrawEngine> drawEng, UnsafeArray<Math::RectAreaDbl> objBounds, InOutParam<UIntOS> objCnt);
