...
namespace Map
{
	...
	public:
		...
		virtual Bool GetTileImageURL(NN<Text::StringBuilderUTF8> sb, UIntOS level, Math::Coord2D<Int32> imgId);

		virtual UIntOS GetScreenObjCnt();
		virtual Optional<Math::Geometry::Vector2D> CreateScreenObjVector(UIntOS index);
		virtual UnsafeArrayOpt<UTF8Char> GetScreenObjURL(UnsafeArray<UTF8Char> sbuff, UIntOS index);
		virtual Bool GetScreenObjURL(NN<Text::StringBuilderUTF8> sb, UIntOS index);
		...
		static UnsafeArray<UTF8Char> GenQuadkey(UnsafeArray<UTF8Char> sbuff, UIntOS level, Int32 imgX, Int32 imgY);
	};
}
#endif
