...
namespace Map
{
	...
	private:
		...
		static Bool AddLabel(UnsafeArray<MapLabels2> labels, UInt32 maxLabel, InOutParam<UInt32> labelCnt, Text::CStringNN label, UOSInt nPoints, UnsafeArray<Math::Coord2DDbl> points, Int32 priority, Int32 recType, UInt32 fntStyle, UInt32 flags, NN<Map::MapView> view, Double xOfst, Double yOfst);
		...
		static void DrawLabels(NN<Media::DrawImage> img, UnsafeArray<MapLabels2> labels, UInt32 maxLabel, UInt32 labelCnt, NN<Map::MapView> view, UnsafeArray<Optional<Data::ArrayListNN<MapFontStyle>>> fonts, NN<Media::DrawEngine> drawEng, UnsafeArray<Math::RectAreaDbl> objBounds, InOutParam<UOSInt> objCnt);
