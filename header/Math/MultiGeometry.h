#ifndef _SM_MATH_MULTIGEOMETRY
#define _SM_MATH_MULTIGEOMETRY
...
namespace Math
{
	template <class T> class MultiGeometry : public Math::Vector2D
	{
	protected:
		Data::ArrayList<T*> geometries;
	public:
		MultiGeometry(UInt32 srid) : Math::Vector2D(srid)
		{
		}
		...
		virtual Bool JoinVector(Math::Vector2D *vec)
		{
			...
			Math::MultiGeometry<T> *obj = (Math::MultiGeometry<T> *)vec;
			UOSInt i = 0;
