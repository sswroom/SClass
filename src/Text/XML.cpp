...
UIntOS Text::XML::GetXMLTextLen(UnsafeArray<const UTF8Char> text)
{
	UIntOS cnt = 0;
...
UIntOS Text::XML::GetXMLTextLen(UnsafeArray<const WChar> text)
{
	UIntOS cnt = 0;
	UnsafeArray<const WChar> wptr = text;
	WChar c;
	while ((c = *wptr++) != 0)
	{
		switch (c)
...
UIntOS Text::XML::GetXMLTextLiteLen(UnsafeArray<const UTF8Char> text)
{
	UIntOS cnt = 0;
	UnsafeArray<const UTF8Char> sptr = text;
	UTF8Char c;
...
UIntOS Text::XML::GetXMLTextLiteLen(UnsafeArray<const WChar> text)
{
	UIntOS cnt = 0;
	UnsafeArray<const WChar> wptr = text;
	WChar c;
...
UIntOS Text::XML::GetHTMLBodyTextLen(UnsafeArray<const UTF8Char> text)
{
	UIntOS cnt = 0;
	UnsafeArray<const UTF8Char> sptr = text;
	UTF8Char c;
...
UIntOS Text::XML::GetHTMLElementTextLen(UnsafeArray<const UTF8Char> text)
{
	UIntOS cnt = 0;
	UnsafeArray<const UTF8Char> sptr = text;
	UTF8Char c;
...
UIntOS Text::XML::GetHTMLBodyTextLen(UnsafeArray<const WChar> text)
{
	UIntOS cnt = 0;
	UnsafeArray<const WChar> wptr = text;
	WChar c;
...
Bool Text::XML::WriteUTF8Char(NN<IO::Stream> stm, UTF32Char c)
{
	UInt8 buff[6];
	if (c < 0x80)
	{
...
UnsafeArray<WChar> Text::XML::ToXMLTextW(UnsafeArray<WChar> buff, UnsafeArray<const WChar> text)
{
	UnsafeArray<WChar> dptr = buff;
	UnsafeArray<const WChar> wptr = text;
	WChar c;
	while ((c = *wptr++) != 0)
	{
...
UnsafeArray<WChar> Text::XML::ToXMLTextLite(UnsafeArray<WChar> buff, UnsafeArray<const WChar> text)
{
	UnsafeArray<WChar> dptr = buff;
	UnsafeArray<const WChar> wptr = text;
	WChar c;
...
UnsafeArray<WChar> Text::XML::ToHTMLBodyText(UnsafeArray<WChar> buff, UnsafeArray<const WChar> text)
{
	UnsafeArray<WChar> dptr = buff;
	UnsafeArray<const WChar> wptr = text;
	WChar c;
...
UnsafeArray<WChar> Text::XML::ToAttrText(UnsafeArray<WChar> buff, UnsafeArray<const WChar> text)
{
	*buff++ = '"';
	buff = ToXMLTextW(buff, text);
	*buff++ = '"';
	*buff = 0;
	return buff;
}
...
{
	UIntOS cnt = GetXMLTextLiteLen(text);
	NN<Text::String> s = Text::String::New(cnt);
...
{
	UIntOS cnt = GetHTMLBodyTextLen(text);
	NN<Text::String> s = Text::String::New(cnt);
...
{
	UIntOS cnt = GetHTMLElementTextLen(text);
	NN<Text::String> s = Text::String::New(cnt);
	ToHTMLElementText(s->v, text);
	return s;
}

UnsafeArray<const WChar> Text::XML::ToNewHTMLBodyTextW(UnsafeArray<const WChar> text)
{
	UIntOS cnt = GetHTMLBodyTextLen(text) + 1;
...
NN<Text::String> Text::XML::ToNewAttrText(UnsafeArrayOpt<const UTF8Char> text)
{
	...
	{
		UIntOS cnt = GetXMLTextLen(nntext) + 2;
...
UnsafeArray<const WChar> Text::XML::ToNewAttrTextW(UnsafeArrayOpt<const WChar> text)
{
...
void Text::XML::FreeNewText(UnsafeArray<const WChar> text)
{
	MemFreeArr(text);
}

void Text::XML::ParseStr(NN<Text::String> out, UnsafeArray<const UTF8Char> xmlStart, UnsafeArray<const UTF8Char> xmlEnd)
{
...
void Text::XML::ParseStr(UnsafeArray<WChar> out, UnsafeArray<const WChar> xmlStart, UnsafeArray<const WChar> xmlEnd)
{
	UnsafeArray<WChar> currPtr = out;
	WChar c;
	while (xmlStart < xmlEnd)
	{
		c = *xmlStart;
		if (c == '&')
		{
			...
			else if (xmlStart[1] == '#' && xmlStart[2] == 'x')
			{
				Bool valid = true;
				UInt32 v = 0;
				UnsafeArray<const WChar> tmp = xmlStart + 3;
				while (true)
				{
					c = *tmp++;
					if (c >= '0' && c <= '9')
			...
			else if (xmlStart[1] == '#')
			{
				Bool valid = true;
				UInt32 v = 0;
				UnsafeArray<const WChar> tmp = xmlStart + 2;
				while (true)
				{
					c = *tmp++;
					if (c >= '0' && c <= '9')
...
Bool Text::XML::HTMLAppendCharRef(UnsafeArray<const UTF8Char> chrRef, UIntOS refSize, NN<IO::Stream> stm)
{
	UTF8Char sbuff[6];
	UTF32Char wcs;
	if (chrRef[0] != '&')
...
Bool Text::XML::HTMLAppendCharRef(UnsafeArray<const UTF8Char> chrRef, UIntOS refSize, NN<Text::StringBuilderUTF8> sb)
{
	UTF8Char sbuff[6];
	UTF32Char wcs;
	if (chrRef[0] != '&')
