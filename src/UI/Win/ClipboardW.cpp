...
Bool UI::Clipboard::GetDataTextH(void *hand, UInt32 fmtId, NN<Text::StringBuilderUTF8> sb, UInt32 tymed)
{
	...
	case CF_DIB: //8
		...
		sb->AppendI32(ReadLInt32(&memptr[0]));
		sb->AppendC(UTF8STRC("\r\nWidth = "));
		sb->AppendI32(ReadLInt32(&memptr[4]));
		sb->AppendC(UTF8STRC("\r\nHeight = "));
		sb->AppendI32(ReadLInt32(&memptr[8]));
		sb->AppendC(UTF8STRC("\r\nPlanes = "));
		sb->AppendI16(ReadLInt16(&memptr[12]));
		sb->AppendC(UTF8STRC("\r\nBitCount = "));
		sb->AppendI16(ReadLInt16(&memptr[14]));
		sb->AppendC(UTF8STRC("\r\nCompression = 0x"));
		sb->AppendHex32V(ReadLUInt32(&memptr[16]));
		sb->AppendC(UTF8STRC("\r\nSizeImage = "));
		sb->AppendI32(ReadLInt32(&memptr[20]));
		sb->AppendC(UTF8STRC("\r\nXPelsPerMeter = "));
		sb->AppendI32(ReadLInt32(&memptr[24]));
		sb->AppendC(UTF8STRC("\r\nYPelsPerMeter = "));
		sb->AppendI32(ReadLInt32(&memptr[28]));
		sb->AppendC(UTF8STRC("\r\nClrUsed = "));
		sb->AppendI32(ReadLInt32(&memptr[32]));
		sb->AppendC(UTF8STRC("\r\nClrImportant = "));
		sb->AppendI32(ReadLInt32(&memptr[36]));
		GlobalUnlock(hand);
		return true;
#if !defined(_WIN32_WCE)
	case CF_DIBV5: //17
