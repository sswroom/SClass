...
OSInt __stdcall UI::GUITextFileView::TFVWndProc(void *hWnd, UInt32 msg, UInt32 wParam, OSInt lParam)
{
	...
	case WM_HOTKEY:
		...
		case HK_SLINEUP:
			...
			{
				me->GetTextPos((Int32)me->caretDispX, (Int32)me->caretDispY - me->pageLineHeight, &xPos, &yPos);
				me->caretX = xPos;
				me->caretY = yPos;
				me->UpdateCaretSel(false);
				me->EnsureCaretVisible();
		...
		case HK_SLINEDOWN:
			...
			{
				me->GetTextPos(me->caretDispX, me->caretDispY + me->pageLineHeight, &xPos, &yPos);
				me->caretX = xPos;
				me->caretY = yPos;
				me->UpdateCaretSel(false);
				me->EnsureCaretVisible();
	...
	case WM_LBUTTONDOWN:
		...
		me->GetTextPos(xPos, (Int32)yPos, &xPos, &yPos);
		me->caretX = xPos;
		me->caretY = yPos;
		SetFocus((HWND)hWnd);
	...
	case WM_MOUSEMOVE:
		if (me->mouseDown)
		{
			...
			xPos = (Int16)LOWORD(lParam);
			yPos = (Int16)HIWORD(lParam);
			lineOfst = yPos / me->pageLineHeight;
			if (lineOfst < 0)
			{
			...
			UInt32 txtPosX;
			UOSInt txtPosY;
			me->GetTextPos(xPos, (Int32)yPos, &txtPosX, &txtPosY);
			if (me->selLastX != txtPosX || me->selLastY != txtPosY)
			{
				me->selLastX = txtPosX;
				me->selLastY = txtPosY;
				me->selEndX = txtPosX;
...
void UI::GUITextFileView::GetPosFromByteOfst(UInt64 byteOfst, UInt32 *txtPosX, UOSInt *txtPosY)
{
	...
	lineNum = ~lineNum - 1;
	UInt64 thisOfst = this->lineOfsts->GetItem(lineNum);
...
{
	OSInt i = this->textPosUpdHdlr->GetCount();
	while (i-- > 0)
	{
