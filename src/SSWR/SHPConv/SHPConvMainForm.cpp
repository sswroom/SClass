...
void __stdcall SSWR::SHPConv::SHPConvMainForm::OnLangSelChg(AnyType userObj)
{
	...
	UTF8Char sbuff[32];
	UnsafeArray<UTF8Char> sptr;
	sptr = Text::StrIntOS(sbuff, (IntOS)me->lstLang->GetSelectedItem().p);
...
{
	NN<SSWR::SHPConv::SHPConvMainForm> me = userObj.GetNN<SSWR::SHPConv::SHPConvMainForm>();
	Data::ArrayListNative<UInt32> indices;
	UTF8Char sbuff[512];
	UnsafeArray<UTF8Char> sptr;
...
void __stdcall SSWR::SHPConv::SHPConvMainForm::OnGroupClicked(AnyType userObj)
{
	...
	UIntOS i;
	UIntOS j;
	SSWR::SHPConv::SHPConvGroupForm frm(nullptr, me->ui);
...
void __stdcall SSWR::SHPConv::SHPConvMainForm::OnFilterClicked(AnyType userObj)
{
	...
	DB::DBFFile dbf(fd, (UInt32)(UIntOS)me->lstLang->GetSelectedItem().p);
	if (!dbf.IsError())
	{
		SSWR::SHPConv::SHPConvCurrFilterForm frm(nullptr, me->ui, dbf, me->globalFilters, me->deng);
		frm.ShowDialog(me);
	}
}
...
void __stdcall SSWR::SHPConv::SHPConvMainForm::OnPreviewClicked(AnyType userObj)
{
	...
	if (fd.GetDataSize() > 0)
	{
		...
		if (!dbf.IsError())
		{
			...
			SSWR::SHPConv::SHPConvDBFViewForm frm(nullptr, me->ui, dbf, me, sb.ToCString());
			frm.ShowDialog(me);
		}
	}
}
...
void __stdcall SSWR::SHPConv::SHPConvMainForm::OnConvertClicked(AnyType userObj)
{
	...
	sb.RemoveChars(4);
	if (me->currGroup == (UIntOS)-1)
	{
		me->ConvertShp(srcFile->ToCString(), sb.ToCString(), dbCols, blkScale, me->globalFilters, me, dbCols2);
	}
	else
	{
		me->GroupConvert(srcFile->ToCString(), sb.ToCString(), dbCols, blkScale, me->globalFilters, me, me->currGroup, nullptr, dbCols2);
	}
	srcFile->Release();
...
Int32 SSWR::SHPConv::SHPConvMainForm::GroupConvert(Text::CStringNN sourceFile, Text::CString outFilePrefix, NN<Data::ArrayListArr<const UTF8Char>> dbCols, Int32 blkScale, NN<Data::ArrayListNN<MapFilter>> filters, NN<IO::ProgressHandler> progress, UIntOS groupCol, Optional<Data::ArrayListArr<const UTF8Char>> outNames, NN<Data::ArrayListNative<UInt32>> dbCols2)
{
	UIntOS i;
	IntOS si;
	Text::StringBuilderUTF8 sb;
	...
	{
		IO::StmData::FileData fd(sb.ToCString(), false);
		DB::DBFFile dbf(fd, (UInt32)(UIntOS)this->lstLang->GetSelectedItem().p);
		if (dbf.QueryTableData(nullptr, CSTR(""), nullptr, 0, 0, nullptr, nullptr).SetTo(r))
		{
			while (r->ReadNext())
			{
				sb.ClearStr();
				...
				if (si < 0)
				{
					names.Insert((UIntOS)~si, Text::String::New(sb.ToCString()));
				}
			}
			dbf.CloseReader(r);
		}
	...
	while (i-- > 0)
	{
		...
		NN<Data::ArrayListArr<const UTF8Char>> nnoutNames;
		if (outNames.SetTo(nnoutNames))
		{
			nnoutNames->Add(Text::StrCopyNew(sb2.ToString()));
		}
		s->Release();
	}
	return shpType;
...
Int32 SSWR::SHPConv::SHPConvMainForm::ConvertShp(Text::CStringNN sourceFile, Text::CString outFilePrefix, NN<Data::ArrayListArr<const UTF8Char>> dbCols, Int32 blkScale, NN<Data::ArrayListNN<MapFilter>> filters, NN<IO::ProgressHandler> progress, NN<Data::ArrayListNative<UInt32>> dbCols2)
{
	Text::StringBuilderUTF8 sb;
	UInt8 buff[259];
	UIntOS i;
...
//	Dim tmpWriter As IO.StreamWriter
	...
	if (shpType == 3 || shpType == 5)
	{
		...
		if (!cip.IsError() && !cix.IsError() && !blk.IsError() && !cib.IsError())
		{
			...
			IO::StmData::FileData fd(sb.ToCString(), false);
			DB::DBFFile dbf(fd, (UInt32)(UIntOS)this->lstLang->GetSelectedItem().p);
			dbfr = dbf.QueryTableData(nullptr, CSTR(""), nullptr, 0, 0, nullptr, nullptr);
			NN<StrRecord> strRec;

//			tmpWriter = New IO.StreamWriter(sourceFile.Substring(0, sourceFile.LastIndexOf(".")) + ".txt")

			WriteLUInt32(&buff[0], nRecords);
			WriteLInt32(&buff[4], shpType);
			...
			while (tRec < nRecords)
			{
				...
				Bool chkVal = true;
				if (recSize == 2 && ReadLInt32(&buff[8]) == 0)
				{
//					tmpWriter.WriteLine(ControlChars.Tab + ControlChars.Tab + ControlChars.Tab)
				...
				else if (ReadLInt32(&buff[8]) != shpType)
				{
...
//					tmpWriter.WriteLine(ControlChars.Tab + ControlChars.Tab + ControlChars.Tab)
				...
				else
				{
					...
					xMin = ReadLDouble(&buff[12]);
					yMin = ReadLDouble(&buff[20]);
					xMax = ReadLDouble(&buff[28]);
					yMax = ReadLDouble(&buff[36]);
					if (Math::IsNAN(xMin))
					{
						if (dbfr.SetTo(r))
						{
...
//					'tmpWriter.WriteLine(nParts.ToString() + ControlChars.Tab + nPoints.ToString())
					...
					if (isGrid80)
					{
						...
						currX = ReadLDouble(&buff[16]);
						currY = ReadLDouble(&buff[24]);
						WriteLInt32(&buff[0], Double2Int32(currX));
						WriteLInt32(&buff[4], Double2Int32(currY));
						cip.Write(Data::ByteArrayR(buff, 8));
						cipPos += 8;
						xMin = currX;
						xMax = currX;
...
//						Double lastY;
						...
						currX = ReadLDouble(&buff[16]);
						currY = ReadLDouble(&buff[24]);
						WriteLInt32(&buff[0], Double2Int32(currX * LATSCALE));
						WriteLInt32(&buff[4], Double2Int32(currY * LATSCALE));
						cip.Write(Data::ByteArrayR(buff, 8));
						cipPos += 8;
						xMin = currX;
						xMax = currX;
...
//					tmpWriter.WriteLine(ControlChars.Tab + GetDBFName(dbf, dbCols, tRec, dbCols2))
					...
					while (si <= bottom)
					{
						...
						{
							sk = 0;
							sl = (IntOS)blks.GetCount() - 1;
							while (sk <= sl)
							{
								sm = (sk + sl) >> 1;
								theBlk = blks.GetItem((UIntOS)sm);
								if (theBlk->blockX > sj)
								{
									sl = sm - 1;
								}
							...
							if (sk >= 0)
							{
								...
								NEW_CLASSNN(theBlk->records, Data::ArrayListNN<StrRecord>());
			...
			WriteLInt32(&buff[0], (Int32)blks.GetCount());
			WriteLInt32(&buff[4], blkScale);
			...
			while (i < j)
			{
				...
				WriteLInt32(&buff[0], (Int32)theBlk->records->GetCount());
				WriteLInt32(&buff[4], theBlk->blockX);
				WriteLInt32(&buff[8], theBlk->blockY);
				blk.Write(Data::ByteArrayR(buff, 12));
				k = 0;
				l = theBlk->records->GetCount();
				while (k < l)
				{
					WriteLInt32(&buff[0], theBlk->records->GetItemNoCheck(k)->recId);
			...
			while (i < j)
			{
				...
				WriteLInt32(&buff[0], theBlk->blockX);
				WriteLInt32(&buff[4], theBlk->blockY);
				WriteLInt32(&buff[8], (Int32)theBlk->records->GetCount());
				WriteLInt32(&buff[12], (Int32)filePos);
				cib.Write(Data::ByteArrayR(buff, 16));
				...
				while (k < l)
				{
					...
					WriteLInt32(&buff[0], strRec->recId);
					u16buff.ClearStr();
		...
		if (!cip.IsError() && !cix.IsError() && !blk.IsError() && !cib.IsError())
		{
			...
			while (tRec < nRecords)
			{
				...
				else
				{
					...
					if (shpType == 11)
					{
						currX = ReadLDouble(&buff[12]);
						currY = ReadLDouble(&buff[20]);
	//					Double currZ = ReadLDouble(&buff[28]);
						if (recSize >= 18)
						{
	//						Double currM = ReadLDouble(&buff[36]);
						}
					}
					else
					{
				...
				if (chkVal)
				{
					...
					WriteLInt32(&buff[4], 1);
					WriteLInt32(&buff[8], 0);
					WriteLInt32(&buff[12], 1);
...
Int32 SSWR::SHPConv::SHPConvMainForm::LoadShape(Text::CStringNN fileName, Bool updateTxt)
{
...

	{
		...
		xMin = ReadLDouble(&buff[36]);
		yMin = ReadLDouble(&buff[44]);
		xMax = ReadLDouble(&buff[52]);
		yMax = ReadLDouble(&buff[60]);
		shpType = ReadLInt32(&buff[32]);
		if (updateTxt)
		{
			sptr = Text::StrInt32(sbuff, ReadMInt32(&buff[24]) * 2);
			this->txtFileLength->SetText(CSTRP(sbuff, sptr));
			sptr = Text::StrInt32(sbuff, ReadLInt32(&buff[28]));
			this->txtVersion->SetText(CSTRP(sbuff, sptr));
			this->txtShpType->SetText(this->typeName[ReadLInt32(&buff[32])]);
			...
			sptr = Text::StrDouble(sbuff, ReadLDouble(&buff[68]));
			this->txtZMin->SetText(CSTRP(sbuff, sptr));
			sptr = Text::StrDouble(sbuff, ReadLDouble(&buff[76]));
			this->txtZMax->SetText(CSTRP(sbuff, sptr));
			sptr = Text::StrDouble(sbuff, ReadLDouble(&buff[84]));
			this->txtMMin->SetText(CSTRP(sbuff, sptr));
			sptr = Text::StrDouble(sbuff, ReadLDouble(&buff[92]));
...

	{
		...
		if (!dbf.IsError())
		{
			...
			sptr = Text::StrUIntOS(sbuff, dbRecCnt);
			this->txtRecCnt->SetText(CSTRP(sbuff, sptr));
		}
	}

	this->currGroup = (UIntOS)-1;
	...
	{
		Double tVal = (yMax - yMin) * (xMax - xMin) / UIntOS2Double(dbRecCnt);
...
void SSWR::SHPConv::SHPConvMainForm::ParseLabelStr(Text::CStringNN labelStr, NN<Data::ArrayListArr<const UTF8Char>> dbCols, NN<Data::ArrayListNative<UInt32>> dbCols2)
{
	Text::StringBuilderUTF8 sb;
	sb.Append(labelStr);
	UIntOS i;
...
void SSWR::SHPConv::SHPConvMainForm::FreeLabelStr(NN<Data::ArrayListArr<const UTF8Char>> dbCols, NN<Data::ArrayListNative<UInt32>> dbCols2)
{
	UIntOS i;
	i = dbCols->GetCount();
	while (i-- > 0)
	{
		Text::StrDelNew(dbCols->GetItemNoCheck(i));
	}
	dbCols->Clear();
	dbCols2->Clear();
}

NN<Text::String> SSWR::SHPConv::SHPConvMainForm::GetNewDBFName(NN<DB::DBFFile> dbf, NN<Data::ArrayListArr<const UTF8Char>> dbCols, UIntOS currRec, NN<Data::ArrayListNative<UInt32>> dbCols2)
{
	...
	{
		output.Append(dbCols->GetItemNoCheck(0));
		while (i < j)
		{
			...
			if (shpType == 1)
			{
				...
				Text::StrTrim(sbuff);
				output.Append(sbuff);
				output.Append(dbCols->GetItemNoCheck(i));
			}
			else if (shpType == 2)
			{
				dbf->GetRecord(sbuff, currRec, col);
...
SSWR::SHPConv::SHPConvMainForm::SHPConvMainForm(Optional<UI::GUIClientControl> parent, NN<UI::GUICore> ui, NN<Media::DrawEngine> deng, NN<Media::MonitorMgr> monMgr) : UI::GUIForm(parent, 576, 464, ui)
{
	...
	UTF8Char sbuff[256];
	UnsafeArray<UTF8Char> sptr;
	Data::ArrayListNative<UInt32> codePages;
	...
	while (i < j)
	{
		...
		else
		{
			...
			k = this->lstLang->AddItem(CSTRP(sbuff, sptr), (void*)(IntOS)cp);
			if (cp == sysCP)
			{
