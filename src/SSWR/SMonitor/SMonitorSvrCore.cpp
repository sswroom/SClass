...
void SSWR::SMonitor::SMonitorSvrCore::LoadData()
{
	...
	UOSInt i;
	UOSInt j;
	r = db->ExecuteReaderC(UTF8STRC("select id, userName, pwd, userType from webuser order by id"));
	if (r)
	{
		while (r->ReadNext())
		{
	...
	r = db->ExecuteReaderC(UTF8STRC("select webuser_id, device_id from webuser_device order by webuser_id"));
	if (r)
	{
		while (r->ReadNext())
		{
...
SSWR::SMonitor::SMonitorSvrCore::SMonitorSvrCore(IO::Writer *writer, Media::DrawEngine *deng)
{
	...
	else
	{
		...
		if (s && s2)
		{
			this->db = Net::MySQLTCPClient::CreateDBTool(this->sockf, s, s2, cfg->GetValue(CSTR("UID")), cfg->GetValue(CSTR("PWD")), log, CSTR("DB: "));
		...
		else
		{
			...
			if (s)
			{
				this->db = DB::ODBCConn::CreateDBTool(s, s2, s3, cfg->GetValue(CSTR("Schema")), log, CSTR("DB: "));
				NEW_CLASS(this->dbMut, Sync::Mutex());
				if (this->db == 0)
				{
