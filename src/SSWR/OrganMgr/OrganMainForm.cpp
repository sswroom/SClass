...
void __stdcall SSWR::OrganMgr::OrganMainForm::OnObjDblClicked(void *userObj)
{
	OrganMainForm *me = (OrganMainForm*)userObj;
	UTF8Char u8buff[256];
	OSInt i = me->lbObj->GetSelectedIndex();
	...
	{
		if (me->lbDir->GetSelectedIndex() > 0)
		{
...
void __stdcall SSWR::OrganMgr::OrganMainForm::OnObjSelChg(void *userObj)
{
	...
	if (me->restoreObj)
		return;
	if (me->lbObj->GetSelectedIndex() == -1)
	{
		me->lastSpeciesObj = 0;
		me->lastGroupObj = 0;
		me->ClearGroupForm();
...
//	StopAudio();
	...
	if (me->lbImage->GetSelectedIndex() == -1)
	{
...
Bool __stdcall SSWR::OrganMgr::OrganMainForm::OnImgRClicked(void *userObj, OSInt scnX, OSInt scnY, MouseButton btn)
{
	...
	if (me->inputMode == IM_SPECIES)
	{
		OSInt index = me->lbImage->GetSelectedIndex();
		if (index == -1)
			return false;
		Bool showDef = false;
		if (me->lbObj->GetSelectedIndex() == -1)
...
Bool __stdcall SSWR::OrganMgr::OrganMainForm::OnImgMouseUp(void *userObj, OSInt scnX, OSInt scnY, MouseButton btn)
{
	...
	if (me->dispImageToCrop && me->dispImageDown)
	{
		...
		if (me->CalcCropRect(rect))
		{
			...
			if (me->dispImageUF)
			{
				me->env->UpdateUserFileCrop(me->dispImageUF, ptX1, ptY1, img->info->dispWidth - ptX2, img->info->dispHeight - ptY2);
			}
			else if (me->dispImageWF)
			{
				me->env->UpdateWebFileCrop(me->dispImageWF, ptX1, ptY1, img->info->dispWidth - ptX2, img->info->dispHeight - ptY2);
			}
		}
...
void __stdcall SSWR::OrganMgr::OrganMainForm::OnImgDraw(void *userObj, UInt8 *imgPtr, OSInt w, OSInt h, OSInt bpl)
{
	...
	if (me->dispImage)
	{
		if (me->dispImageUF)
		{
			if (me->dispImageUF->cropLeft != 0 || me->dispImageUF->cropTop != 0 || me->dispImageUF->cropRight != 0 || me->dispImageUF->cropBottom)
			{
				...
				me->pbImg->Image2ScnPos(img->info->dispWidth - me->dispImageUF->cropRight, img->info->dispHeight - me->dispImageUF->cropBottom, &x2, &y2);
				...
				{
					ImageUtil_DrawRectNA32(imgPtr + bpl * Math::Double2Int32(y1) + Math::Double2Int32(x1) * 4, Math::Double2Int32(x2 - x1), Math::Double2Int32(y2 - y1), bpl, 0xff4040ff);
				}
			}
		}
		else if (me->dispImageWF)
		{
			if (me->dispImageWF->cropLeft != 0 || me->dispImageWF->cropTop != 0 || me->dispImageWF->cropRight != 0 || me->dispImageWF->cropBottom)
			{
				...
				me->pbImg->Image2ScnPos(img->info->dispWidth - me->dispImageWF->cropRight, img->info->dispHeight - me->dispImageWF->cropBottom, &x2, &y2);
				if (x2 < 0 || x1 >= w || y2 < 0 || y1 >= h)
				{
	...
	if (me->dispImageDown)
	{
		...
		if (me->CalcCropRect(rect))
		{
			ImageUtil_DrawRectNA32(imgPtr + bpl * rect[1] + rect[0] * 4, rect[2] - rect[0], rect[3] - rect[1], bpl, 0xffff0000);
		}
	}
}
...
void __stdcall SSWR::OrganMgr::OrganMainForm::OnSpBookAddClicked(void *userObj)
{
	...
	if (me->inputMode != IM_SPECIES)
		return ;
	OSInt i = me->cboSpBook->GetSelectedIndex();
	if (i < 0)
	{
...
void __stdcall SSWR::OrganMgr::OrganMainForm::OnMapDraw(void *userObj, Media::DrawImage *dimg, OSInt xOfst, OSInt yOfst)
{
	...
	if (me->mapCurrImage)
	{
		...
		dimg->DrawImagePt(me->mapCurrImage, Math::OSInt2Double(xOfst), Math::OSInt2Double(yOfst + scnH - me->mapCurrImage->GetHeight()));
...
Bool SSWR::OrganMgr::OrganMainForm::CalcCropRect(OSInt *rect)
{
	...
	if (img->info->dispWidth * drawHeight > img->info->dispHeight * drawWidth)
	{
		drawHeight = MulDivOS(drawWidth, img->info->dispHeight, img->info->dispWidth);
	}
	else
	{
		drawWidth = MulDivOS(drawHeight, img->info->dispWidth, img->info->dispHeight);
	}
...
void SSWR::OrganMgr::OrganMainForm::UpdateImgDir()
{
	...
	if (this->inputMode == IM_SPECIES)
	{
		...
		if (this->lbObj->GetSelectedIndex() != -1)
		{
		...
		if (showDef)
		{
			...
			this->mapUFiles->AddRange(&ufileList);
...
{
	UTF8Char sbuff[512];
	if (this->lbDir->GetSelectedIndex() == -1)
	{
    ...
    if (this->cboGroupType->GetSelectedIndex() == -1)
	{
...
//	RegisterDragDrop((HWND)this->pbImg->GetHandle(), this);
	...
	NEW_CLASS(tileMap, Map::OSM::OSMTileMap((const UTF8Char*)"http://a.tile.opencyclemap.org/cycle/", this->env->GetCacheDir(), 18, this->env->GetSocketFactory()));
