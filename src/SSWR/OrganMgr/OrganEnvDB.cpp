...
SSWR::OrganMgr::OrganEnvDB::OrganEnvDB() : OrganEnv()
{
	...
	this->cfgImgDirBase = Text::String::OrEmpty(cfg->GetValue(CSTR("ImageDir")));
	this->cfgDataPath = Text::String::OrEmpty(cfg->GetValue(CSTR("DataDir")));
	this->cfgCacheDir = Text::String::OrEmpty(cfg->GetValue(CSTR("CacheDir")));
	this->cfgMapFont = cfg->GetValue(CSTR("MapFont"));
	NN<Text::String> userId;

	if (this->cfgImgDirBase->leng == 0 || this->cfgDataPath->leng == 0 || this->cfgCacheDir->leng == 0)
	{
		this->errType = ERR_CONFIG;
		return;
	}
...
NN<Text::String> SSWR::OrganMgr::OrganEnvDB::GetDataDir()
{
	return this->cfgDataPath;
}

NN<Text::String> SSWR::OrganMgr::OrganEnvDB::GetCacheDir()
{
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetGroupItems(NN<Data::ArrayListNN<OrganGroupItem>> items, Optional<OrganGroup> grp)
{
	NN<DB::DBReader> r;
	Int32 grpId;
	Int32 photoGrp;
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetGroupImages(NN<Data::ArrayListNN<OrganImageItem>> items, NN<OrganGroup> grp)
{
	UTF8Char sbuff[512];
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetSpeciesImages(NN<Data::ArrayListNN<OrganImageItem>> items, NN<OrganSpecies> sp)
{
	UTF8Char sbuff[512];
	UTF8Char sbuff2[512];
	UnsafeArray<UTF8Char> sptr;
		...
		*/
		...
		while (IO::Path::FindNextFile(sptr, sess, 0, pt, 0).SetTo(sptr2))
		{
			if (pt == IO::Path::PathType::File)
			{
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".JPG")))
				{
...
/*
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".TIF")))
				{
...
/*
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".PCX")) ||
					Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".GIF")) ||
					Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".HEIC")) ||
					Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".HEIF")) ||
					 Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".PNG")))
				{
					NEW_CLASSNN(imgItem, OrganImageItem(this->userId));
					imgItem->SetDispName(CSTRP(sptr, sptr2));
					imgItem->SetIsCoverPhoto(isCoverPhoto);
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".AVI")) || Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".MOV")) == 0 || Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".MTS")) || Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".M2TS")))
				{
					NEW_CLASSNN(imgItem, OrganImageItem(this->userId));
					imgItem->SetDispName(CSTRP(sptr, sptr2));
					imgItem->SetIsCoverPhoto(isCoverPhoto);
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".WAV")))
				{
					NEW_CLASSNN(imgItem, OrganImageItem(this->userId));
					imgItem->SetDispName(CSTRP(sptr, sptr2));
					imgItem->SetIsCoverPhoto(isCoverPhoto);
		...
		if (IO::Path::GetPathType(CSTRP(sbuff, sptr2)) == IO::Path::PathType::File)
		{
			...
			{
				if (Text::StrSplitP(cols, 4, {sbuff2, (UIntOS)(sptr2 - sbuff2)}, '\t') == 3)
				{
					NEW_CLASSNN(imgItem, OrganImageItem(0));
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetGroupAllSpecies(NN<Data::ArrayListNN<OrganSpecies>> items, Optional<OrganGroup> grp)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return 0;
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetGroupAllUserFile(NN<Data::ArrayListNN<UserFileInfo>> items, NN<Data::ArrayListNative<UInt32>> colors, Optional<OrganGroup> grp)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return 0;
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetSpeciesItems(NN<Data::ArrayListNN<OrganGroupItem>> items, NN<Data::ArrayListNative<Int32>> speciesIds)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return 0;
...
SSWR::OrganMgr::OrganEnvDB::FileStatus SSWR::OrganMgr::OrganEnvDB::AddSpeciesFile(NN<OrganSpecies> sp, Text::CStringNN fileName, Bool firstPhoto, Bool moveFile, OptOut<Int32> fileId)
{
	...
	if (fileType == 1)
	{
		...
		if (valid)
		{
			...
			if (si >= 0)
			{
				while (si > 0)
				{
					if (webUser->userFileIndex.GetItem((UIntOS)si - 1) == ts)
					{
						si--;
					}
					else
				...
				*sptr++ = IO::Path::PATH_SEPERATOR;
				...
				if (succ)
				{
					...
					if (this->db.SetTo(db))
					{
						...
						sql.AppendCmdC(CSTR(", "));
						sql.AppendStrUTF8(camera.SetTo(nncamera)?UnsafeArray<const UTF8Char>(nncamera->v):UnsafeArrayOpt<const UTF8Char>(nullptr));
						sql.AppendCmdC(CSTR(", "));
						sql.AppendDbl(0);
						sql.AppendCmdC(CSTR(", "));
						sql.AppendDbl(0);
	...
	else if (fileType == 3)
	{
		...
		if (valid && graphImg.SetTo(nngraphImg))
		{
			NN<WebUserInfo> webUser = this->GetWebUser(this->userId);
					...
					if (this->db.SetTo(db))
					{
						...
						if (db->ExecuteNonQuery(sql.ToCString()) > 0)
						{
							...
							*sptr++ = IO::Path::PATH_SEPERATOR;
							...
							this->drawEng->DeleteImage(nngraphImg);
							return FS_SUCCESS;
						}
						else
						{
...
SSWR::OrganMgr::OrganEnvDB::FileStatus SSWR::OrganMgr::OrganEnvDB::AddSpeciesWebFile(NN<OrganSpecies> sp, NN<Text::String> srcURL, NN<Text::String> imgURL, NN<IO::Stream> stm, UnsafeArrayOpt<UTF8Char> webFileName)
{
	...
	NN<SpeciesInfo> spInfo;
	{
		...
		while (i-- > 0)
		{
			...
			{
				if (Text::String::OrEmpty(imgItem->GetImgURL())->Equals(imgURL))
				{
					found = true;
				}
			}
...
SSWR::OrganMgr::OrganEnvDB::FileStatus SSWR::OrganMgr::OrganEnvDB::AddSpeciesWebFileOld(NN<OrganSpecies> sp, NN<Text::String> srcURL, NN<Text::String> imgURL, NN<IO::Stream> stm, UnsafeArrayOpt<UTF8Char> webFileName)
{
	UTF8Char sbuff[512];
	...
	*sptr++ = IO::Path::PATH_SEPERATOR;
	...
	if (firstPhoto)
	{
		i = Text::StrLastIndexOfCharC(fileName, (UIntOS)(fileNameEnd - fileName), '.');
		fileName[i] = 0;
	...
	if (webFileName.SetTo(nnwebFileName))
	{
		...
		*sptr++ = IO::Path::PATH_SEPERATOR;
		Text::StrConcatC(sptr, fileName, (UIntOS)(fileNameEnd - fileName));
	}
	return SSWR::OrganMgr::OrganEnvDB::FS_SUCCESS;
}

Bool SSWR::OrganMgr::OrganEnvDB::UpdateSpeciesWebFile(NN<OrganSpecies> sp, NN<WebFileInfo> wfile, NN<Text::String> srcURL, NN<Text::String> location)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return false;
...
Bool SSWR::OrganMgr::OrganEnvDB::UpdateSpeciesWebFileOld(NN<OrganSpecies> sp, UnsafeArray<const UTF8Char> webFileName, UnsafeArray<const UTF8Char> srcURL)
{
	...
	*sptr++ = IO::Path::PATH_SEPERATOR;
	...
	UIntOS webFileNameLen = Text::StrCharCnt(webFileName);
	UIntOS srcURLLen = Text::StrCharCnt(srcURL);
	Text::StringBuilderUTF8 sb;
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetGroupCount(Int32 groupId)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return 0;
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetSpeciesCount(Int32 groupId)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return 0;
...
Bool SSWR::OrganMgr::OrganEnvDB::MoveImages(NN<Data::ArrayListNN<OrganImages>> imgList, NN<OrganSpecies> destSp, NN<UI::GUIForm> frm)
{
	...
	while (i < j)
	{
		...
		else
		{
			...
			*sptr2++ = IO::Path::PATH_SEPERATOR;
			sptr2 = Text::String::OrEmpty(img->GetImgItem()->GetDispName())->ConcatTo(sptr2);
			if (Text::String::OrEmpty(img->GetImgItem()->GetFullName())->Equals(sbuff, (UIntOS)(sptr2 - sbuff)))
			{
				break;
			}
			if (!IO::FileUtil::MoveFile(Text::String::OrEmpty(img->GetImgItem()->GetFullName())->ToCString(), CSTRP(sbuff, sptr2), IO::FileUtil::FileExistAction::Fail, nullptr, 0))
			{
				Text::StringBuilderUTF8 sb;
				NN<Text::String> s;
				...
				sb.Append(s);
				s->Release();
				sb.AppendOpt(img->GetImgItem()->GetDispName());
		...
		sptr = Text::StrConcatC(sptr + 1, UTF8STRC("web.txt"));
		{
			...
			while (i < j)
			{
				...
				if (img->GetImgItem()->GetFileType() == OrganImageItem::FileType::Webimage)
				{
					...
					name = Text::String::OrEmpty(img->GetImgItem()->GetDispName())->ToCString();
					name = name.Substring(name.LastIndexOf(IO::Path::PATH_SEPERATOR) + 1);
					sb.ClearStr();
					sb.Append(name);
					sb.AppendC(UTF8STRC("\t"));
					sb.AppendOpt(img->GetImgItem()->GetImgURL());
					sb.AppendC(UTF8STRC("\t"));
					sb.AppendOpt(img->GetImgItem()->GetSrcURL());
					writer.WriteLine(sb.ToCString());
				}
				i++;
			}
				...
				while (reader.ReadLine(sb, 512))
				{
					...
					if (Text::StrSplitP(sarr, 4, sb2, '\t') == 3)
					{
						...
						while (i-- > 0)
						{
							...
							if (img->GetImgItem()->GetFileType() == OrganImageItem::FileType::Webimage)
							{
								if (Text::String::OrEmpty(img->GetImgItem()->GetDispName())->EndsWith(sarr[0].v, sarr[0].leng))
								{
									found = true;
									break;
								}
...
UIntOS SSWR::OrganMgr::OrganEnvDB::GetWebUsers(NN<Data::ArrayListNN<OrganWebUser>> userList)
{
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return 0;
	UIntOS initCnt = userList->GetCount();
...
Bool SSWR::OrganMgr::OrganEnvDB::AddWebUser(UnsafeArray<const UTF8Char> userName, UnsafeArray<const UTF8Char> pwd, UnsafeArray<const UTF8Char> watermark, UserType userType)
{
	...
	sql.AppendCmdC(CSTR(", "));
	{
		UIntOS len = Text::StrCharCnt(pwd);
		UInt8 md5Value[16];
		UTF8Char sbuff[33];
		Crypto::Hash::MD5 md5;
		md5.Calc(pwd, len);
...
Bool SSWR::OrganMgr::OrganEnvDB::ModifyWebUser(Int32 id, UnsafeArray<const UTF8Char> userName, UnsafeArrayOpt<const UTF8Char> pwd, UnsafeArray<const UTF8Char> watermark)
{
	...
	if (pwd.SetTo(nnpwd))
	{
		UIntOS len = Text::StrCharCnt(nnpwd);
		UInt8 md5Value[16];
		UTF8Char sbuff[33];
		Crypto::Hash::MD5 md5;
		md5.Calc(nnpwd, len);
...
Bool SSWR::OrganMgr::OrganEnvDB::AddDataFile(Text::CStringNN fileName)
{
	...
	{
		IO::ParserType t = nnpobj->GetParserType();
		if (t == IO::ParserType::MapLayer)
		{
			NN<Map::MapDrawLayer> lyr = NN<Map::MapDrawLayer>::ConvertFrom(nnpobj);
		...
		if (IO::FileUtil::CopyFile(fileName, CSTRP(sbuff, sptr), IO::FileUtil::FileExistAction::Fail, nullptr, 0))
		{
			...
			if (db->ExecuteNonQuery(sql.ToCString()) >= 1)
			{
				...
				if (fileType == 1)
				{
					...
					{
						IO::StmData::FileData fd(fileName, false);
						if (this->parsers.ParseFileType(fd, IO::ParserType::MapLayer).SetTo(nnpobj))
						{
							NN<Map::GPSTrack> gpsTrk = NN<Map::GPSTrack>::ConvertFrom(nnpobj);

							IntOS startIndex = webUser->userFileIndex.SortedIndexOf(dataFile->startTime);
							IntOS endIndex = webUser->userFileIndex.SortedIndexOf(dataFile->endTime);
							if (startIndex < 0)
							{
							...
							{
								NN<SSWR::OrganMgr::UserFileInfo> userFile = webUser->userFileObj.GetItemNoCheck((UIntOS)startIndex);
...
Bool SSWR::OrganMgr::OrganEnvDB::DelDataFile(NN<DataFileInfo> dataFile)
{
	...
	*sptr++ = IO::Path::PATH_SEPERATOR;
	...
	if (dataFile->fileType == 1 && this->userMap.Get(dataFile->webUserId).SetTo(webUser))
	{
		...
		if (j >= 0)
		{
			webUser->gpsFileIndex.RemoveAt((UIntOS)j);
			webUser->gpsFileObj.RemoveAt((UIntOS)j);
		}
	}
...
Bool SSWR::OrganMgr::OrganEnvDB::GetGPSPos(Int32 userId, const Data::Timestamp &ts, OutParam<Math::Coord2DDbl> pos)
{
	...
	if (this->gpsTrk.IsNull() || this->gpsUserId != userId || this->gpsStartTime > ts || this->gpsEndTime < ts)
	{
		...
		if (webUser->gpsFileObj.GetItem((UIntOS)i).SetTo(dataFile))
		{
	...
	{
		pos.Set(gpsTrk->GetPosByTime(ts));
		return true;
	}
	else
	{
...
Optional<Map::GPSTrack> SSWR::OrganMgr::OrganEnvDB::OpenGPSTrack(NN<DataFileInfo> dataFile)
{
	...
	*sptr++ = IO::Path::PATH_SEPERATOR;
	...
	if (Optional<Map::MapDrawLayer>::ConvertFrom(this->parsers.ParseFileType(fd, IO::ParserType::MapLayer)).SetTo(lyr))
	{
		if (lyr->GetObjectClass() == Map::MapDrawLayer::OC_GPS_TRACK)
		{
			trk = NN<Map::GPSTrack>::ConvertFrom(lyr);
		}
		else
		{
			trk = nullptr;
...
Bool SSWR::OrganMgr::OrganEnvDB::TripAdd(const Data::Timestamp &fromDate, const Data::Timestamp &toDate, Int32 locId)
{
	...
	if (!this->db.SetTo(db))
		return false;
	IntOS i = this->TripGetIndex(fromDate);
	IntOS j = this->TripGetIndex(toDate);
	IntOS k = this->LocationGetIndex(locId);
	if (i < 0 && j < 0 && k >= 0)
	{
		if (i == j)
		{
			...
			if (db->ExecuteNonQuery(sql.ToCString()) != -2)
			{
				...
				this->trips.Insert((UIntOS)(-i - 1), t);
				return true;
			}
			else
			{
...
Bool SSWR::OrganMgr::OrganEnvDB::LocationAdd(Int32 locId, Text::CStringNN engName, Text::CStringNN chiName)
{
	...
	if ((UIntOS)lType + 1 >= this->locType.GetCount())
		return false;
	NN<DB::DBTool> db;
	if (!this->db.SetTo(db))
		return false;
...
Optional<Media::ImageList> SSWR::OrganMgr::OrganEnvDB::ParseImage(NN<OrganImageItem> img, OptOut<Optional<UserFileInfo>> outUserFile, OptOut<Optional<WebFileInfo>> outWebFile)
{
...
/*				if (userFile->rotType != 0)
	...
	else if (img->GetFullName().NotNull())
	{
		outUserFile.Set(nullptr);
		outWebFile.Set(nullptr);
		NN<IO::ParsedObject> pobj;
...
Optional<Media::ImageList> SSWR::OrganMgr::OrganEnvDB::ParseSpImage(NN<OrganSpecies> sp)
{
	UTF8Char sbuff[512];
	UTF8Char sbuff2[512];
	UnsafeArray<UTF8Char> sptr;
	...
	*sptr++ = IO::Path::PATH_SEPERATOR;
	...
	if (pobj.SetTo(nnpobj))
	{
		...
		{
			imgList = NN<Media::ImageList>::ConvertFrom(nnpobj);
		}
		else
		{
			nnpobj.Delete();
	...
	if (db->ExecuteReader(sql.ToCString()).SetTo(r))
	{
		if (r->ReadNext())
		{
			...
			*parentId = r->GetInt32(5);
			...
			if (Text::StrEqualsC(sbuff, (UIntOS)(sptr - sbuff), searchStr, searchStrLen) || Text::StrEqualsC(sbuff2, (UIntOS)(sptr2 - sbuff2), searchStr, searchStrLen))
			{
			}
			else
			{
...
void SSWR::OrganMgr::OrganEnvDB::LoadGroupTypes()
{
	...
	if (db->ExecuteReader(sql.ToCString()).SetTo(r))
	{
		while (r->ReadNext())
		{
			...
			NEW_CLASSNN(grpType, OrganGroupType(seq, {cname, (UIntOS)(cnameEnd - cname)}, {ename, (UIntOS)(enameEnd - ename)}));
...
void SSWR::OrganMgr::OrganEnvDB::Test()
{
	...
	UIntOS i = 0;
	UIntOS j = this->userFileMap.GetCount();
	while (i < j)
	{
	...
	if (IO::Path::FindFile(CSTRP(sbuff, sptr2)).SetTo(sess))
	{
		...
		while (IO::Path::FindNextFile(sptr, sess, 0, pt, 0).SetTo(sptr2))
		{
			if (pt == IO::Path::PathType::File)
			{
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".JPG")) || Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".PCX")) || Text::StrEqualsICaseC(&sptr[i], (UIntOS)(sptr2 - &sptr[i]), UTF8STRC(".WAV")))
				{
					Int32 fileId = 0;
...
void SSWR::OrganMgr::OrganEnvDB::ExportLite(UnsafeArray<const UTF8Char> folder)
{
	...
	*sptr3++ = IO::Path::PATH_SEPERATOR;
	NN<DB::DBReader> r;
	if (db->ExecuteReader(CSTR("select srcDir from category")).SetTo(r))
	{
		while (r->ReadNext())
		{
			sb.ClearStr();
	...
	if (db->ExecuteReader(CSTR("select fileType, fileTime, webuser_id, dataFileName from userfile")).SetTo(r))
	{
		...
		*sptr3++ = IO::Path::PATH_SEPERATOR;
		...
		while (r->ReadNext())
		{
			...
			{
				if (fileType == 1 && Text::StrEndsWithICaseC(sptr2, (UIntOS)(sptr2End - sptr2), UTF8STRC(".jpg")))
				{
					Optional<Media::ImageList> imgList;
					NN<Media::ImageList> nnimgList;
					{
						IO::StmData::FileData fd({sbuff2, (UIntOS)(sptr3End - sbuff2)}, false);
					...
					if (imgList.SetTo(nnimgList))
					{
						...
						if (Optional<Media::StaticImage>::ConvertFrom(nnimgList->GetImage2(0, 0)).SetTo(simg))
						{
							...
							{
								resizer.SetTargetSize(Math::Size2D<UIntOS>(1920, 1920));
