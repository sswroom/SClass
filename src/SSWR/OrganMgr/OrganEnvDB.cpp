...
SSWR::OrganMgr::OrganEnvDB::OrganEnvDB() : OrganEnv()
{
	...
	{
		this->db = Net::MySQLTCPClient::CreateDBTool(this->sockf, cfgMySQLHost, cfgMySQLDB, cfgUID, cfgPassword, log, false, 0);
	}
	else if (cfgDSN)
	{
		...
		if (IO::FileUtil::CopyFile(fileName, sbuff, IO::FileUtil::FEA_FAIL, 0, 0))
		{
			...
			if (this->db->ExecuteNonQuery(sql.ToString()) >= 1)
			{
				...
				if (fileType == 1)
				{
					...
					NEW_CLASS(fd, IO::StmData::FileData(fileName, false));
					pobj = this->parsers->ParseFileType(fd, IO::ParsedObject::PT_MAP_LAYER_PARSER);
					DEL_CLASS(fd);
...
Bool SSWR::OrganMgr::OrganEnvDB::UpdateUserFileLoc(UserFileInfo *userFile, const UTF8Char *location)
{
	Bool succ = false;
	DB::SQLBuilder sql(this->db->GetSvrType());
	sql.AppendCmd((const UTF8Char*)"update userfile set location = ");
	sql.AppendStrUTF8(location);
	sql.AppendCmd((const UTF8Char*)" where id = ");
	sql.AppendInt32(userFile->id);
	if (this->db->ExecuteNonQuery(sql.ToString()) >= 0)
...
Bool SSWR::OrganMgr::OrganEnvDB::TripAdd(Data::DateTime *fromDate, Data::DateTime *toDate, Int32 locId)
{
	...
	if (i < 0 && j < 0 && k >= 0)
	{
		...
		{
			DB::SQLBuilder sql(this->db->GetSvrType());
			sql.AppendCmd((const UTF8Char*)"insert into trip (fromDate, toDate, locId, cate_id) values (");
