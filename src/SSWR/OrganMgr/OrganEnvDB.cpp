...
SSWR::OrganMgr::OrganEnvDB::OrganEnvDB() : OrganEnv()
{
	...
	this->log.AddFileLog(CSTR("OrganMgr.log"), IO::ILogHandler::LOG_TYPE_SINGLE_FILE, IO::ILogHandler::LOG_GROUP_TYPE_NO_GROUP, IO::ILogHandler::LOG_LEVEL_RAW, 0, false);
	...
	{
		this->db = DB::ODBCConn::CreateDBTool(cfgDSN, Text::String::OrEmpty(cfgUID), Text::String::OrEmpty(cfgPassword), 0, &this->log, CSTR_NULL);
	}
	if (db == 0)
	{
		this->errType = ERR_DB;
...
Int32 SSWR::OrganMgr::OrganEnvDB::NewBook(Text::CString title, Text::CString author, Text::CString press, Data::DateTime *publishDate, Text::CString url)
{
	OrganBook *book;
	DB::SQLBuilder sql(this->db);
	UOSInt i;
...
Bool SSWR::OrganMgr::OrganEnvDB::UpdateUserFileDesc(UserFileInfo *userFile, const UTF8Char *descript)
{
	...
	if (this->db->ExecuteNonQuery(sql.ToCString()) >= 0)
	{
		...
		if (descript)
		{
			userFile->descript = Text::String::NewNotNullSlow(descript);
		}
	}
	return succ;
}
...
Bool SSWR::OrganMgr::OrganEnvDB::UpdateUserFileLoc(UserFileInfo *userFile, const UTF8Char *location)
{
	...
	if (this->db->ExecuteNonQuery(sql.ToCString()) >= 0)
	{
		...
		if (location)
		{
			userFile->location = Text::String::NewNotNullSlow(location);
		}
	}
	return succ;
}
		...
		while (r->ReadNext())
		{
			...
			if (IO::Path::GetPathType(CSTRP(sbuff, sptr2End)) == IO::Path::PathType::Unknown)
			{
				if (fileType == 1 && Text::StrEndsWithICaseC(sptr2, (UOSInt)(sptr2End - sptr2), UTF8STRC(".jpg")))
				{
					IO::FileStream *fs;
					IO::StmData::FileData *fd;
					NEW_CLASS(fd, IO::StmData::FileData({sbuff2, (UOSInt)(sptr3End - sbuff2)}, false));
