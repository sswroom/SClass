...
SSWR::OrganMgr::OrganWebForm::OrganWebForm(Optional<UI::GUIClientControl> parent, NN<UI::GUICore> ui, NN<Media::DrawEngine> eng) : UI::GUIForm(parent, 160, 100, ui)
{
	...
	{
		db = Net::MySQLTCPClient::CreateDBTool(this->clif, nullptr, s, cfg->GetValue(CSTR("MySQLDB")), Text::String::OrEmpty(cfg->GetValue(CSTR("MySQLUID"))), Text::String::OrEmpty(cfg->GetValue(CSTR("MySQLPwd"))), this->log, CSTR("DB: "));
	}
	else
	{
	...
	NN<OrganEnvDB> env;
	NEW_CLASSNN(env, OrganEnvDB(this->sockf, 0, this->log, db, cfg->GetValue(CSTR("ImageDir")), port, 0, cfg->GetValue(CSTR("CacheDir")), cfg->GetValue(CSTR("DataDir")), scnSize, cfg->GetValue(CSTR("ReloadPwd")), 0, eng, cfg->GetValue(CSTR("OSMCacheOath"))->ToCString()));
	cfg.Delete();
	this->env = env;
