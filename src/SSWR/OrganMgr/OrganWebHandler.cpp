...
void SSWR::OrganMgr::OrganWebHandler::LoadCategory()
{
	...
	UTF8Char sbuff[512];
	UTF8Char *sptr;
	DB::DBReader *r = this->db->ExecuteReaderC(UTF8STRC("select cate_id, chi_name, dirName, srcDir, flags from category"));
	if (r != 0)
	{
	...
	r = this->db->ExecuteReaderC(UTF8STRC("select seq, eng_name, chi_name, cate_id from group_type"));
	if (r != 0)
	{
		Text::StringBuilderUTF8 sb;
		while (r->ReadNext())
...
void SSWR::OrganMgr::OrganWebHandler::LoadSpecies()
{
	...
	DB::DBReader *r = this->db->ExecuteReaderC(UTF8STRC("select id, eng_name, chi_name, sci_name, group_id, description, dirName, photo, idKey, cate_id, flags, photoId, photoWId from species"));
	if (r != 0)
	{
		while (r->ReadNext())
		{
...
void SSWR::OrganMgr::OrganWebHandler::LoadGroups()
{
	...
	SSWR::OrganMgr::OrganWebHandler::CategoryInfo *cate;
	UOSInt i;
	DB::DBReader *r = this->db->ExecuteReaderC(UTF8STRC("select id, group_type, eng_name, chi_name, description, parent_id, photo_group, photo_species, idKey, cate_id, flags from groups"));
	if (r != 0)
	{
		while (r->ReadNext())
		{
...
void SSWR::OrganMgr::OrganWebHandler::LoadBooks()
{
	...
	DB::DBReader *r = this->db->ExecuteReaderC(UTF8STRC("select id, title, dispAuthor, press, publishDate, url from book"));
	if (r != 0)
	{
		while (r->ReadNext())
		{
	...
	r = this->db->ExecuteReaderC(UTF8STRC("select species_id, book_id, dispName from species_book"));
	if (r != 0)
	{
		while (r->ReadNext())
		{
...
void SSWR::OrganMgr::OrganWebHandler::LoadUsers()
{
	...
	DB::DBReader *r = this->db->ExecuteReaderC(UTF8STRC("select id, userName, pwd, watermark, userType from webuser"));
	if (r != 0)
	{
		while (r->ReadNext())
		{
	...
	r = this->db->ExecuteReaderC(UTF8STRC("select id, fileType, oriFileName, fileTime, lat, lon, webuser_id, species_id, captureTime, dataFileName, crcVal, rotType, prevUpdated, cropLeft, cropTop, cropRight, cropBottom, descript, location from userfile"));
	if (r != 0)
	{
	...
	r = this->db->ExecuteReaderC(UTF8STRC("select fromDate, toDate, locId, cate_id, webuser_id from trip"));
	if (r != 0)
	{
		Int32 cateId;
		Int64 fromDate;
...
void SSWR::OrganMgr::OrganWebHandler::LoadLocations()
{
	...
	DB::DBReader *r = this->db->ExecuteReaderC(UTF8STRC("select id, parentId, cname, ename, lat, lon, cate_id, locType from location"));
	Int32 id;
	if (r != 0)
	{
		while (r->ReadNext())
...
void SSWR::OrganMgr::OrganWebHandler::UserFilePrevUpdated(SSWR::OrganMgr::OrganWebHandler::UserFileInfo *userFile)
{
	if (userFile->prevUpdated)
	{
		...
		sql.AppendCmdC(UTF8STRC("update userfile set prevUpdated = 0 where id = "));
...
void SSWR::OrganMgr::OrganWebHandler::WebFilePrevUpdated(SSWR::OrganMgr::OrganWebHandler::WebFileInfo *wfile)
{
	...
	{
		DB::SQLBuilder sql(this->db);
		sql.AppendCmdC(UTF8STRC("update webfile set prevUpdated = 0 where id = "));
...
Bool SSWR::OrganMgr::OrganWebHandler::SpeciesSetPhotoId(Int32 speciesId, Int32 photoId)
{
	...
	DB::SQLBuilder sql(this->db);
	sql.AppendCmdC(UTF8STRC("update species set photoId = "));
...
Bool SSWR::OrganMgr::OrganWebHandler::SpeciesMove(Int32 speciesId, Int32 groupId, Int32 cateId)
{
	...
	sql.AppendInt32(groupId);
	sql.AppendCmdC(UTF8STRC(", cate_id = "));
	sql.AppendInt32(cateId);
	sql.AppendCmdC(UTF8STRC(" where id = "));
	sql.AppendInt32(speciesId);
	if (this->db->ExecuteNonQueryC(sql.ToString(), sql.GetLength()) == 1)
...
Bool SSWR::OrganMgr::OrganWebHandler::UserfileUpdateRotType(Int32 userfileId, Int32 rotType)
{
	...
	sql.AppendCmdC(UTF8STRC(", prevUpdated = "));
	sql.AppendInt32(1);
	sql.AppendCmdC(UTF8STRC(" where id = "));
	sql.AppendInt32(userfileId);
	if (this->db->ExecuteNonQueryC(sql.ToString(), sql.GetLength()) > 0)
...
Bool SSWR::OrganMgr::OrganWebHandler::GroupSetPhotoSpecies(Int32 groupId, Int32 photoSpeciesId)
{
	...
	DB::SQLBuilder sql(this->db);
	sql.AppendCmdC(UTF8STRC("update groups set photo_species = "));
...
Bool SSWR::OrganMgr::OrganWebHandler::GroupSetPhotoGroup(Int32 groupId, Int32 photoGroupId)
{
	...
	DB::SQLBuilder sql(this->db);
	sql.AppendCmdC(UTF8STRC("update groups set photo_group = "));
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcPhotoDown(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
		...
		req->GetQueryValueI32(UTF8STRC("fileId"), &fileId))
	{
		...
		if (sp && sp->cateId == cateId && env.user && userFile && userFile->webuserId == env.user->id)
		{
			...
			u8ptr = Text::StrConcatC(u8ptr, UTF8STRC("UserFile"));
			*u8ptr++ = IO::Path::PATH_SEPERATOR;
			...
			*u8ptr++ = IO::Path::PATH_SEPERATOR;
			if (userFile->fileType == 3)
			{
				...
				u8ptr = Text::StrConcatC(u8ptr, UTF8STRC("_"));
				u8ptr = Text::StrHexVal32(u8ptr, userFile->crcVal);
				u8ptr = Text::StrConcatC(u8ptr, UTF8STRC(".png"));
			}
			else
			{
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcGroup(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
		...
		req->GetQueryValueI32(UTF8STRC("cateId"), &cateId))
	{
		...
		if (group->descript)
		{
			s = Text::XML::ToNewHTMLText(group->descript->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
		}
		if (!notAdmin)
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcGroupMod(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
		...
		req->GetQueryValueI32(UTF8STRC("cateId"), &cateId))
	{
		...
		while (i < j)
		{
			...
			sb.AppendC(UTF8STRC(">"));
			s = Text::XML::ToNewHTMLText(groupType->chiName->v);
			sb.Append(s);
			s->Release();
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcSpecies(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
		...
		req->GetQueryValueI32(UTF8STRC("cateId"), &cateId))
	{
		...
		if (species->descript)
		{
			s = Text::XML::ToNewHTMLText(species->descript->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
		}
		...
		if (species->books->GetCount() > 0)
		{
			...
			while (i < j)
			{
				...
				if (book != 0)
				{
					...
					writer->WriteStrC(sb.ToString(), sb.GetLength());
					s = Text::XML::ToNewHTMLText(bookSp->dispName->v);
					writer->WriteStrC(s->v, s->leng);
					s->Release();
			...
			while (i < j)
			{
				...
				if (env.user != 0 && (env.user->userType == 0 || userFile->webuserId == env.user->id))
				{
					...
					if (userFile->webuserId == env.user->id)
					{
						...
						{
							writer->WriteStrC(UTF8STRC(" "));
							s = Text::XML::ToNewHTMLText(userFile->location->v);
							writer->WriteStrC(s->v, s->leng);
							s->Release();
						}
	/*					Data::Int64Map<SSWR::OrganMgr::OrganWebHandler::TripInfo*> *tripCate = user->tripCates->Get(species->cateId);
					...
					if (userFile->descript && userFile->descript->leng > 0)
					{
						...
						s = Text::XML::ToNewHTMLText(userFile->descript->v);
						writer->WriteStrC(s->v, s->leng);
						s->Release();
					}
					if (userFile->webuserId == env.user->id)
					{
						writer->WriteStrC(UTF8STRC("<br/>"));
						s = Text::XML::ToNewHTMLText(userFile->oriFileName->v);
						writer->WriteStrC(s->v, s->leng);
						s->Release();
					}
					if (env.user->userType == 0)
			...
			while (i < j)
			{
				...
				if (env.user && env.user->userType == 0)
				{
					...
					{
						writer->WriteStrC(UTF8STRC("<br/>"));
						s = Text::XML::ToNewHTMLText(wfile->location->v);
						writer->WriteStrC(s->v, s->leng);
						s->Release();
					}
				}
		...
		if (refURLList->GetCount() > 0)
		{
			...
			while (i < j)
			{
				...
				s->Release();
				writer->WriteStrC(UTF8STRC(">"));
				s = Text::XML::ToNewHTMLText(url->v);
				writer->WriteStrC(s->v, s->leng);
				s->Release();
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcPhotoDetail(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
		...
		req->GetQueryValueI32(UTF8STRC("cateId"), &cateId))
	{
		...
		u8ptr = cate->srcDir->ConcatTo(u8buff);
		if (IO::Path::PATH_SEPERATOR != '\\')
		{
		...
		while (startIndex < endIndex)
		{
			...
			if (dt.GetMonth() != month || dt.GetDay() != day)
			{
				if (month != 0 && day != 0)
				{
					...
					while (i < j)
					{
						Text::String *s = Text::XML::ToNewHTMLText(locList.GetItem(i));
						writer->WriteStrC(UTF8STRC(" "));
						writer->WriteStrC(s->v, s->leng);
						s->Release();
						i++;
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcPhotoUpload(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
	...
	while (true)
	{
		...
		s = Text::XML::ToNewHTMLText(fileName);
		writer->WriteStrC(s->v, s->leng);
		s->Release();
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcBookList(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
	...
	if (req->GetQueryValueI32(UTF8STRC("id"), &id))
	{
		...
		while (i < j)
		{
			...
			writer->WriteStrC(UTF8STRC("<td>"));
			s = Text::XML::ToNewHTMLText(book->author->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
			writer->WriteLineC(UTF8STRC("</td>"));
			writer->WriteStrC(UTF8STRC("<td>"));
			s = Text::XML::ToNewHTMLText(book->press->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
			writer->WriteLineC(UTF8STRC("</td>"));
			writer->WriteStrC(UTF8STRC("<td>"));
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcBook(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
		...
		req->GetQueryValueI32(UTF8STRC("cateId"), &cateId))
	{
		...
		if (book->url)
		{
			...
			s->Release();
			writer->WriteStrC(UTF8STRC(">"));
			s = Text::XML::ToNewHTMLText(book->url->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
			writer->WriteLineC(UTF8STRC("</a><br/>"));
		}
...
Bool __stdcall SSWR::OrganMgr::OrganWebHandler::SvcIndex(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, const UTF8Char *subReq, Net::WebServer::WebServiceHandler *parent)
{
	...
	while (i < j)
	{
		...
		if ((cate->flags & 1) == 0 || !notAdmin)
		{
			...
			writer->WriteStrC(UTF8STRC(">"));
			s->Release();
			s = Text::XML::ToNewHTMLText(cate->chiName->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
			writer->WriteLineC(UTF8STRC("</a><br/>"));
	...
	if (sp && sp->cateId == cateId)
	{
		...
		if (wfile)
		{
			...
			*u8ptr++ = IO::Path::PATH_SEPERATOR;
			...
			u8ptr = Text::StrConcatC(u8ptr, UTF8STRC("WebFile"));
			*u8ptr++ = IO::Path::PATH_SEPERATOR;
			u8ptr = Text::StrInt32(u8ptr, wfile->id >> 10);
			*u8ptr++ = IO::Path::PATH_SEPERATOR;
			u8ptr = Text::StrInt32(u8ptr, wfile->id);
			u8ptr = Text::StrConcatC(u8ptr, UTF8STRC(".jpg"));
			this->dataMut->UnlockRead();
...
void SSWR::OrganMgr::OrganWebHandler::ResponseMstm(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp, IO::MemoryStream *mstm, const UTF8Char *contType, UOSInt len)
{
	resp->AddDefHeaders(req);
...
void SSWR::OrganMgr::OrganWebHandler::WriteHeaderPart1(IO::Writer *writer, const UTF8Char *title, Bool isMobile)
{
	...
	writer->WriteStrC(UTF8STRC("<title>"));
	s = Text::XML::ToNewHTMLText(title);
	writer->WriteStrC(s->v, s->leng);
	s->Release();
...
void SSWR::OrganMgr::OrganWebHandler::WriteHeaderPart2(IO::Writer *writer, SSWR::OrganMgr::OrganWebHandler::WebUserInfo *user, const UTF8Char *onLoadFunc)
{
	...
	if (user)
	{
		...
		s = Text::XML::ToNewHTMLText(user->userName->v);
		writer->WriteStrC(s->v, s->leng);
		s->Release();
...
void SSWR::OrganMgr::OrganWebHandler::WriteLocator(IO::Writer *writer, SSWR::OrganMgr::OrganWebHandler::GroupInfo *group, SSWR::OrganMgr::OrganWebHandler::CategoryInfo *cate)
{
	...
	while (i-- > 0)
	{
		...
		{
			writer->WriteStrC(UTF8STRC("<td>"));
			s = Text::XML::ToNewHTMLText(grpType->chiName->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
			writer->WriteLineC(UTF8STRC("</td>"));
			writer->WriteStrC(UTF8STRC("<td>"));
			s = Text::XML::ToNewHTMLText(grpType->engName->v);
			writer->WriteStrC(s->v, s->leng);
			s->Release();
			writer->WriteLineC(UTF8STRC("</td>"));
		}
	...
	if (j > 0)
	{
		...
		while (i < j)
		{
			...
			if (group->totalCount != 0 || showSelect)
			{
				...
				{
					s = Text::XML::ToNewHTMLText(group->engName->v);
					writer->WriteStrC(s->v, s->leng);
					s->Release();
				}
				if (showSelect)
...
void SSWR::OrganMgr::OrganWebHandler::WriteSpeciesTable(IO::Writer *writer, Data::ArrayList<SSWR::OrganMgr::OrganWebHandler::SpeciesInfo *> *spList, UInt32 scnWidth, Int32 cateId, Bool showSelect)
{
	...
	if (j > 0)
	{
		...
		while (i < j)
		{
			...
			{
				s = Text::XML::ToNewHTMLText(sp->sciName->v);
				writer->WriteStrC(s->v, s->leng);
				s->Release();
				writer->WriteLineC(UTF8STRC("<br/>"));
			}
...
SSWR::OrganMgr::OrganWebHandler::OrganWebHandler(Net::SocketFactory *sockf, Net::SSLEngine *ssl, IO::LogTool *log, DB::DBTool *db, Text::String *imageDir, UInt16 port, Text::String *cacheDir, Text::String *dataDir, UInt32 scnSize, Text::String *reloadPwd, Int32 unorganizedGroupId, Media::DrawEngine *eng)
{
	...
	{
		this->AddService(UTF8STRC("/photo.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcPhoto);
		this->AddService(UTF8STRC("/photodown.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcPhotoDown);
		this->AddService(UTF8STRC("/group.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcGroup);
		this->AddService(UTF8STRC("/group.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcGroup);
		this->AddService(UTF8STRC("/groupmod.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcGroupMod);
		this->AddService(UTF8STRC("/groupmod.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcGroupMod);
		this->AddService(UTF8STRC("/species.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcSpecies);
		this->AddService(UTF8STRC("/species.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcSpecies);
		this->AddService(UTF8STRC("/speciesmod.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcSpeciesMod);
		this->AddService(UTF8STRC("/speciesmod.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcSpeciesMod);
		this->AddService(UTF8STRC("/list.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcList);
		this->AddService(UTF8STRC("/listimage.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcList);
		this->AddService(UTF8STRC("/photodetail.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcPhotoDetail);
		this->AddService(UTF8STRC("/photodetail.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcPhotoDetail);
		this->AddService(UTF8STRC("/photodetaild.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcPhotoDetailD);
		this->AddService(UTF8STRC("/photoyear.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcPhotoYear);
		this->AddService(UTF8STRC("/photoday.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcPhotoDay);
		this->AddService(UTF8STRC("/photoupload.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcPhotoUpload);
		this->AddService(UTF8STRC("/photouploadd.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcPhotoUploadD);
		this->AddService(UTF8STRC("/searchinside.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcSearchInside);
		this->AddService(UTF8STRC("/searchinsidemores.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcSearchInsideMoreS);
		this->AddService(UTF8STRC("/searchinsidemoreg.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcSearchInsideMoreG);
		this->AddService(UTF8STRC("/booklist.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcBookList);
		this->AddService(UTF8STRC("/book.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcBook);
		this->AddService(UTF8STRC("/bookview.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcBookView);
		this->AddService(UTF8STRC("/login.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcLogin);
		this->AddService(UTF8STRC("/login.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcLogin);
		this->AddService(UTF8STRC("/logout"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcLogout);
		this->AddService(UTF8STRC("/reload"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcReload);
		this->AddService(UTF8STRC("/reload"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcReload);
		this->AddService(UTF8STRC("/restart"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcRestart);
		this->AddService(UTF8STRC("/restart"), Net::WebServer::IWebRequest::RequestMethod::HTTP_POST, SvcRestart);
		this->AddService(UTF8STRC("/"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcIndex);
		this->AddService(UTF8STRC("/index.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcIndex);
		this->AddService(UTF8STRC("/cate.html"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcCate);
		this->AddService(UTF8STRC("/favicon.ico"), Net::WebServer::IWebRequest::RequestMethod::HTTP_GET, SvcFavicon);
