			...
			if (found)
			{
				...
				else
				{
					...
					else
					{
						...
						NEW_CLASS(fs, IO::FileStream(u8buff, IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_SEQUENTIAL));
						NEW_CLASS(reader, Text::UTF8Reader(fs));
						sb.ClearStr();
						found = false;
						while (reader->ReadLine(&sb, 4096))
		...
		if (cate && ((cate->flags & 1) == 0 || !notAdmin))
		{
			...
			if (this->cacheDir && imgWidth == PREVIEW_SIZE && imgHeight == PREVIEW_SIZE)
			{
				...
				*u8ptr++ = IO::Path::PATH_SEPERATOR;
				...
				NEW_CLASS(fs, IO::FileStream(u8buff, IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
				if (fs->IsError())
				{
					DEL_CLASS(fs);
				}
...
/*						Int32 xRand;
						...
						while (true)
						{
							...
							if (!gimg->GetTextSizeUTF8(f, this->watermark, leng, sz))
							{
								gimg->DelFont(f);
								break;
							}
						...
						{
							NEW_CLASS(fs, IO::FileStream(u8buff, IO::FileStream::FILE_MODE_CREATE, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
			...
			if (dimg)
			{
				...
				if (user && user->watermark)
				{
					...
					if ((this->cacheDir && imgWidth == PREVIEW_SIZE && imgHeight == PREVIEW_SIZE) || user != reqUser)
					{
						...
						{
							f = gimg->NewFontW(L"Arial", fontSize, Media::DrawEngine::DFS_NORMAL);
							if (!gimg->GetTextSizeUTF8(f, user->watermark, leng, sz))
							{
								gimg->DelFont(f);
								break;
							}
							if (sz[0] <= dimg->info->dispWidth && sz[1] <= dimg->info->dispHeight)
							{
								...
								iHeight = Math::Double2Int32(sz[1]);
								gimg2 = this->eng->CreateImage32(iWidth, iHeight, Media::AT_NO_ALPHA);
								gimg2->DrawStringUTF8(0, 0, user->watermark, f, b);
								gimg2->SetAlphaType(Media::AT_ALPHA);
								{
									Bool revOrder;
									UInt8 *bits = gimg2->GetImgBits(&revOrder);
					...
					if (this->cacheDir && imgWidth == PREVIEW_SIZE && imgHeight == PREVIEW_SIZE)
					{
						NEW_CLASS(fs, IO::FileStream(u8buff2, IO::FileStream::FILE_MODE_CREATE, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
						fs->Write(buff, buffSize);
						DEL_CLASS(fs);
						if (userFile->prevUpdated)
						{
