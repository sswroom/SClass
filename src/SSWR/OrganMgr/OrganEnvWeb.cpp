...
SSWR::OrganMgr::OrganEnvWeb::OrganEnvWeb() : OrganEnv()
{
	...
	log->AddFileLog(CSTR("OrganMgr.log"), IO::LogHandler::LogType::SingleFile, IO::LogHandler::LogGroup::NoGroup, IO::LogHandler::LogLevel::Raw, 0, false);
	...
	r = this->db->ExecuteReader(CSTR("select id, fileType, oriFileName, fileTime, lat, lon, webuser_id, species_id, captureTime, dataFileName, crcVal, rotType, camera, descript, cropLeft, cropTop, cropRight, cropBottom from userfile order by id"));
	if (r)
	{
		Text::StringBuilderUTF8 sb;
		Data::DateTime dt;
		...
		while (r->ReadNext())
		{
			...
			userFile->dataFileName = r->GetNewStrB(9, &sb, true);
			...
			i = webUser->userFileIndex.SortedInsert(userFile->fileTime);
			...
			if (pt == IO::Path::PathType::File)
			{
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UOSInt)(sptr2 - &sptr[i]), UTF8STRC(".JPG")))
				{
					...
					if (exif)
					{
						...
						{
							imgItem->SetPhotoDate(dt.ToTimestamp());
						}
						DEL_CLASS(exif);
					}
					imgItem->SetRotateType(OrganImageItem::RotateType::None);
...
/*
		...
		if (IO::Path::GetPathType(CSTRP(sbuff, sptr2)) == IO::Path::PathType::File)
		{
			...
			while (reader.ReadLine(sbuff2, 511))
			{
				...
				{
					NEW_CLASS(imgItem, OrganImageItem(0));
					sptr2 = Text::StrConcat(Text::StrConcatC(sptr, UTF8STRC("web\\")), cols[0]);
					imgItem->SetDispName(CSTRP(sptr, sptr2));
					...
					imgItem->SetImgURL(Text::CString::FromPtr(cols[1]));
					imgItem->SetSrcURL(Text::CString::FromPtr(cols[2]));
					newFlags |= 8;
					items->Add(imgItem);
					retCnt++;
				}
...
Bool SSWR::OrganMgr::OrganEnvWeb::IsBookSpeciesExist(const UTF8Char *sName, Text::StringBuilderUTF8 *sb)
{
...
SSWR::OrganMgr::OrganEnvWeb::FileStatus SSWR::OrganMgr::OrganEnvWeb::AddSpeciesFile(OrganSpecies *sp, Text::CString fileName, Bool firstPhoto, Bool moveFile, Int32 *fileId)
{
	UOSInt i;
	UOSInt j;
	Int32 fileType = 0;
	...
	if (fileType == 1)
	{
		...
		NEW_CLASS(mstm, IO::MemoryStream((Int32)fs->GetLength()));
		while (true)
		{
		...
		NEW_CLASS(md, IO::StmData::MemoryDataRef(readBuff, readSize));
		...
		if (valid)
		{
			...
			Data::Timestamp ts = fileTime.ToTimestamp();
			OSInt k;
			j = webUser->userFileIndex.SortedIndexOf(ts);
			if (j >= 0)
			{
				while (j > 0)
				{
					if (webUser->userFileIndex.GetItem(j - 1).DiffSec(ts) == 0)
					{
						j--;
					}
					else
				...
				while (j < k)
				{
					if (webUser->userFileIndex.GetItem(j).DiffSec(ts) == 0)
						break;
				...
				*sptr++ = IO::Path::PATH_SEPERATOR;
				...
				if (succ)
				{
					...
					if (this->db->ExecuteNonQuery(sql.ToCString()) > 0)
					{
						...
						userFile->dataFileName = Text::String::NewNotNullSlow(dataFileName);
...
SSWR::OrganMgr::OrganEnvWeb::FileStatus SSWR::OrganMgr::OrganEnvWeb::AddSpeciesWebFile(OrganSpecies *sp, Text::String *srcURL, Text::String *imgURL, IO::Stream *stm, UTF8Char *webFileName)
{
	...
	while (i-- > 0)
	{
		...
		if (imgItem->GetFileType() == OrganImageItem::FileType::WEB_IMAGE || imgItem->GetFileType() == OrganImageItem::FileType::WEBFILE)
		{
...
Bool SSWR::OrganMgr::OrganEnvWeb::MoveImages(Data::ArrayList<OrganImages*> *imgList, OrganSpecies *destSp, UI::GUIForm *frm)
{
	...
	while (i < j)
	{
		...
		{
			if (img->GetImgItem()->GetFileType() == OrganImageItem::FileType::WEB_IMAGE)
			{
				sptr2 = sptr;
				if (!moveWeb)
				{
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetWebUsers(Data::ArrayList<OrganWebUser*> *userList)
{
	...
	OrganWebUser *user;
	Text::StringBuilderUTF8 sb;
	DB::DBReader *r = this->db->ExecuteReader(CSTR("select id, userName, watermark from webuser"));
	if (r)
	{
		while (r->ReadNext())
		{
...
Bool SSWR::OrganMgr::OrganEnvWeb::AddWebUser(const UTF8Char *userName, const UTF8Char *pwd, const UTF8Char *watermark)
{
	...
	this->db->CloseReader(r);
	sql.Clear();
	sql.AppendCmdC(CSTR("insert into webuser (userName, pwd, watermark) values ("));
	sql.AppendStrUTF8(userName);
	sql.AppendCmdC(CSTR(", "));
	{
	...
	if (pobj)
	{
		...
		else if (t == IO::ParserType::ReadingDB)
		{
			...
			if (tables.GetCount() == 2)
			{
				...
				{
					DB::DBReader *reader = db->QueryTableData(CSTR("Records"), 0, 0, 0, 0, 0);
					Bool found = false;
					if (reader)
					{
...
void SSWR::OrganMgr::OrganEnvWeb::TripReload(Int32 cateId)
{
	...
	sql.AppendInt32(cateId);
	sql.AppendCmdC(CSTR(" and webuser_id = "));
	sql.AppendInt32(this->userId);
...
Bool SSWR::OrganMgr::OrganEnvWeb::TripAdd(Data::DateTime *fromDate, Data::DateTime *toDate, Int32 locId)
{
	...
	if (i < 0 && j < 0 && k >= 0)
	{
		...
		{
			DB::SQLBuilder sql(this->db);
			sql.AppendCmdC(CSTR("insert into trip (fromDate, toDate, locId, cate_id, webuser_id) values ("));
	...
	if (sess)
	{
		if ((sptr2 = IO::Path::FindNextFile(sptr, sess, 0, &pt, 0)) != 0)
		{
			if (pt == IO::Path::PathType::File)
			{
				...
				else if (Text::StrEqualsICaseC(&sptr[i], (UOSInt)(sptr2 - &sptr[i]), UTF8STRC(".AVI")))
				{
				}
				else if (Text::StrEqualsICaseC(&sptr[i], (UOSInt)(sptr2 - &sptr[i]), UTF8STRC(".WAV")))
				{
