...
#include "SSWR/OrganMgr/OrganEnvWeb.h"
...
SSWR::OrganMgr::OrganEnvWeb::OrganEnvWeb() : OrganEnv()
{
	OSInt i;
	UTF8Char sbuff[256];
	Category *cate;
	...
	this->cfgDSN = cfg->GetValue((const UTF8Char*)"DBDSN");
	this->cfgUID = cfg->GetValue((const UTF8Char*)"DBUID");
	this->cfgPassword = cfg->GetValue((const UTF8Char*)"DBPwd");
	...
	DB::DBReader *r = this->db->ExecuteReader((const UTF8Char*)"select cate_id, chi_name, dirName, srcDir from category");
	if (r)
	{
		while (r->ReadNext())
		{
	...
	r = this->db->ExecuteReader((const UTF8Char*)"select id, fileType, oriFileName, fileTime, lat, lon, webuser_id, species_id, captureTime, dataFileName, crcVal, rotType, camera, descript, cropLeft, cropTop, cropRight, cropBottom from userfile order by id");
	if (r)
	{
		Text::StringBuilderUTF8 sb;
		Data::DateTime dt;
		...
		while (r->ReadNext())
		{
			...
			sb.ClearStr();
			r->GetStr(2, &sb);
			userFile->oriFileName = Text::String::New(sb.ToString(), sb.GetLength());
			r->GetDate(3, &dt);
			...
			sb.ClearStr();
			r->GetStr(9, &sb);
			userFile->dataFileName = Text::String::New(sb.ToString(), sb.GetLength());
			userFile->crcVal = r->GetInt32(10);
			userFile->rotType = r->GetInt32(11);
			sb.ClearStr();
			if (r->GetStr(12, &sb))
			{
				userFile->camera = Text::String::New(sb.ToString(), sb.GetLength());
			}
			else
			{
				userFile->camera = 0;
			...
			if (r->GetStr(13, &sb))
			{
				userFile->descript = Text::String::New(sb.ToString(), sb.GetLength());
			}
			else
			{
				userFile->descript = 0;
	...
	if (r)
	{
		...
		while (r->ReadNext())
		{
			...
			sb.ClearStr();
			r->GetStr(3, &sb);
			wfile->imgUrl = Text::String::New(sb.ToString(), sb.GetLength());
			sb.ClearStr();
			r->GetStr(4, &sb);
			wfile->srcUrl = Text::String::New(sb.ToString(), sb.GetLength());
			...
			sb.ClearStr();
			r->GetStr(10, &sb);
			wfile->location = Text::String::New(sb.ToString(), sb.GetLength());
...
SSWR::OrganMgr::OrganEnvWeb::~OrganEnvWeb()
{
	if (this->db)
	{
		DEL_CLASS(this->db);
...
Text::String *SSWR::OrganMgr::OrganEnvWeb::GetDataDir()
{
	return this->cfgDataPath;
}

Text::String *SSWR::OrganMgr::OrganEnvWeb::GetCacheDir()
{
	return this->cfgCacheDir;
}

UOSInt SSWR::OrganMgr::OrganEnvWeb::GetGroupItems(Data::ArrayList<OrganGroupItem*> *items, OrganGroup *grp)
{
	DB::DBReader *r;
	Int32 grpId;
	Int32 photoGrp;
	...
	{
		sql.Clear();
		sql.AppendCmd((const UTF8Char*)"SELECT id, chi_name, sci_name, eng_name, description, dirName, photo, idKey, flags, photoId, mapColor, photoWId FROM species where group_id = ");
		sql.AppendInt32(grpId);
		sql.AppendCmd((const UTF8Char*)" and cate_id = ");
		sql.AppendInt32(this->currCate->cateId);
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetSpeciesImages(Data::ArrayList<OrganImageItem*> *items, OrganSpecies *sp)
{
	UTF8Char sbuff[512];
	UTF8Char sbuff2[512];
	UTF8Char *sptr;
		...
		*/
		...
		while (IO::Path::FindNextFile(sptr, sess, 0, &pt, 0))
		{
			if (pt == IO::Path::PathType::File)
			{
				...
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char*)".JPG"))
				{
...
/*
				...
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".TIF"))
				{
...
/*
				...
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".PCX") || Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".GIF") || Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".PNG"))
				{
					NEW_CLASS(imgItem, OrganImageItem(this->userId));
					imgItem->SetDispName(sptr);
					imgItem->SetIsCoverPhoto(isCoverPhoto);
				...
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".AVI") || Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".MOV") || Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".MTS") || Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".M2TS"))
				{
					NEW_CLASS(imgItem, OrganImageItem(this->userId));
					imgItem->SetDispName(sptr);
					imgItem->SetIsCoverPhoto(isCoverPhoto);
				...
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char *)".WAV"))
				{
					NEW_CLASS(imgItem, OrganImageItem(this->userId));
					imgItem->SetDispName(sptr);
					imgItem->SetIsCoverPhoto(isCoverPhoto);
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetGroupAllSpecies(Data::ArrayList<OrganSpecies*> *items, OrganGroup *grp)
{
	UOSInt i;
	UOSInt j;
	...
	{
		sql.Clear();
		sql.AppendCmd((const UTF8Char*)"select id from groups where parent_id in (");
		i = 0;
		j = thisId.GetCount();
		while (i < j)
		{
	...
	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"SELECT id, chi_name, sci_name, eng_name, description, dirName, photo, idKey, flags, photoId, mapColor, photoWId FROM species where group_id in (");
	i = 0;
	j = allId.GetCount();
	while (i < j)
	{
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetGroupAllUserFile(Data::ArrayList<UserFileInfo*> *items, Data::ArrayList<Int32> *colors, OrganGroup *grp)
{
	UOSInt i;
	UOSInt j;
	Data::ArrayListInt32 allId;
	...
	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"SELECT id, mapColor FROM species where group_id in (");
	i = 0;
	j = allId.GetCount();
	while (i < j)
	{
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetSpeciesItems(Data::ArrayList<OrganGroupItem*> *items, Data::ArrayList<Int32> *speciesIds)
{
	DB::DBReader *r;
	UOSInt i;
	UOSInt j;
	...
	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"SELECT id, chi_name, sci_name, eng_name, description, dirName, photo, idKey, flags, photoId, mapColor, photoWId FROM species where id in (");
	i = 0;
...
SSWR::OrganMgr::OrganGroup *SSWR::OrganMgr::OrganEnvWeb::GetGroup(Int32 groupId, Int32 *parentId)
{
	DB::SQLBuilder sql(this->db);
	DB::DBReader *r;
	Int32 photoGroup;
...
SSWR::OrganMgr::OrganSpecies *SSWR::OrganMgr::OrganEnvWeb::GetSpecies(Int32 speciesId)
{
	DB::SQLBuilder sql(this->db);
	DB::DBReader *r;
	OrganSpecies *sp = 0;
	sql.AppendCmd((const UTF8Char*)"SELECT id, chi_name, sci_name, eng_name, group_id, description, dirName, photo, idKey, flags, photoId, mapColor FROM species where id = ");
...
Bool SSWR::OrganMgr::OrganEnvWeb::IsBookSpeciesExist(const UTF8Char *sName, Text::StringBuilderUTF *sb)
{
	...
	{
		sql.Clear();
		sql.AppendCmd((const UTF8Char*)"select sci_name from species where id in (");
		i = 0;
		while (i < j)
		{
			if (i > 0)
...
Bool SSWR::OrganMgr::OrganEnvWeb::AddSpecies(OrganSpecies *sp)
{
	DB::SQLBuilder sql(this->db);
...
Bool SSWR::OrganMgr::OrganEnvWeb::DelSpecies(OrganSpecies *sp)
{
	DB::SQLBuilder sql(this->db);
	Bool succ = true;
	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"update groups set photo_group=NULL, photo_species=NULL where photo_species=");
	sql.AppendInt32(sp->GetSpeciesId());
	if (this->db->ExecuteNonQuery(sql.ToString()) < -1)
		succ = false;

	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"delete from species where id=");
	sql.AppendInt32(sp->GetSpeciesId());
	if (this->db->ExecuteNonQuery(sql.ToString()) < -1)
		succ = false;

	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"delete from species_book where species_id=");
	sql.AppendInt32(sp->GetSpeciesId());
	if (this->db->ExecuteNonQuery(sql.ToString()) < -1)
		succ = false;
	return succ;
...
SSWR::OrganMgr::OrganEnvWeb::FileStatus SSWR::OrganMgr::OrganEnvWeb::AddSpeciesFile(OrganSpecies *sp, const UTF8Char *fileName, Bool firstPhoto, Bool moveFile, Int32 *fileId)
{
	UOSInt i;
	UOSInt j;
	Int32 fileType = 0;
	...
	if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"JPG"))
	{
		fileType = 1;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"TIF"))
	{
		fileType = 1;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"PCX"))
	{
		fileType = 1;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"GIF"))
	{
		fileType = 1;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"PNG"))
	{
		fileType = 1;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"AVI"))
	{
		fileType = 2;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"MOV"))
	{
		fileType = 2;
	}
	else if (Text::StrEqualsICase(&fileName[i + j + 2], (const UTF8Char*)"WAV"))
	{
		fileType = 3;
	}
	else
	...
	if (fileType == 1)
	{
		...
		NEW_CLASS(mstm, IO::MemoryStream((Int32)fs->GetLength(), (const UTF8Char*)"OrganEnv.AddSpeciesFile"));
		while (true)
		{
...
SSWR::OrganMgr::OrganEnvWeb::FileStatus SSWR::OrganMgr::OrganEnvWeb::AddSpeciesWebFile(OrganSpecies *sp, const UTF8Char *srcURL, const UTF8Char *imgURL, IO::Stream *stm, UTF8Char *webFileName)
{
	...
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"insert into webfile (species_id, crcVal, imgUrl, srcUrl, prevUpdated, cropLeft, cropTop, cropRight, cropBottom, location) values (");
	sql.AppendInt32(sp->GetSpeciesId());
	sql.AppendCmd((const UTF8Char*)", ");
	sql.AppendInt32(crcVal);
	sql.AppendCmd((const UTF8Char*)", ");
...
SSWR::OrganMgr::OrganEnvWeb::FileStatus SSWR::OrganMgr::OrganEnvWeb::AddSpeciesWebFileOld(OrganSpecies *sp, const UTF8Char *srcURL, const UTF8Char *imgURL, IO::Stream *stm, UTF8Char *webFileName)
{
	UTF8Char sbuff[512];
...
{
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"update webfile set srcUrl=");
	sql.AppendStrUTF8(srcURL);
	sql.AppendCmd((const UTF8Char*)", location=");
...
Bool SSWR::OrganMgr::OrganEnvWeb::UpdateSpeciesWebFileOld(OrganSpecies *sp, const UTF8Char *webFileName, const UTF8Char *srcURL)
{
	UTF8Char sbuff[512];
	UTF8Char *sptr;
	sptr = this->GetSpeciesDir(sp, sbuff);
...
Bool SSWR::OrganMgr::OrganEnvWeb::SaveSpecies(OrganSpecies *sp)
{
	DB::SQLBuilder sql(this->db);
	...
	sql.AppendStrUTF8(sp->GetDirName());
	sql.AppendCmd((const UTF8Char*)", photo=");
	sql.AppendStrUTF8(sp->GetPhoto());
	sql.AppendCmd((const UTF8Char*)", photoId=");
	sql.AppendInt32(sp->GetPhotoId());
	sql.AppendCmd((const UTF8Char*)", photoWId=");
	sql.AppendInt32(sp->GetPhotoWId());
	sql.AppendCmd((const UTF8Char*)", idKey=");
	sql.AppendStrUTF8(sp->GetIDKey());
	sql.AppendCmd((const UTF8Char*)", mapColor=");
	sql.AppendInt32(sp->GetMapColor());
	sql.AppendCmd((const UTF8Char*)" where id=");
	sql.AppendInt32(sp->GetSpeciesId());
	return this->db->ExecuteNonQuery(sql.ToString()) >= -1;
}

Bool SSWR::OrganMgr::OrganEnvWeb::SaveGroup(OrganGroup *grp)
{
	Int32 flags = 0;
	if (grp->GetAdminOnly())
	{
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetGroupCount(Int32 groupId)
{
	DB::SQLBuilder sql(this->db);
	DB::DBReader *r;
	sql.AppendCmd((const UTF8Char*)"select count(*) from groups where parent_id = ");
	sql.AppendInt32(groupId);
	sql.AppendCmd((const UTF8Char*)" and cate_id = ");
	sql.AppendInt32(this->currCate->cateId);
	r = this->db->ExecuteReaderC(sql.ToString(), sql.GetLength());
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetSpeciesCount(Int32 groupId)
{
	DB::SQLBuilder sql(this->db);
	DB::DBReader *r;
	sql.AppendCmd((const UTF8Char*)"select count(*) from species where group_id = ");
	sql.AppendInt32(groupId);
	sql.AppendCmd((const UTF8Char*)" and cate_id = ");
	sql.AppendInt32(this->currCate->cateId);
 	r = this->db->ExecuteReaderC(sql.ToString(), sql.GetLength());
...
Bool SSWR::OrganMgr::OrganEnvWeb::AddGroup(OrganGroup *grp, Int32 parGroupId)
{
	Int32 flags = 0;
	if (grp->GetAdminOnly())
	{
...
Bool SSWR::OrganMgr::OrganEnvWeb::DelGroup(Int32 groupId)
{
	DB::SQLBuilder sql(this->db);
...
Bool SSWR::OrganMgr::OrganEnvWeb::SetGroupDefSp(OrganGroup *grp, OrganImageItem *img)
{
/*	DB::SQLBuilder sql(this->db);
	...
	if (oldId == -1)
	{
		sql.AppendCmd((const UTF8Char*)"update groups set photo_group=NULL, photo_species=");
		sql.AppendInt32(sp->GetSpeciesId());
		sql.AppendCmd((const UTF8Char*)" where id=");
		sql.AppendInt32(grp->GetGroupId());
		this->db->ExecuteNonQuery(sql.ToString());
	...
	else
	{
		sql.AppendCmd((const UTF8Char*)"update groups set photo_group=NULL, photo_species=");
		sql.AppendInt32(sp->GetSpeciesId());
		sql.AppendCmd((const UTF8Char*)" where photo_species=");
...
Bool SSWR::OrganMgr::OrganEnvWeb::MoveGroups(Data::ArrayList<OrganGroup*> *grpList, OrganGroup *destGroup)
{
	DB::SQLBuilder sql(this->db);
	Bool found;
	OSInt i;
	...
	sql.AppendCmd((const UTF8Char*)" where id in (");
	found = false;
	while (i < j)
	{
		if (found)
...
Bool SSWR::OrganMgr::OrganEnvWeb::MoveSpecies(Data::ArrayList<OrganSpecies*> *spList, OrganGroup *destGroup)
{
	DB::SQLBuilder sql(this->db);
	...
	sql.AppendInt32(destGroup->GetGroupId());
	sql.AppendCmd((const UTF8Char*)" where id in (");
	sql2.AppendCmd((const UTF8Char*)"update groups set photo_group=NULL, photo_species=NULL where photo_species in (");
	found = false;
	i = 0;
	j = spList->GetCount();
	while (i < j)
	...
	sql.AppendCmd((const UTF8Char*)")");
	sql2.AppendCmd((const UTF8Char*)")");
	db->ExecuteNonQuery(sql2.ToString());
	return db->ExecuteNonQuery(sql.ToString()) > 0;
}

Bool SSWR::OrganMgr::OrganEnvWeb::MoveImages(Data::ArrayList<OrganImages*> *imgList, OrganSpecies *destSp, UI::GUIForm *frm)
{
	UTF8Char sbuff[512];
	UTF8Char *sptr;
	UTF8Char *sptr2;
	...
	while (i < j)
	{
		...
		else if (img->GetImgItem()->GetFileType() == OrganImageItem::FT_WEBFILE)
		{
			...
			if (wfile->speciesId != destSp->GetSpeciesId())
			{
				...
				DB::SQLBuilder sql(this->db);
				sql.AppendCmd((const UTF8Char*)"update webfile set species_id = ");
		...
		else
		{
			...
			*sptr2++ = IO::Path::PATH_SEPERATOR;
			...
			{
				Text::StringBuilderUTF8 sb;
				const UTF8Char *csptr = Text::StrToUTF8New(L"移動");
				sb.Append(csptr);
				...
				sb.Append(csptr);
				Text::StrDelNew(csptr);
				const UTF8Char *title = Text::StrToUTF8New(L"錯誤");
				if (!UI::MessageDialog::ShowYesNoDialog(sb.ToString(), title, frm))
				{
			...
			*sptr++ = IO::Path::PATH_SEPERATOR;
			...
			while (reader->ReadLine(&sb, 512))
			{
				...
				if (Text::StrSplit(sarr, 4, sb2.ToString(), '\t') == 3)
				{
					...
					if (!found)
					{
						webLines.Add(Text::String::New(sb.ToString(), sb.GetLength()));
					}
				}
				sb.ClearStr();
				sb2.ClearStr();
...
Bool SSWR::OrganMgr::OrganEnvWeb::CombineSpecies(OrganSpecies *destSp, OrganSpecies *srcSp)
{
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"update species_book set species_id = ");
	sql.AppendInt32(destSp->GetSpeciesId());
	sql.AppendCmd((const UTF8Char*)" where species_id = ");
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetWebUsers(Data::ArrayList<OrganWebUser*> *userList)
{
	...
	if (r)
	{
		while (r->ReadNext())
		{
			...
			sb.ClearStr();
			r->GetStr(1, &sb);
			user->userName = Text::String::New(sb.ToString(), sb.GetLength());
			sb.ClearStr();
			r->GetStr(2, &sb);
			user->watermark = Text::String::New(sb.ToString(), sb.GetLength());
			userList->Add(user);
		}
		this->db->CloseReader(r);
	}
...
Bool SSWR::OrganMgr::OrganEnvWeb::AddWebUser(const UTF8Char *userName, const UTF8Char *pwd, const UTF8Char *watermark)
{
	DB::SQLBuilder sql(this->db);
...
Bool SSWR::OrganMgr::OrganEnvWeb::ModifyWebUser(Int32 id, const UTF8Char *userName, const UTF8Char *pwd, const UTF8Char *watermark)
{
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"select id from webuser where userName = ");
	sql.AppendStrUTF8(userName);
	...
	this->db->CloseReader(r);
	sql.Clear();
	sql.AppendCmd((const UTF8Char*)"update webuser set userName = ");
	sql.AppendStrUTF8(userName);
	if (pwd != 0)
	{
		OSInt len = Text::StrCharCnt(pwd);
		...
		md5.GetValue(md5Value);
		Text::StrHexBytes(sbuff, md5Value, 16, 0);
		sql.AppendCmd((const UTF8Char*)", pwd = ");
		sql.AppendStrUTF8(sbuff);
	}
	sql.AppendCmd((const UTF8Char*)", watermark = ");
	sql.AppendStrUTF8(watermark);
	sql.AppendCmd((const UTF8Char*)" where id = ");
	sql.AppendInt32(id);
	return this->db->ExecuteNonQuery(sql.ToString()) >= 0;
...
void SSWR::OrganMgr::OrganEnvWeb::ReleaseWebUsers(Data::ArrayList<OrganWebUser*> *userList)
{
	OrganWebUser *user;
	OSInt i = userList->GetCount();
	while (i-- > 0)
	{
...
Bool SSWR::OrganMgr::OrganEnvWeb::IsSpeciesBookExist(Int32 speciesId, Int32 bookId)
{
	DB::SQLBuilder sql(this->db);
	DB::DBReader *r;
	sql.AppendCmd((const UTF8Char*)"select dispName from species_book where species_id = ");
	sql.AppendInt32(speciesId);
...
Bool SSWR::OrganMgr::OrganEnvWeb::NewSpeciesBook(Int32 speciesId, Int32 bookId, const UTF8Char *dispName)
{
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"insert into species_book (species_id, book_id, dispName) values (");
	sql.AppendInt32(speciesId);
	sql.AppendCmd((const UTF8Char*)", ");
	sql.AppendInt32(bookId);
	sql.AppendCmd((const UTF8Char*)", ");
	...
	sql.AppendCmd((const UTF8Char*)")");
	if (this->db->ExecuteNonQuery(sql.ToString()) == -2)
	{
		return false;
	}
	else
...
UOSInt SSWR::OrganMgr::OrganEnvWeb::GetSpeciesBooks(Data::ArrayList<SpeciesBook*> *items, Int32 speciesId)
{
	DB::SQLBuilder sql(this->db);
	DB::DBReader *r;
	OrganBook *book;
	...
	while (r->ReadNext())
	{
		...
		if (i >= 0)
		{
			...
			sb.ClearStr();
			r->GetStr(1, &sb);
			spBook->dispName = Text::String::New(sb.ToString(), sb.GetLength());
...
void SSWR::OrganMgr::OrganEnvWeb::ReleaseSpeciesBooks(Data::ArrayList<SpeciesBook*> *items)
{
	SpeciesBook *spBook;
	OSInt i;
	i = items->GetCount();
...
Int32 SSWR::OrganMgr::OrganEnvWeb::NewBook(const UTF8Char *title, const UTF8Char *author, const UTF8Char *press, Data::DateTime *publishDate, const UTF8Char *url)
{
	OrganBook *book;
	DB::SQLBuilder sql(this->db);
	OSInt i;
...
Bool SSWR::OrganMgr::OrganEnvWeb::AddDataFile(const UTF8Char *fileName)
{
	IO::StmData::FileData *fd;
	IO::ParserType t;
	IO::ParsedObject *pobj;
...
Bool SSWR::OrganMgr::OrganEnvWeb::DelDataFile(DataFileInfo *dataFile)
{
	Bool found = false;
	UTF8Char sbuff[512];
	OSInt i;
	...
	*sptr++ = IO::Path::PATH_SEPERATOR;
	...
	IO::Path::DeleteFile(sbuff);
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"delete from datafile where id = ");
	...
	if (dataFile->fileType == 1)
	{
		...
		if (j >= 0)
		{
			webUser->gpsFileIndex->RemoveAt(j);
			webUser->gpsFileObj->RemoveAt(j);
		}
	}
...
Bool SSWR::OrganMgr::OrganEnvWeb::GetGPSPos(Int32 userId, Data::DateTime *t, Double *lat, Double *lon)
{
	OSInt i;
...
Map::GPSTrack *SSWR::OrganMgr::OrganEnvWeb::OpenGPSTrack(DataFileInfo *dataFile)
{
	UTF8Char sbuff[512];
	UTF8Char *sptr;
	IO::StmData::FileData *fd;
...
void SSWR::OrganMgr::OrganEnvWeb::UpdateUserFileCrop(UserFileInfo *userFile, Double cropLeft, Double cropTop, Double cropRight, Double cropBottom)
{
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"update userfile set cropLeft = ");
	sql.AppendDbl(cropLeft);
	sql.AppendCmd((const UTF8Char*)", cropTop = ");
	sql.AppendDbl(cropTop);
	sql.AppendCmd((const UTF8Char*)", cropRight = ");
	sql.AppendDbl(cropRight);
	sql.AppendCmd((const UTF8Char*)", cropBottom = ");
	sql.AppendDbl(cropBottom);
	sql.AppendCmd((const UTF8Char*)", prevUpdated = 1");
	sql.AppendCmd((const UTF8Char*)" where id = ");
	sql.AppendInt32(userFile->id);
	if (this->db->ExecuteNonQuery(sql.ToString()) >= 0)
	{
...
void SSWR::OrganMgr::OrganEnvWeb::UpdateUserFileRot(UserFileInfo *userFile, Int32 rotType)
{
...
Bool SSWR::OrganMgr::OrganEnvWeb::UpdateUserFilePos(UserFileInfo *userFile, Data::DateTime *captureTime, Double lat, Double lon)
{
	Bool succ = false;
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"update userfile set captureTime = ");
	sql.AppendDate(captureTime);
	sql.AppendCmd((const UTF8Char*)", lat = ");
	sql.AppendDbl(lat);
	sql.AppendCmd((const UTF8Char*)", lon = ");
	sql.AppendDbl(lon);
	sql.AppendCmd((const UTF8Char*)" where id = ");
	sql.AppendInt32(userFile->id);
	if (this->db->ExecuteNonQuery(sql.ToString()) >= 0)
...
Bool SSWR::OrganMgr::OrganEnvWeb::GetUserFilePath(UserFileInfo *userFile, Text::StringBuilderUTF *sb)
{
	Data::DateTime dt;
...
Bool SSWR::OrganMgr::OrganEnvWeb::UpdateUserFileDesc(UserFileInfo *userFile, const UTF8Char *descript)
{
	Bool succ = false;
	DB::SQLBuilder sql(this->db);
	...
	if (this->db->ExecuteNonQuery(sql.ToString()) >= 0)
	{
		...
		userFile->descript = Text::String::NewOrNull(descript);
	}
	return succ;
}

void SSWR::OrganMgr::OrganEnvWeb::UpdateWebFileCrop(WebFileInfo *wfile, Double cropLeft, Double cropTop, Double cropRight, Double cropBottom)
{
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"update webfile set cropLeft = ");
	sql.AppendDbl(cropLeft);
	sql.AppendCmd((const UTF8Char*)", cropTop = ");
	sql.AppendDbl(cropTop);
	sql.AppendCmd((const UTF8Char*)", cropRight = ");
...
void SSWR::OrganMgr::OrganEnvWeb::TripReload(Int32 cateId)
{
	DB::DBReader *r;
	DB::SQLBuilder sql(this->db);
	UTF8Char sbuff[256];
	...
	sql.AppendInt32(cateId);
	sql.AppendCmd((const UTF8Char*)" and webuser_id = ");
	sql.AppendInt32(this->userId);
...
Bool SSWR::OrganMgr::OrganEnvWeb::TripAdd(Data::DateTime *fromDate, Data::DateTime *toDate, Int32 locId)
{
	...
	if (i < 0 && j < 0 && k >= 0)
	{
		...
		{
			DB::SQLBuilder sql(this->db);
			sql.AppendCmd((const UTF8Char*)"insert into trip (fromDate, toDate, locId, cate_id, webuser_id) values (");
...
Bool SSWR::OrganMgr::OrganEnvWeb::LocationUpdate(Int32 locId, const UTF8Char *engName, const UTF8Char *chiName)
{
	...
	DB::SQLBuilder sql(this->db);
	sql.AppendCmd((const UTF8Char*)"update location set ename=");
	sql.AppendStrUTF8(engName);
	sql.AppendCmd((const UTF8Char*)", cname=");
	...
	else
	{
		...
		loc->ename = Text::String::NewNotNull(engName);
		loc->cname = Text::String::NewNotNull(chiName);
		return true;
	}
}

Bool SSWR::OrganMgr::OrganEnvWeb::LocationAdd(Int32 locId, const UTF8Char *engName, const UTF8Char *chiName)
{
	Location *loc;
	Int32 lType;
	if (locId == 0)
	...
	if (lType + 1 >= this->locType->GetCount())
		return false;
...
void SSWR::OrganMgr::OrganEnvWeb::UpgradeFileStruct(OrganSpecies *sp)
{
	UTF8Char sbuff[512];
	UTF8Char *sptr;
	Int32 newFlags = 0;
	...
	SpeciesInfo *spInfo = this->GetSpeciesInfo(sp->GetSpeciesId(), true);
...
void SSWR::OrganMgr::OrganEnvWeb::BooksInit()
{
	OrganBook *book;
	Text::StringBuilderUTF8 sb;
	DB::DBReader *r;
	...
	r = this->db->ExecuteReader((const UTF8Char*)"select id, title, dispAuthor, press, publishDate, groupId, url from book");
	while (r->ReadNext())
	{
		NEW_CLASS(book, OrganBook());
...
Media::ImageList *SSWR::OrganMgr::OrganEnvWeb::ParseImage(OrganImageItem *img, UserFileInfo **outUserFile, WebFileInfo **outWebFile)
{
	IO::StmData::FileData *fd;
	IO::ParserType pt;
...
Media::ImageList *SSWR::OrganMgr::OrganEnvWeb::ParseSpImage(OrganSpecies *sp)
{
	UTF8Char sbuff[512];
	UTF8Char sbuff2[512];
	UTF8Char *sptr;
	...
	if (sess)
	{
		if (IO::Path::FindNextFile(sptr, sess, 0, &pt, 0))
		{
			if (pt == IO::Path::PathType::File)
			{
				...
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char*)".AVI"))
				{
				}
				else if (Text::StrEqualsICase(&sptr[i], (const UTF8Char*)".WAV"))
				{
...
Media::ImageList *SSWR::OrganMgr::OrganEnvWeb::ParseFileImage(UserFileInfo *userFile)
{
	IO::StmData::FileData *fd;
	IO::ParserType pt;
	UTF8Char sbuff[512];
...
Media::ImageList *SSWR::OrganMgr::OrganEnvWeb::ParseWebImage(WebFileInfo *wfile)
{
	IO::StmData::FileData *fd;
	IO::ParserType pt;
	UTF8Char sbuff[512];
...
SSWR::OrganMgr::OrganGroup *SSWR::OrganMgr::OrganEnvWeb::SearchObject(const UTF8Char *searchStr, UTF8Char *resultStr, UOSInt resultBuffSize, Int32 *parentId)
{
	OrganGroup *foundGroup = 0;
	Bool found = false;
	UTF8Char sbuff[256];
	...
	sql.AppendCmd((const UTF8Char*)"select group_id, sci_name from species where chi_name = ");
	...
	if (r)
	{
		if (r->ReadNext())
		{
			...
			*parentId = r->GetInt32(0);
			r->GetStr(1, resultStr, resultBuffSize);
		}
		this->db->CloseReader(r);
	}
	if (!found)
	...
	{
		sql.AppendCmd((const UTF8Char*)"select id, group_type, eng_name, chi_name, description, parent_id, photo_group, photo_species, idKey, flags from groups where eng_name like ");
		Text::StrConcat(Text::StrConcat(Text::StrConcat(sbuff, (const UTF8Char*)"%"), searchStr), (const UTF8Char*)"%");
		sql.AppendStrUTF8(sbuff);
		sql.AppendCmd((const UTF8Char*)" or chi_name like ");
		sql.AppendStrUTF8(sbuff);
...
void SSWR::OrganMgr::OrganEnvWeb::LoadGroupTypes()
{
...
void SSWR::OrganMgr::OrganEnvWeb::Test()
{
	UpgradeDB2();
	...
	while (i < j)
	{
		...
		if (userFile->fileType == 1)
		{
			...
			if (exif)
			{
				...
				{
					sql.Clear();
					sql.AppendCmd((const UTF8Char*)"update userfile set camera = ");
...
void SSWR::OrganMgr::OrganEnvWeb::UpgradeDB2()
{
	UTF8Char sbuff[512];
	UTF8Char sbuff2[512];
	UTF8Char *sptr;
	...
	DB::SQLBuilder sql(this->db);
	r = this->db->ExecuteReader((const UTF8Char*)"select id, dirName, photo, photoId, photoWId from species");
	if (r)
	{
		while (r->ReadNext())
		{
			...
			sb.ClearStr();
			r->GetStr(1, &sb);
			sp->dirName = Text::String::New(sb.ToString(), sb.GetLength());
			sb.ClearStr();
			r->GetStr(2, &sb);
			sp->photoName = Text::String::New(sb.ToString(), sb.GetLength());
							...
							*sptr2++ = IO::Path::PATH_SEPERATOR;
							...
							else
							{
								if (isCover)
								{
									...
									sql.Clear();
									sql.AppendCmd((const UTF8Char*)"update species set photo = ");
									sql.AppendStrUTF8(0);
									sql.AppendCmd((const UTF8Char*)", photoWId = ");
									sql.AppendInt32(id);
									sql.AppendCmd((const UTF8Char*)" where id = ");
...
void SSWR::OrganMgr::OrganEnvWeb::ExportLite(const UTF8Char *folder)
{
	UTF8Char sbuff[512];
	UTF8Char sbuff2[512];
	UTF8Char *sptr;
