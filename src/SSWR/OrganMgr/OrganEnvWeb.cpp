...
const UTF8Char *SSWR::OrganMgr::OrganEnvWeb::GetDataDir()
{
	return this->cfgDataPath;
}

const UTF8Char *SSWR::OrganMgr::OrganEnvWeb::GetCacheDir()
{
...
Bool SSWR::OrganMgr::OrganEnvWeb::AddDataFile(const UTF8Char *fileName)
{
	...
	if (pobj)
	{
		...
		else if (t == IO::ParsedObject::PT_READINGDB_PARSER)
		{
			...
			if (tables.GetCount() == 2)
			{
				...
				{
					DB::DBReader *reader = db->GetTableData((const UTF8Char*)"Records", 0, 0, 0);
					Bool found = false;
					if (reader)
					{
		...
		while (r->ReadNext())
		{
			...
			if (IO::Path::GetPathType(sbuff) == IO::Path::PT_UNKNOWN)
			{
				if (fileType == 1 && Text::StrEndsWithICase(sptr2, (const UTF8Char*)".jpg"))
				{
					...
					Media::ImageList *imgList = (Media::ImageList*)this->parsers->ParseFileType(fd, IO::ParsedObject::PT_IMAGE_LIST_PARSER);
					DEL_CLASS(fd);
