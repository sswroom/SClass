...
/*	case MNU_OPEN_ODBC:
		{
			SSWR::AVIRead::AVIRODBCForm dlg(nullptr, this->ui, this->core);
			dlg.ShowDialog(this);
		}
		break;
	...
	case MNU_SET_COLOR:
		{
			SSWR::AVIRead::AVIRColorSettingForm dlg(nullptr, this->ui, this->core);
			dlg.ShowDialog(this);
		}
		break;
	case MNU_GEN_IMAGE:
	...
	case MNU_CAP_DEV:
		{
			...
			if (dlg.ShowDialog(this) == UI::GUIForm::DR_OK)
			{
				...
				NEW_CLASSNN(mf, Media::MediaFile(wbuff));
	...
	case MNU_MTK_GPS:
		{
			...
			NN<IO::MTKGPSNMEA> mtk;
			NN<Map::GPSTrack> trk;

			NEW_CLASSNN(port, IO::SerialPort(IO::MTKGPSNMEA::GetMTKSerialPort(), 115200, true));
			if (!port->IsError())
			{
				NEW_CLASSNN(mtk, IO::MTKGPSNMEA(port));
				if (mtk->IsMTKDevice())
				{
					NEW_CLASSNN(trk, Map::GPSTrack(L"MTK_Tracker", true, 0));
					if (mtk->ParseLog(trk))
					{
						core->OpenObject(trk);
					}
	...
	case MNU_GPS_TRACKER:
		{
			...
			if (frm.ShowDialog(this) == UI::GUIForm::DR_OK)
			{
				...
				NN<IO::GPSNMEA> gps;
				NEW_CLASSNN(gps, IO::GPSNMEA(frm->port));
				NEW_CLASSNN(gpsFrm, SSWR::AVIRead::AVIRGPSTrackerForm(this->ui->GetHInst(), 0, this->ui, this->core, frm->port, gps));
	...
	case MNU_OSM_TILE:
		...
		NEW_CLASSNN(tileMap, Map::OSM::OSMTileMap(L"http://a.tile.openstreetmap.org/", wbuff, 18, this->core->GetSocketFactory()));
	...
	case MNU_OSM_CYCLE:
		...
		NEW_CLASSNN(tileMap, Map::OSM::OSMTileMap(L"http://a.tile.opencyclemap.org/cycle/", wbuff, 18, this->core->GetSocketFactory()));
	...
	case MNU_OSM_TRANSP:
		...
		NEW_CLASSNN(tileMap, Map::OSM::OSMTileMap(L"http://a.tile2.opencyclemap.org/transport/", wbuff, 18, this->core->GetSocketFactory()));
	...
	case MNU_OSM_MAPQUEST:
		...
		NEW_CLASSNN(tileMap, Map::OSM::OSMTileMap(L"http://otile1.mqcdn.com/tiles/1.0.0/osm/", wbuff, 18, this->core->GetSocketFactory()));
	...
	case MNU_ESRI_MAP:
		{
			...
			if (frm.ShowDialog(this) == UI::GUIForm::DR_OK)
			{
				...
				NEW_CLASSNN(map, Map::ESRI::ESRITileMap(url, wbuff, this->core->GetSocketFactory()));
...
/*	case MNU_PLAYLIST:
		{
			...
			NEW_CLASSNN(playlist, Media::Playlist(L"Untitled", this->core->GetParserList()));
	...
	case MNU_HTTP_SVR:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRHTTPSvrForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_TV_CONTROL:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRTVControlForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_STMTERM:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRStreamTermForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_NTPCLIENT:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRNTPClientForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_SET_AUDIO:
	...
	case MNU_MPEG_TOOL:
		{
			NN<SSWR::AVIRead::AVIRMPEGToolForm> frm;
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRMPEGToolForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_SET_DPI:
	...
	case MNU_DNSCLIENT:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRDNSClientForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_FILEEX:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRFileExForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_DNSPROXY:
	...
	case MNU_SMTPSERVER:
		{
			NN<SSWR::AVIRead::AVIRSMTPServerForm> frm;
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRSMTPServerForm(0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	...
	case MNU_TEXT_ENCRYPT:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIREncryptForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}
		break;
	case MNU_STREAM_CONV:
		{
			...
			NEW_CLASSNN(frm, SSWR::AVIRead::AVIRStreamConvForm(hInst, 0, this->ui, this->core));
			this->core->ShowForm(frm);
		}*/
	}
}
