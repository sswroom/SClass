...
void __stdcall SSWR::AVIRead::AVIRDWQB30Form::OnModeSettingClicked(void *userObj)
{
	...
	if (me->scanner)
	{
		...
		while (i < j)
		{
			me->cboSetCmd->AddItem(me->scanner->GetCommandName(cmdList.GetItem(i)).OrEmpty(), (void*)(OSInt)cmdList.GetItem(i));
			i++;
		}
		if (j > 0)
		{
	...
	if (me->scanner)
	{
		...
		if (i != INVALID_INDEX)
		{
			...
			else if (me->cmdType == IO::CodeScanner::CT_SELECT_COMMAND)
			{
				...
				while (i <= me->cmdMax)
				{
					me->cboSetParam->AddItem(me->scanner->GetCommandParamName(dcmd, i).OrEmpty(), (void*)(OSInt)i); 
					i++;
				}
			}
...
void __stdcall SSWR::AVIRead::AVIRDWQB30Form::OnSetCmdClicked(void *userObj)
{
	...
	if (me->scanner)
	{
		...
		else if (me->cmdType == IO::CodeScanner::CT_GET_COMMAND_NAME)
		{
			...
			{
				me->txtSetCmd->SetText(me->scanner->GetCommandParamName(me->cmdCurr, val).OrEmpty());
			}
		}
		else if (me->cmdType == IO::CodeScanner::CT_SELECT_COMMAND)
		{
...
SSWR::AVIRead::AVIRDWQB30Form::AVIRDWQB30Form(UI::GUIClientControl *parent, NotNullPtr<UI::GUICore> ui, NotNullPtr<SSWR::AVIRead::AVIRCore> core) : UI::GUIForm(parent, 640, 360, ui)
{
