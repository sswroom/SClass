...
void __stdcall SSWR::AVIRead::AVIRCameraControlForm::OnFilesDblClick(void *userObj, UOSInt index)
{
	...
	if (file->fileType == IO::CameraControl::FT_IMAGE)
	{
		if (Text::StrEndsWithICase(file->fileName, (const UTF8Char*)".JPG"))
		{
			...
			NEW_CLASS(mstm, IO::MemoryStream((const UTF8Char*)"SSWR.AVIRead.AVIRCameraControlForm.OnFilesDblClick.mstm"));
			if (me->camera->GetFile(file, mstm))
			{
				...
				IO::ParsedObject *pobj = me->core->GetParserList()->ParseFileType(fd, IO::ParsedObject::PT_IMAGE_LIST_PARSER);
				DEL_CLASS(fd);
				if (pobj)
				{
...
void __stdcall SSWR::AVIRead::AVIRCameraControlForm::OnFilesSelChg(void *userObj)
{
	...
	IO::MemoryStream *mstm;
	NEW_CLASS(mstm, IO::MemoryStream((const UTF8Char*)"SSWR.AVIRead.AVIROlympusCameraForm.OnFilesSelChg.mstm"));
	if (me->camera->GetThumbnailFile(file, mstm))
	{
		...
		IO::StmData::MemoryData *fd;
		NEW_CLASS(fd, IO::StmData::MemoryData(buff, size));
		previewImg = (Media::ImageList*)me->core->GetParserList()->ParseFileType(fd, IO::ParsedObject::PT_IMAGE_LIST_PARSER);
		DEL_CLASS(fd);
		if (previewImg)
		{
