...
void __stdcall SSWR::AVIRead::AVIRSetDPIForm::OnDPIChanged(void *userObj, OSInt newVal)
{
	SSWR::AVIRead::AVIRSetDPIForm *me = (SSWR::AVIRead::AVIRSetDPIForm*)userObj;
	UTF8Char sbuff[32];
...
void SSWR::AVIRead::AVIRSetDPIForm::UpdatePreview()
{
	...
	if (w > 0 && h > 0)
	{
		...
		f = gimg->NewFontH(L"Arial", 12 * v * 0.1 / ddpi, Media::DrawEngine::DFS_ANTIALIAS, 0);
		p = gimg->NewPenARGB(0xff000000, 1, 0, 0);
		b = gimg->NewBrushARGB(0xff000000);
		currV = 0;
		gimg->GetTextSize(f, L"0", -1, sz);
		initX = sz[0] * 0.5;
		lastX = initX - 20.0;
		while (true)
		{
			currX = Math::Unit::Distance::Convert(Math::Unit::Distance::DU_CENTIMETER, Math::Unit::Distance::DU_INCH, currV) * v * 0.1 + initX;
			if (currX > w)
				break;
			...
			if (currX >= lastX + 20)
			{
				...
				gimg->GetTextSizeUTF8(f, sbuff, -1, sz);
				gimg->DrawLine(currX, 0, currX, h - sz[1], p);
				gimg->DrawStringUTF8(currX - sz[0] * 0.5, h - sz[1], sbuff, f, b);
				lastX = currX;
			}
			currV++;
		}
...
SSWR::AVIRead::AVIRSetDPIForm::AVIRSetDPIForm(UI::GUIClientControl *parent, UI::GUICore *ui, SSWR::AVIRead::AVIRCore *core) : UI::GUIForm(parent, 1024, 174, ui)
{
	...
	this->hsbDPI->InitScrollBar(100, 3010, Math::Double2Int32(this->core->GetMonitorHDPI(this->GetHMonitor()) * 10), 10);
...
void SSWR::AVIRead::AVIRSetDPIForm::OnMonitorChanged()
{
	...
	this->txtMagnifyRatio->SetText(sbuff);
	this->hsbDPI->SetPos(Math::Double2Int32(hdpi * 10));
