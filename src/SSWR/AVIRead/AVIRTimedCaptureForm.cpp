...
void __stdcall SSWR::AVIRead::AVIRTimedCaptureForm::OnTimerTick(void *userObj)
{
	SSWR::AVIRead::AVIRTimedCaptureForm *me = (SSWR::AVIRead::AVIRTimedCaptureForm *)userObj;
	UTF8Char sbuff[32];
	Text::StrInt32(sbuff, me->frameCnt);
	me->txtFrameCnt->SetText(sbuff);
	Text::StrInt32(sbuff, me->saveCnt);
...
void __stdcall SSWR::AVIRead::AVIRTimedCaptureForm::OnVideoFrame(UInt32 frameTime, UInt32 frameNum, UInt8 **imgData, UOSInt dataSize, Media::IVideoSource::FrameStruct frameStruct, void *userData, Media::FrameType frameType, Media::IVideoSource::FrameFlag flags, Media::YCOffset ycOfst)
{
	...
	if (me->lastSaveTime + me->interval <= frameTime)
	{
		...
		NEW_CLASS(mstm, IO::MemoryStream((const UTF8Char*)"SSWR.AVIRead.AVIRTimedCaptureForm.OnVidoeFrame.mstm"));
		me->exporter->ExportFile(mstm, (const UTF8Char*)"Temp", imgList, param);
...
SSWR::AVIRead::AVIRTimedCaptureForm::AVIRTimedCaptureForm(UI::GUIClientControl *parent, UI::GUICore *ui, SSWR::AVIRead::AVIRCore *core) : UI::GUIForm(parent, 652, 480, ui)
{
	...
	NEW_CLASS(this->lblInterval, UI::GUILabel(ui, this->tpControl, (const UTF8Char*)"Capture Interval(ms)"));
	this->lblInterval->SetRect(4, 28, 100, 23, false);
	NEW_CLASS(this->txtInterval, UI::GUITextBox(ui, this->tpControl, (const UTF8Char*)"60000"));
	this->txtInterval->SetRect(104, 28, 100, 23, false);
	NEW_CLASS(this->lblJPGQuality, UI::GUILabel(ui, this->tpControl, (const UTF8Char*)"JPG Quality(%)"));
	this->lblJPGQuality->SetRect(4, 52, 100, 23, false);
	NEW_CLASS(this->txtJPGQuality, UI::GUITextBox(ui, this->tpControl, (const UTF8Char*)"95"));
	this->txtJPGQuality->SetRect(104, 52, 100, 23, false);
	NEW_CLASS(this->lblFileName, UI::GUILabel(ui, this->tpControl, (const UTF8Char*)"File Name"));
	this->lblFileName->SetRect(4, 76, 100, 23, false);
	NEW_CLASS(this->txtFileName, UI::GUITextBox(ui, this->tpControl, (const UTF8Char*)"TimedCapture.til"));
	...
	NEW_CLASS(this->txtDeviceInfo, UI::GUITextBox(ui, this->tpControl, (const UTF8Char*)"", true));
	...
	NEW_CLASS(this->txtFrameCnt, UI::GUITextBox(ui, this->tpStatus, (const UTF8Char*)""));
	this->txtFrameCnt->SetReadOnly(true);
	this->txtFrameCnt->SetRect(104, 4, 100, 23, false);
	NEW_CLASS(this->lblSaveCnt, UI::GUILabel(ui, this->tpStatus, (const UTF8Char*)"Save Count"));
	this->lblSaveCnt->SetRect(4, 28, 100, 23, false);
	NEW_CLASS(this->txtSaveCnt, UI::GUITextBox(ui, this->tpStatus, (const UTF8Char*)""));
