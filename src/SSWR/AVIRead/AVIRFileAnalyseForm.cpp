...
void __stdcall SSWR::AVIRead::AVIRFileAnalyseForm::OnTimerTick(AnyType userObj)
{
	...
	if (me->file.SetTo(file))
	{
		UIntOS currCnt = file->GetFrameCount();
		IntOS i;
		IntOS j;
		if (currCnt != me->lastPackCount)
		{
			i = (IntOS)(currCnt - 1) / PER_PAGE;
			j = (IntOS)(me->lastPackCount - 1) / PER_PAGE;
			if (currCnt == 0)
				i = -1;
			...
			if (me->lastPackCount % PER_PAGE)
			{
				if (j == i)
				{
					...
					me->lbPackList->RemoveItem((UIntOS)j);
...
void __stdcall SSWR::AVIRead::AVIRFileAnalyseForm::OnPackListChanged(AnyType userObj)
{
	...
	UIntOS j;
	me->lbPackItems->ClearItems();
	i = (UIntOS)me->lbPackList->GetSelectedIndex();
	if (i == (UIntOS)-1)
		return;
	i = i * PER_PAGE;
	j = i + PER_PAGE;
...
void __stdcall SSWR::AVIRead::AVIRFileAnalyseForm::OnPackItemChanged(AnyType userObj)
{
	...
	Text::StringBuilderUTF8 sb;
	UIntOS i = (UIntOS)me->lbPackItems->GetSelectedIndex();
	if (i == (UIntOS)-1)
	{
		me->txtPack->SetText(CSTR(""));
		return;
	}
	i = (UIntOS)me->lbPackItems->GetItem(i).p;
	sb.ClearStr();
