...
{
	SSWR::AVIRead::AVIRBluetoothLogForm *me = (SSWR::AVIRead::AVIRBluetoothLogForm*)userObj;
	const IO::BTLog::LogEntry *log = (const IO::BTLog::LogEntry*)me->lvContent->GetItem(index);
	if (log == 0)
		return;
	...
	if (frm->ShowDialog(me) == UI::GUIForm::DR_OK)
	{
		...
		while (i < j)
		{
			log = (const IO::BTLog::LogEntry*)me->lvContent->GetItem(i);
...
Bool SSWR::AVIRead::AVIRBluetoothLogForm::LogFileStore()
{
	...
	IO::Path::AppendPath(sbuff, (const UTF8Char*)"BTLog.txt");
...
void SSWR::AVIRead::AVIRBluetoothLogForm::LogUIUpdate()
{
	...
	IO::BTLog::LogEntry *log;
	Data::ArrayList<IO::BTLog::LogEntry*> *logList = this->btLog->GetLogList();
