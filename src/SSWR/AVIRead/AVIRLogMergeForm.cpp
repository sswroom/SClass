...
void __stdcall SSWR::AVIRead::AVIRLogMergeForm::OnConvertClicked(void *userObj)
{
	...
	else
	{
		...
		NEW_CLASS(fs2, IO::FileStream(sb2.ToString(), IO::FileMode::ReadOnly, IO::FileShare::DenyNone, IO::FileStream::BufferType::Normal));
		NEW_CLASS(reader2, Text::UTF8Reader(fs2));
		NEW_CLASS(fs3, IO::FileStream(sb3.ToString(), IO::FileMode::Create, IO::FileShare::DenyNone, IO::FileStream::BufferType::Normal));
		if (fs3->IsError())
		{
		...
		else
		{
			...
			while (succ1 || succ2)
			{
				if (succ1 && succ2)
				{
					...
					{
						sb1.ToString()[i] = 0;
						dt.SetValue(sb1.ToString());
						t1 = dt.ToTicks();
						sb1.ToString()[i] = '\t';
						sb2.ToString()[j] = 0;
						dt.SetValue(sb2.ToString());
						t2 = dt.ToTicks();
						sb2.ToString()[j] = '\t';
						if (t1 == 0)
						{
