...
void __stdcall SSWR::AVIRead::AVIRHTTPClientCertTestForm::OnStartClick(void *userObj)
{
	...
	ssl->SetRequireClientCert(Net::SSLEngine::ClientCertType::Optional);
	sb.ClearStr();
	me->txtClientCA->GetText(&sb);
	if (sb.leng != 0)
	{
		ssl->SetClientCA(sb.ToCString());
	}
	...
	if (me->sslCert == 0 || me->sslKey == 0)
	{
		...
		if (!ssl->SetServerCertsASN1(me->sslCert, me->sslKey, 0))
		{
	...
	if (port > 0 && port < 65535)
	{
		...
		NEW_CLASS(me->svr, Net::WebServer::WebListener(me->core->GetSocketFactory(), ssl, me, port, 120, Sync::Thread::GetThreadCnt(), CSTRP(sbuff, sptr), false, Net::WebServer::KeepAlive::Default, false));
		if (me->svr->IsError())
		{
			valid = false;
			SDEL_CLASS(me->svr);
