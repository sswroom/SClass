	...
	while (i < j)
	{
...

		{
			...
			while (succ && it.HasNext())
			{
				...
				succ = succ & zip.AddFile(s->ToCString().Substring((UIntOS)(filePath - sbuff)), rfs, lastModTime, accessTime, createTime, Data::Compress::Deflater::CompLevel::BestCompression, unixAttr);
			}
		}
