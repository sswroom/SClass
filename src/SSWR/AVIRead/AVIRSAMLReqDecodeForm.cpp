...
void __stdcall SSWR::AVIRead::AVIRSAMLReqDecodeForm::OnDecodeClicked(void *userObj)
{
	...
	if (Net::SAMLUtil::DecodeRequest(sb.ToCString(), &sbResult))
	{
		...
		sb.ClearStr();
		IO::MemoryStream mstm(sbResult.v, sbResult.leng, UTF8STRC("SSWR.AVIRead.AVIRSAMLReqDecodeForm.OnDecodeClicked.mstm"));
...
SSWR::AVIRead::AVIRSAMLReqDecodeForm::AVIRSAMLReqDecodeForm(UI::GUIClientControl *parent, UI::GUICore *ui, SSWR::AVIRead::AVIRCore *core) : UI::GUIForm(parent, 1024, 768, ui)
{
	this->core = core;
