...
void SSWR::AVIRead::AVIRCore::SetAudioDevice(Data::ArrayList<const UTF8Char *> *devList)
{
	...
	if (reg)
	{
		...
		else
		{
			...
			while (i < j)
			{
				Text::StrOSInt(Text::StrConcat(sbuff, L"AudioDevice"), i);
				const WChar *wptr = Text::StrToWCharNew(devList->GetItem(i));
				reg->SetValue(sbuff, wptr);
				Text::StrDelNew(wptr);
				i++;
			}
			Text::StrOSInt(Text::StrConcat(sbuff, L"AudioDevice"), j);
			reg->DelValue(sbuff);
		}
		IO::Registry::CloseRegistry(reg);
	}
...
Bool SSWR::AVIRead::AVIRCore::GenLinePreview(Media::DrawImage *img, Media::DrawEngine *eng, UOSInt lineThick, Int32 lineColor, Media::ColorConv *colorConv)
{
	...
	img->DrawLine(0, img->GetHeight() * 0.5, Math::OSInt2Double(img->GetWidth()), img->GetHeight() * 0.5, p);
	img->DelPen(p);
	return true;
}
...
Bool SSWR::AVIRead::AVIRCore::GenFontStylePreview(Media::DrawImage *img, Media::DrawEngine *eng, Map::MapEnv *env, UOSInt fontStyle, Media::ColorConv *colorConv)
{
	...
	if (env->GetFontStyle(fontStyle, &fontName, &fontSize, &bold, &fontColor, &buffSize, &buffColor))
	{
		buffSize = Math::Double2Int32(buffSize * dpi / 96.0);
...
Bool SSWR::AVIRead::AVIRCore::GenFontPreview(Media::DrawImage *img, Media::DrawEngine *eng, const UTF8Char *fontName, Double fontSize, Int32 fontColor, Media::ColorConv *colorConv)
{
	if (fontName == 0)
	{
