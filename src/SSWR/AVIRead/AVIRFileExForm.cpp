...
void __stdcall SSWR::AVIRead::AVIRFileExForm::OnStartClicked(void *userObj)
{
	...
	sb.ClearStr();
	me->txtSrc->GetText(&sb);
	NEW_CLASS(srcFS, IO::FileStream(sb.ToString(), IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_SEQUENTIAL));
	if (srcFS->IsError())
	{
		DEL_CLASS(srcFS);
	...
	sb.ClearStr();
	me->txtDest->GetText(&sb);
	NEW_CLASS(destFS, IO::FileStream(sb.ToString(), IO::FileStream::FILE_MODE_CREATE, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
	if (destFS->IsError())
	{
		DEL_CLASS(destFS);
		DEL_CLASS(srcFS);
...
void __stdcall SSWR::AVIRead::AVIRFileExForm::OnFileDrop(void *userObj, const UTF8Char **files, OSInt fileCnt)
{
	SSWR::AVIRead::AVIRFileExForm *me = (SSWR::AVIRead::AVIRFileExForm*)userObj;
