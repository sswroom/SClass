...
void SSWR::AVIRead::AVIRAudioViewerForm::UpdateImages()
{
	...
	if (this->totalSample <= 0)
	{
		...
		f = gimg->NewFontH(L"Arial", 10, Media::DrawEngine::DFS_ANTIALIAS, 0);
		gimg->DrawString(0, 0, L"Format not supported", f, b);
		gimg->DelFont(f);
		gimg->DelBrush(b);
		...
		if (channelH > 0)
		{
			...
			if (this->format->bitpersample == 16)
			{
				...
				while (currCh-- > 0)
				{
					...
					while (i < w)
					{
						thisY = currY + channelH + (((*(Int16*)&buff[j]) * channelH) >> 15);
						gimg->DrawLine(Math::OSInt2Double(i - 1), Math::OSInt2Double(lastY), Math::OSInt2Double(i), Math::OSInt2Double(thisY), p);
...
//		OSInt align = this->format->align;
		...
		if (i < 0)
		{
			MemClear(buff, this->format->align * -i);
			i = this->audSrc->ReadSample(0, FFTSAMPLE + i + FFTAVG - 1, buff + (this->format->align * -i)) - i;
		}
		else
		{
		...
		while (i < this->format->nChannels)
		{
			...
			{
				Data::RandomOS rand;
				p = gimg->NewPenARGB(rand.NextInt30() | 0xff000000, 1, 0, 0);
			}
			...
			while (j < (FFTSAMPLE / 2))
			{
				...
				thisX = j * w / (Double)(FFTSAMPLE / 2 - 1);
				thisY = -Math::Log10(rVal / (FFTSAMPLE * 0.5)) * h / 7.0;
				if (lastX >= 0)
				{
...
SSWR::AVIRead::AVIRAudioViewerForm::AVIRAudioViewerForm(UI::GUIClientControl *parent, UI::GUICore *ui, SSWR::AVIRead::AVIRCore *core, Media::IAudioSource *audSrc) : UI::GUIForm(parent, 1024, 768, ui)
{
	...
	this->currSample = 0;
	this->totalSample = this->audSrc->GetSampleCount();
