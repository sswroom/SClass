...
Sync::RWMutexUsage::RWMutexUsage(NotNullPtr<const Sync::RWMutex> mut, Bool writing)
{
	this->mut = mut.Ptr();
...
void Sync::RWMutexUsage::ReplaceMutex(NotNullPtr<const Sync::RWMutex> mut, Bool writing)
{
	if (!this->used)
	{
		this->mut = mut.Ptr();
