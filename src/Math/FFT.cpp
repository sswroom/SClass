...
void Math::FFT::ApplyWindow(Double *complexData, UOSInt sampleCount, WindowType wtype)
{
	...
	case WT_TRIANGULAR:
		...
		while (j < k)
		{
			complexData[j * 2] = complexData[j * 2] * (Math::UOSInt2Double(j) + 0.5) / dk;
			j++;
		}
		while (j < sampleCount)
		{
			complexData[j * 2] = complexData[j * 2] * (Math::UOSInt2Double(sampleCount - j) + 0.5) / dk;
			j++;
		}
		break;
...
Bool Math::FFT::Forward(ComplexNumber *data, UOSInt sampleCount)
{
	...
	ComplexNumber odd;
	ComplexNumber even;
	Double dsampleCount = Math::UOSInt2Double(sampleCount);
	i = 0;
	while (i < j)
	{
