...
extern "C"
{
	void FFTCalc_ApplyWindowI16(Double *complexOut, UInt8 *sampleIn, Double *sampleWindow, UIntOS sampleCnt, UIntOS sampleAdd, Double sampleMul);
	void FFTCalc_ApplyWindowI24(Double *complexOut, UInt8 *sampleIn, Double *sampleWindow, UIntOS sampleCnt, UIntOS sampleAdd, Double sampleMul);
	void FFTCalc_FFT2Freq(Double *freq, Double *complexIn, UIntOS sampleCnt);
	UIntOS FFTCalc_Forward(Double *complexData, UIntOS sampleCount);
}
...
void Math::FFTCalc::BuildSampleWin()
{
	...
	case WT_TRIANGULAR:
		...
		j = 0;
		dk = UIntOS2Double(k);
		dsampleCount = UIntOS2Double(sampleCount);
		while (j < k)
		{
			this->sampleWindow[j]= (UIntOS2Double(j) + 0.5) / dk;
			j++;
		}
		while (j < sampleCount)
		{
			this->sampleWindow[j] = (dsampleCount - UIntOS2Double(j) + 0.5) / dk;
			j++;
		}
		break;
	...
	case WT_HAMMING:
		...
		while (j < sampleCount)
		{
			this->sampleWindow[j] = (a0 - a1 * Math_Cos(UIntOS2Double(j) * pi2 / dk));
			j++;
		}
		break;
...
Math::FFTCalc::FFTCalc(UIntOS sampleCount, WindowType wtype)
{
...
Bool Math::FFTCalc::ForwardBits(UnsafeArray<UInt8> samples, UnsafeArray<Double> freq, SampleType sampleType, UIntOS nChannels, Double magnify)
{
