...
IO::ParsedObject *Net::URL::OpenObject(const UTF8Char *url, const UTF8Char *userAgent, Net::SocketFactory *sockf)
{
	IO::ParsedObject *pobj;
	UTF8Char sbuff[512];
	if (Text::StrStartsWithICase(url, (const UTF8Char*)"HTTP://"))
	{
		Net::HTTPClient *cli = Net::HTTPClient::CreateClient(sockf, userAgent, true, false);
		cli->Connect(url, "GET", 0, 0, true);
		return cli;
	}
	else if (Text::StrStartsWithICase(url, (const UTF8Char*)"HTTPS://"))
	{
		Net::HTTPClient *cli = Net::HTTPClient::CreateClient(sockf, userAgent, true, true);
		cli->Connect(url, "GET", 0, 0, true);
		return cli;
	}
	else if (Text::StrStartsWithICase(url, (const UTF8Char*)"FILE:///"))
	{
		...
		NEW_CLASS(pobj, IO::FileStream(sbuff, IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
		return pobj;
	}
