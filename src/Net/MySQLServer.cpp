...
void __stdcall Net::MySQLServer::OnClientEvent(Net::TCPClient *cli, void *userObj, void *cliData, Net::TCPClientMgr::TCPEventType evtType)
{
	...
	if (evtType == Net::TCPClientMgr::TCP_EVENT_DISCONNECT)
	{
		...
		#endif
		Data::ArrayList<const UTF8Char*> *attrList = data->attrMap->GetValues();
		i = attrList->GetCount();
		while (i-- > 0)
		{
	...
	if (data->mode == 0)
	{
		if (data->buffSize >= 4)
		{
			...
			if (packetSize + 4 <= data->buffSize)
			{
				...
				{
					UTF8Char *sptr;
					sptr = Text::StrConcat(&sbuff[7], (const UTF8Char*)"#28000Access denied for user '");
...
*/
								...
								while (j < k)
								{
									r->GetColDef(j, &col);
									sbuff[3] = seqId++;
									sptr = Net::MySQLUtil::AppendLenencStr(&sbuff[4], (const UTF8Char*)"def"); //catalog
...
*/
									...
									while (j < k)
									{
										...
										else
										{
											sb.ClearStr();
											r->GetStr(j, &sb);
											sptr = Net::MySQLUtil::AppendLenencStr(sptr, sb.ToString());
										}
...
void __stdcall Net::MySQLServer::OnClientConn(Socket *s, void *userObj)
{
	...
	buff[4] = 10;
	bptr = Text::StrConcat(Text::StrConcat(&buff[5], (const UTF8Char*)MYSQLVERSION), me->dbms->GetVersion()) + 1;
