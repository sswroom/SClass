...
void __stdcall Net::MySQLServer::OnClientData(NN<Net::TCPClient> cli, AnyType userObj, AnyType cliData, const Data::ByteArrayR &buff)
{
	...
	if (data->mode == 0)
	{
		if (data->buffSize >= 4)
		{
			...
			packetSize = ReadLUInt24(&data->buff[0]);
			...
			if (packetSize + 4 <= data->buffSize)
			{
				...
				data->clientCap = ReadLUInt32(&data->buff[4]);
				if (data->clientCap & Net::MySQLUtil::CLIENT_PROTOCOL_41)
				{
					if (data->param.clientMaxPacketSize != ReadLUInt32(&data->buff[8]))
					{
						if (data->param.clientMaxPacketSize < ReadLUInt32(&data->buff[8]))
						{
							data->param.clientMaxPacketSize = ReadLUInt32(&data->buff[8]);
					...
					len = (UIntOS)(Text::StrConcat(data->userName, &data->buff[36]) - data->userName);
					...
					{
						len = (UIntOS)(Text::StrConcat(data->database, bptr) - data->database);
						sb.AppendC(UTF8STRC("\r\nDatabase = "));
					...
					if (data->clientCap & Net::MySQLUtil::CLIENT_CONNECT_ATTRS)
					{
						...
						while (bptr < bptrEnd)
						{
							...
							if (bptr + iVal > bptrEnd)
								break;
							sb.AppendC(bptr, (UIntOS)iVal);
							data->attrMap->Put(sbuff, Text::StrCopyNewC(bptr, (UIntOS)iVal).Ptr());
							bptr += iVal;
						}
					}
				}
				else
				{
					...
					data->param.clientMaxPacketSize = ReadLUInt24(&data->buff[6]);
					sb.AppendC(UTF8STRC("Handshake Response 320"));
					sb.AppendC(UTF8STRC("\r\nCapability Flags = 0x"));
					sb.AppendHex16((UInt16)data->clientCap);
					sb.AppendC(UTF8STRC("\r\nMax Packet Size = "));
					sb.AppendU32(data->param.clientMaxPacketSize);
					len = (UIntOS)(Text::StrConcat(data->userName, &data->buff[9]) - data->userName);
					data->userNameLen = len;
					...
					{
						authResp = bptr;
						authLen = packetSize - (UIntOS)(bptr - data->buff) + 4;
					}
					sb.AppendC(UTF8STRC("\r\nAuth Response = "));
					sb.AppendHexBuff(authResp, authLen, ' ', Text::LineBreakType::None);
				}
				...
				else
				{
					...
					*sptr++ = '\'';
					WriteLInt24(sbuff, (sptr - sbuff - 4));
					sbuff[3] = 2;
					sbuff[4] = 0xff;
					WriteLInt16(&sbuff[5], 0x6A2);
					cli->Write(Data::ByteArrayR(sbuff, (UIntOS)(sptr - sbuff)));
					data->mode = -1;
					#if defined(VERBOSE)
	...
	else if (data->mode == 1)
	{
		...
		{
			UInt32 packetSize = ReadLUInt32(&data->buff[i]);
				...
				case 3:
					{
						...
						if (me->dbms->ExecuteReader(data->connId, sql->v, sql->leng).SetTo(r))
						{
							if (r->GetRowChanged() != -1)
							{
								...
								sbuff[3] = 1;
								sbuff[4] = 0;
								sptr = Net::MySQLUtil::AppendLenencInt(&sbuff[5], (UIntOS)r->GetRowChanged());
								sptr = Net::MySQLUtil::AppendLenencInt(sptr, 0); //last insert-id
								WriteLInt16(&sptr[0], 0x4002);
								WriteLInt16(&sptr[2], 0);
								sptr += 4;
								WriteLInt24(&sbuff[0], sptr - sbuff - 4);
								cli->Write(Data::ByteArrayR(sbuff, (UIntOS)(sptr - sbuff)));
								#if defined(VERBOSE)
...
*/
								...
								while (j < k)
								{
									...
									WriteLInt16(&sptr[0], data->clientCS);
									WriteLUInt32(&sptr[2], (UInt32)col.GetColSize());
...
*/
								WriteLInt24(&sbuff[0], 5);
								sbuff[3] = seqId++;
								sbuff[4] = 0xfe;
								WriteLInt16(&sbuff[5], 0);
								WriteLInt16(&sbuff[7], 2);
...
void __stdcall Net::MySQLServer::OnClientConn(NN<Socket> s, AnyType userObj)
{
	...
	WriteLInt32(&bptr[0], data->connId);
	MemCopyNO(&bptr[4], data->authPluginData, 8);
	bptr += 12;
	bptr[0] = 0; //filter_1
	WriteLInt16(&bptr[1], (data->capability & 0xffff));
	bptr[3] = 45; //character set = 45 (utf8mb4)
	WriteLInt16(&bptr[4], 2); //status flags = AUTOCOMMIT
	WriteLInt16(&bptr[6], (data->capability >> 16));
	bptr[8] = 21;
	MemClear(&bptr[9], 10);
	bptr += 19;
	...
	WriteLInt32(buff, (Int32)(bptr - buff - 4));
	cli->Write(Data::ByteArrayR(buff, (UIntOS)(bptr - buff)));
}
