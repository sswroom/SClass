...
UIntOS Net::HTTPMyClient::ReadRAWInternal(Data::ByteArray buff)
{
	...
	if (buff.GetSize() > (this->contLeng - this->contRead))
	{
		...
		buff = buff.SubArray(0, (UIntOS)(this->contLeng - this->contRead));
	}
#ifdef SHOWDEBUG
	printf("Read size = %d\r\n", (Int32)buff.GetSize());
#endif
	...
	if (this->contEnc == 1)
	{
		...
		if (chunkSizeLeft > 0)
		{
			...
			if (this->chunkSizeLeft <= 2 && this->cli.SetTo(cli))
			{
				...
				MemCopyO(this->dataBuff.Ptr(), &this->dataBuff[this->chunkSizeLeft], this->buffSize - this->chunkSizeLeft);
			...
			else if (buff.GetSize() >= this->buffSize)
			{
				...
				if (this->buffSize > 0)
				{
					MemCopyO(this->dataBuff.Ptr(), &this->dataBuff[sizeOut], this->buffSize);
				}
		...
		{
			MemCopyO(this->dataBuff.Ptr(), &this->dataBuff[2], this->buffSize - 2);
			buffSize -= 2;
		}
		...
		if (j == 0 && i == 1 && this->dataBuff[0] == '0')
		{
			...
			if (this->buffSize)
			{
				MemCopyO(this->dataBuff.Ptr(), &this->dataBuff[i], this->buffSize);
			}
			this->contLeng = 0;
#ifdef SHOWDEBUG
		...
		if (this->buffSize > i)
		{
			if (buff.GetSize() >= this->buffSize - i)
			{
				...
				if (this->buffSize > 0)
				{
					MemCopyO(this->dataBuff.Ptr(), &this->dataBuff[sizeOut + i], this->buffSize);
				}
				if (this->chunkSizeLeft < 2)
				{
					sizeOut -= (2 - this->chunkSizeLeft);
...
/*			if (this->buffSize == 0)
		...
		if (this->buffSize >= buff.GetSize())
		{
			...
			if (this->buffSize > buff.GetSize())
			{
				MemCopyO(this->dataBuff.Ptr(), &this->dataBuff[buff.GetSize()], this->buffSize - buff.GetSize());
			}
...
UIntOS Net::HTTPMyClient::ReadRAW(const Data::ByteArray &buff)
{
	this->EndRequest(0, 0);
...
UIntOS Net::HTTPMyClient::Read(const Data::ByteArray &buff)
{
	this->EndRequest(0, 0);
	if (this->respStatus == 0)
		return 0;
...
UIntOS Net::HTTPMyClient::Write(Data::ByteArrayR buff)
{
...
Bool Net::HTTPMyClient::Connect(Text::CStringNN url, Net::WebUtil::RequestMethod method, OptOut<Double> timeDNS, OptOut<Double> timeConn, Bool defHeaders)
{
	...
	{
		this->cliHost = cliHost = Text::String::New(urltmp, urltmpLen);
		...
		if (addr.addrType != Net::AddrType::Unknown)
		{
			...
			if (secure && this->ssl.SetTo(ssl))
			{
				...
				this->cli = ssl->ClientConnect(cliHost->ToCString(), port, err, this->timeout);
#if defined(SHOWDEBUG)
				if (this->cli.IsNull())
				{
	...
	else if (Text::StrEqualsC(cliHost->v, cliHost->leng, urltmp, urltmpLen) && this->cli.SetTo(cli))
	{
		if (this->buffSize > 0)
		{
#ifdef SHOWDEBUG
		...
		{
			UIntOS size = BUFFSIZE;
			if (size > (this->contLeng - this->contRead))
			{
				size = (UIntOS)(this->contLeng - this->contRead);
			}
	...
	this->reqMstm.Write(Data::ByteArrayR(dataBuff, (UIntOS)(cptr - (UTF8Char*)dataBuff.Ptr())));
...
#endif
			this->reqMstm.Write(Data::ByteArrayR(buff, (UIntOS)(sptr - (UTF8Char*)buff)));
		}
...
void Net::HTTPMyClient::EndRequest(OptOut<Double> timeReq, OptOut<Double> timeResp)
{
	...
	else
	{
		...
		{
			UIntOS recvSize = cli->Read(Data::ByteArray(&this->dataBuff[this->buffSize], BUFFSIZE - 1 - this->buffSize));
#ifdef SHOWDEBUG
		...
		if (Text::StrStartsWithC(this->dataBuff, this->buffSize, UTF8STRC("HTTP/")))
		{
			...
			while (header)
			{
				while ((i = Text::StrIndexOfCharC(ptr, (UIntOS)(ptrEnd - ptr), '\n')) != INVALID_INDEX && i > 0)
				{
					if (i == 1 && ptr[0] == '\r')
					{
						ptr++;
					...
					{
						s->leng = (UIntOS)(Text::StrTrimC(&s->v[18], s->leng - 18) - s->v);
					...
					else if (s->StartsWithICase(UTF8STRC("Content-")))
					{
						...
						{
							s->leng = (UIntOS)(Text::StrTrimC(&s->v[15], s->leng - 15) - s->v);
						...
						{
							UIntOS i = s->IndexOf(UTF8STRC("filename="), 20);
							if (i >= 0)
							{
								if (s->v[i + 9] == '"')
								{
									UIntOS j = s->IndexOf('"', i + 10);
									if (j > 0)
									{
				...
				{
					ptr = &ptr[1];
					this->hdrLen += (UIntOS)(ptr - this->dataBuff);
					this->buffSize -= (UIntOS)(ptr - this->dataBuff);
				...
				this->hdrLen += (UIntOS)(ptr - this->dataBuff);
				this->buffSize = (UIntOS)(ptrEnd - ptr);
				MemCopyO(this->dataBuff.Ptr(), ptr.Ptr(), this->buffSize);
				i = cli->Read(Data::ByteArray(&this->dataBuff[this->buffSize], BUFFSIZE - 1 - this->buffSize));
				if (i <= 0)
				{
