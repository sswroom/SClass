...
Net::WebServer::IWebSession *Net::WebServer::MemoryWebSessionManager::CreateSession(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp)
{
	...
	resp->AddSetCookie(CSTR("WebSessId"), CSTRP(sbuff, sptr), this->path->ToCString(), true, req->IsSecure(), Net::WebServer::SameSiteType::None, 0);
	UOSInt i;
...
void Net::WebServer::MemoryWebSessionManager::DeleteSession(Net::WebServer::IWebRequest *req, Net::WebServer::IWebResponse *resp)
{
	...
	if (sessId != 0)
	{
		...
		resp->AddSetCookie(CSTR("WebSessId"), CSTR(""), this->path->ToCString(), true, req->IsSecure(), Net::WebServer::SameSiteType::None, Data::Timestamp::UtcNow().AddMonth(-12));
	}
}
