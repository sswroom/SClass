			...
			if (pitem)
			{
				...
				if (pot == IO::PackageFile::POT_STREAMDATA)
				{
					...
					if (stmData)
					{
						...
						mime = Net::MIME::GetMIMEFromExt(sbuff, (UOSInt)(sptr3 - sbuff));
						...
						Net::WebServer::HTTPServerUtil::SendContent(req, resp, mime.v, mime.leng, dataLen, dataBuff);
						MemFree(dataBuff);
						return true;
					}
				}
					...
					if (innerPF)
					{
						...
						if (pitem2 && innerPF->GetPItemType(pitem2) == IO::PackageFile::POT_STREAMDATA)
						{
							...
							if (stmData)
							{
								...
								stmData->GetRealData(0, dataLen, dataBuff);
								DEL_CLASS(stmData);
								mime = Net::MIME::GetMIMEFromExt(UTF8STRC("html"));
	...
	if (cache != 0)
	{
		...
		if (i != INVALID_INDEX)
		{
			...
			else
			{
				...
				mime = Net::MIME::GetMIMEFromExt(sbc.ToString() + i + 1, sbc.GetLength() - i - 1);
			}
		}
		else
		{
			...
			else
			{
				...
				mime = Net::MIME::GetMIMEFromExt(subReq + i + 1, subReqLen - i - 1);
			}
		}
		resp->AddContentType(mime.v, mime.leng);
		Net::WebServer::HTTPServerUtil::SendContent(req, resp, mime.v, mime.leng, cache->buffSize, cache->buff);
