...
void __stdcall Net::SyslogServer::OnUDPPacket(NN<const Net::SocketUtil::AddressInfo> addr, UInt16 port, Data::ByteArrayR data, AnyType userData)
{
	...
	if (data[0] == '<')
	{
		Optional<Net::SyslogServer::IPStatus> status = me->GetIPStatus(addr);
		NN<IPStatus> nnstatus;
		...
		if (status.SetTo(nnstatus))
		{
			sb.ClearStr();
			sb.AppendC(&data[0], data.GetSize());
			nnstatus->log->LogMessage(sb.ToCString(), IO::LogHandler::LogLevel::Command);

			if (me->logHdlr)
			{
				me->logHdlr(me->logHdlrObj, nnstatus->ip, sb.ToCString());
			}
		}
	}
}

Optional<Net::SyslogServer::IPStatus> Net::SyslogServer::GetIPStatus(NN<const Net::SocketUtil::AddressInfo> addr)
{
	UTF8Char sbuff[512];
	UnsafeArray<UTF8Char> sptr;
	...
	if (addr->addrType == Net::AddrType::IPv4)
	{
		...
		NN<Net::SyslogServer::IPStatus> status;
...
Net::SyslogServer::SyslogServer(NN<Net::SocketFactory> sockf, UInt16 port, Text::CStringNN logPath, NN<IO::LogTool> svrLog, Bool redirLog)
{
	...
	this->logHdlr = 0;
	this->logHdlrObj = 0;
	NEW_CLASSNN(this->svr, Net::UDPServer(this->sockf, nullptr, port, nullptr, OnUDPPacket, this, log, CSTR("UDP: "), 2, false));
}
