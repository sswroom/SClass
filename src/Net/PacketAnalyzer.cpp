...
Bool Net::PacketAnalyzer::PacketDataGetName(UInt32 linkType, UnsafeArray<const UInt8> packet, UIntOS packetSize, NN<Text::StringBuilderUTF8> sb)
{
	switch (linkType)
	{
	case 0:
...
void Net::PacketAnalyzer::PacketDataGetDetail(UInt32 linkType, UnsafeArray<const UInt8> packet, UIntOS packetSize, NN<Text::StringBuilderUTF8> sb)
{
	IO::FileAnalyse::SBFrameDetail frame(sb);
	PacketDataGetDetail(linkType, packet, packetSize, 0, frame);
}

void Net::PacketAnalyzer::PacketDataGetDetail(UInt32 linkType, UnsafeArray<const UInt8> packet, UIntOS packetSize, UInt32 frameOfst, NN<IO::FileAnalyse::FrameDetailHandler> frame)
{
	switch (linkType)
	{
	case 0:
