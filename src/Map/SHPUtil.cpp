...
Math::Vector2D *Map::SHPUtil::ParseShpRecord(Int32 srid, const UInt8 *buff, OSInt buffSize)
{
	...
	case 3: //Polyline
		if (buffSize >= 44)
		{
			...
			*/
			nParts = ReadInt32(&buff[36]);
			nPoints = ReadInt32(&buff[40]);
			if (buffSize >= 44 + nParts * 4 + nPoints * 16)
			{
				UOSInt tmpV;
				UInt32 *parts;
				Double *points;
				NEW_CLASS(pl, Math::Polyline(srid, nParts, nPoints));
				parts = pl->GetPartList(&tmpV);
				buff += 44;
				while (tmpV-- > 0)
				{
					*parts++ = ReadInt32(buff);
					buff += 4;
				}
		...
		if (buffSize >= 44)
		{
			...
			*/
			...
			if (buffSize >= 44 + nParts * 4 + nPoints * 16)
			{
				...
				UInt32 *parts;
				Double *points;
				NEW_CLASS(pl, Math::Polygon(srid, nParts, nPoints));
				parts = pl->GetPartList(&tmpV);
				buff += 44;
				while (tmpV-- > 0)
				{
	...
	case 10: //PolylineZ
		if (buffSize >= 44)
		{
			...
			*/
			nParts = ReadInt32(&buff[36]);
			nPoints = ReadInt32(&buff[40]);
			if (buffSize >= 44 + nParts * 4 + nPoints * 24)
			{
				UOSInt tmpV;
				UInt32 *parts;
				Double *points;
				NEW_CLASS(pl, Math::Polyline3D(srid, nParts, nPoints));
				parts = pl->GetPartList(&tmpV);
				buff += 44;
				while (tmpV-- > 0)
				{
	...
	case 13: //PolylineZM
		if (buffSize >= 44)
		{
			...
			*/
			nParts = ReadInt32(&buff[36]);
			nPoints = ReadInt32(&buff[40]);
			if (buffSize >= 44 + nParts * 4 + nPoints * 32)
			{
				UOSInt tmpV;
				UInt32 *parts;
				Double *points;
