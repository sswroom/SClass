...
Map::CustomTileMap::CustomTileMap(Text::CStringNN url, Text::CString cacheDir, UOSInt minLevel, UOSInt maxLevel, NN<Net::TCPClientFactory> clif, Optional<Net::SSLEngine> ssl) : Map::MercatorTileMap(cacheDir, minLevel, maxLevel, clif, ssl)
{
...
Bool Map::CustomTileMap::GetTileImageURL(NN<Text::StringBuilderUTF8> sb, UOSInt level, Math::Coord2D<Int32> tileId)
{
	...
	sptr2 = Text::StrInt32(sbuff2, tileId.y);
	sb->ReplaceStr(UTF8STRC("{y}"), sbuff2, (UOSInt)(sptr2 - sbuff2));
	sptr2 = Text::StrUOSInt(sbuff2, level);
	sb->ReplaceStr(UTF8STRC("{z}"), sbuff2, (UOSInt)(sptr2 - sbuff2));
	return true;
}
