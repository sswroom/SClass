...
Bool Map::MapConfig2::IsDoorNum(UnsafeArray<const UTF8Char> txt)
{
	UTF8Char c;
	while ((c = *txt++) != 0)
	{
...
void Map::MapConfig2::DrawChars(NN<Media::DrawImage> img, Text::CStringNN str1, Math::Coord2DDbl scnPos, Double scaleW, Double scaleH, Optional<Data::ArrayListNN<MapFontStyle>> fontStyle, Bool isAlign)
{
	UTF8Char sbuff[256];
	str1.ConcatTo(sbuff);
	Math::Size2DDbl size;
			...
			if (nnb.Set((Media::DrawBrush*)font->other))
			{
				if (isAlign)
				{
					...
					else
					{
						...
						if (scaleH > 0)
						{
							startY = scnPos.y - (tmp = ((szThis.y * UIntOS2Double(lblSize)) * 0.5));
...
void Map::MapConfig2::DrawCharsLA(NN<Media::DrawImage> img, Text::CStringNN str1, UnsafeArray<Math::Coord2DDbl> mapPts, UnsafeArray<Math::Coord2DDbl> scnPts, UIntOS nPoints, UInt32 thisPt, Double scaleN, Double scaleD, Optional<Data::ArrayListNN<MapFontStyle>> fontStyle, OutParam<Math::RectAreaDbl> realBounds)
{
	UTF8Char sbuff[256];
	str1.ConcatTo(sbuff);
...
void Map::MapConfig2::DrawCharsLAo(NN<Media::DrawImage> img, Text::CStringNN str1, UnsafeArray<Double> mapPts, UnsafeArray<Math::Coord2DDbl> scnPts, UIntOS nPoints, UInt32 thisPt, Double scaleN, Double scaleD, Optional<Data::ArrayListNN<MapFontStyle>> fontStyle)
{
	UTF8Char sbuff[256];
	str1.ConcatTo(sbuff);
	UIntOS lblSize = str1.leng;
...
void Map::MapConfig2::DrawCharsL(NN<Media::DrawImage> img, Text::CStringNN str1, UnsafeArray<Math::Coord2DDbl> mapPts, UnsafeArray<Math::Coord2DDbl> scnPts, UIntOS nPoints, UInt32 thisPt, Double scaleN, Double scaleD, Optional<Data::ArrayListNN<MapFontStyle>> fontStyle, OutParam<Math::RectAreaDbl> realBounds)
{
	UTF8Char sbuff[256];
	str1.ConcatTo(sbuff);
	UIntOS lblSize = str1.leng;
...
void Map::MapConfig2::GetCharsSize(NN<Media::DrawImage> img, OutParam<Math::Coord2DDbl> size, Text::CStringNN label, Optional<Data::ArrayListNN<MapFontStyle>> fontStyle, Double scaleW, Double scaleH)
{
	Double xSizeAdd = 0;
	Double ySizeAdd = 0;
...
UInt32 Map::MapConfig2::ToColor(UnsafeArray<const UTF8Char> str)
{
...
void Map::MapConfig2::DrawPoints(NN<Media::DrawImage> img, NN<MapLayerStyle> lyrs, NN<Map::MapView> view, OutParam<Bool> isLayerEmpty, NN<Map::MapScheduler> sch, NN<Media::DrawEngine> eng, Optional<Media::ImageResizer> resizer, UnsafeArray<Math::RectAreaDbl> objBounds, InOutParam<UIntOS> objCnt, UIntOS maxObjCnt)
{
	...
	Math::RectAreaDbl *objPtr = &objBounds[objCnt.Get()];
#endif
...
void Map::MapConfig2::DrawString(NN<Media::DrawImage> img, NN<MapLayerStyle> lyrs, NN<Map::MapView> view, UnsafeArray<Optional<Data::ArrayListNN<MapFontStyle>>> fonts, UnsafeArray<MapLabels2> labels, UInt32 maxLabels, InOutParam<UInt32> labelCnt, OutParam<Bool> isLayerEmpty)
{
...
UInt32 Map::MapConfig2::NewLabel(UnsafeArray<MapLabels2> labels, UInt32 maxLabel, InOutParam<UInt32> labelCnt, Int32 priority)
{
	Int32 minPriority;
	UInt32 i;
	UInt32 j;
...
Bool Map::MapConfig2::AddLabel(UnsafeArray<MapLabels2> labels, UInt32 maxLabel, InOutParam<UInt32> labelCnt, Text::CStringNN labelt, UIntOS nPoint, UnsafeArray<Math::Coord2DDbl> points, Int32 priority, Int32 recType, UInt32 fontStyle, UInt32 flags, NN<Map::MapView> view, Double xOfst, Double yOfst)
{
	Double size;
	Double visibleSize;
...
//						wprintf(L"Shape: %s inverse merged (%d + %d)\n", labelt, labels[i].nPoints, nPoint);
						UIntOS newSize = labels[i].nPoints + nPoint - 1;
						UnsafeArray<Math::Coord2DDbl> newArr = MemAllocAArr(Math::Coord2DDbl, newSize << 1);
						UIntOS k;
						Int32 l;
						l = 0;
						k = labels[i].nPoints;
...
//			wprintf(L"Update Label: %s, nPoint: %d > %d\n", labelt, labels[i].nPoints, nPoint);
	...
	else if (recType == 5) //polygon
	{
		...
		if (found == 0)
		{
			...
			UnsafeArray<Math::Coord2DDbl> tmpPts;
			tmpPts = MemAllocAArr(Math::Coord2DDbl, nPoint << 1);
			outPtCnt = Math::GeometryTool::BoundPolygonY(points, nPoint, tmpPts, top, bottom, {0, 0});
			outPts = MemAllocAArr(Math::Coord2DDbl, nPoint << 1);
...
void Map::MapConfig2::SwapLabel(UnsafeArray<MapLabels2> mapLabels, UInt32 index, UInt32 index2)
{
	MapLabels2 l;
...
Bool Map::MapConfig2::LabelOverlapped(UnsafeArray<Math::RectAreaDbl> points, UIntOS nPoints, Math::RectAreaDbl rect)
{
	while (nPoints--)
	{
...
void Map::MapConfig2::DrawLabels(NN<Media::DrawImage> img, UnsafeArray<MapLabels2> labels, UInt32 maxLabel, UInt32 labelCnt, NN<Map::MapView> view, UnsafeArray<Optional<Data::ArrayListNN<MapFontStyle>>> fonts, NN<Media::DrawEngine> drawEng, UnsafeArray<Math::RectAreaDbl> objBounds, InOutParam<UIntOS> objCnt)
{
	UInt32 i;
	UIntOS j;
...
//					wprintf(L"Error in handling label: %s\n", lastLbl);
				...
				if (IntOS2Double(diff.x) < szThis.x && IntOS2Double(diff.y) < szThis.y)
				{
					rect.min = scnPt - (szThis * 0.5);
					rect.max = rect.min + szThis;
					...
					while (j)
					{
						...
						else
						{
							...
							if (diff.x > diff.y)
							{
								...
								{
									scnPt.x += IntOS2Double(scnDiff + 1);
									if (scnPt.x >= IntOS2Double(maxX))
									{
										scnDiff = scnDiff * 0.5;
										if (scnDiff < 30)
											break;
							...
							else
							{
								...
								{
									scnPt.y += IntOS2Double(scnDiff + 1);
									if (scnPt.y >= IntOS2Double(maxY))
									{
										scnDiff = scnDiff * 0.5;
										if (scnDiff < 30)
											break;
...
Map::MapConfig2::MapConfig2(Text::CStringNN fileName, NN<Media::DrawEngine> eng, NN<Data::ArrayListNN<Map::MapDrawLayer>> layerList, NN<Parser::ParserList> parserList, Text::CString forceBase, NN<IO::Writer> errWriter, Int32 maxScale, Int32 minScale)
{
	...
	{
		UnsafeArray<Optional<Data::ArrayListNN<MapLineStyle>>> lines;
		UnsafeArray<Optional<Data::ArrayListNN<MapFontStyle>>> fonts;
		NN<Data::ArrayListNN<MapLineStyle>> thisLines;
		NN<Data::ArrayListNN<MapFontStyle>> thisFonts;
		NN<Data::ArrayListNN<MapLayerStyle>> drawList;
		IO::StreamReader rdr(fstm);
		Text::CStringNN nnforceBase;
		if (forceBase.SetTo(nnforceBase) && nnforceBase.leng > 0)
		{
			baseDir = nnforceBase.ConcatTo(layerName);
		}
...
/*						if (osSize != 1)
			...
			case 14:
				if (strCnt > 1)
				{
					NN<Map::MapConfig2::MapArea> area;
					area = MemAllocNN(Map::MapConfig2::MapArea);
					area->mcc = Text::StrToInt32(strs[1].v);
					if (strCnt > 2)
					{
						layerNameEnd = strs[2].ConcatTo(baseDir);
						NEW_CLASSOPT(area->data, Map::MapLayerData(CSTRP(layerName, layerNameEnd)));
					}
					else
					{
						area->data = nullptr;
		...
		if (this->inited && this->drawList.SetTo(drawList))
		{
			i = drawList->GetCount();
			while (i-- > 0)
			{
...
Map::MapConfig2::~MapConfig2()
{
	...
	while (i-- > 0)
	{
		NN<Map::MapConfig2::MapArea> area = this->areaList.GetItemNoCheck(i);
		area->data.Delete();
		MemFreeNN(area);
	}
}
...
Optional<Media::DrawPen> Map::MapConfig2::CreatePen(NN<Media::DrawImage> img, UInt32 lineStyle, UIntOS lineLayer)
{
	UnsafeArray<Optional<Data::ArrayListNN<Map::MapLineStyle>>> lines;
	UnsafeArray<UTF8Char> styles;
	NN<Data::ArrayListNN<Map::MapLineStyle>> thisLines;
...
UnsafeArrayOpt<UTF8Char> Map::MapConfig2::DrawMap(NN<Media::DrawImage> img, NN<Map::MapView> view, OutParam<Bool> isLayerEmpty, NN<Map::MapScheduler> mapSch, Optional<Media::ImageResizer> resizer, UnsafeArrayOpt<UTF8Char> slowLayer, OptOut<Double> slowTime)
{
	UnsafeArray<Optional<Data::ArrayListNN<Map::MapFontStyle>>> fonts;
	NN<Data::ArrayListNN<Map::MapLayerStyle>> drawList;
	...
	UnsafeArrayOpt<UTF8Char> retPtr = nullptr;
	UnsafeArrayOpt<const UTF8Char> maxName = nullptr;
	Double thisSpd = 0;
	Double maxSpd = 0;
	UInt32 index;
	UInt32 index2;
	...
	NN<Data::ArrayListNN<MapFontStyle>> myArrFont;
	NN<Map::MapDrawLayer> lyr;
	...
	Manage::HiResClock clk;
	UnsafeArray<Map::MapLabels2> labels = MemAllocArr(Map::MapLabels2, maxLabel);
	...
	while (i-- > 0)
	{
		if (fonts[i].SetTo(fontArr))
		{
			...
			j = 0;
			k = fontArr->GetCount();
			NEW_CLASSNN(myArrFont, Data::ArrayListNN<MapFontStyle>());
			myArrs[i] = myArrFont;
			while (j < k)
			{
	...
	while (index < layerCnt)
	{
		...
		if (thisScale > lyrs->minScale && thisScale <= lyrs->maxScale)
		{
			...
			{
				maxSpd = thisSpd;
				maxName = UnsafeArray<const UTF8Char>(lyrs->lyr->GetName()->v);
			}
		}
	}
	...
	//printf("Time used: %d\n", (Int32)(t * 1000));
	UnsafeArray<const UTF8Char> nnmaxName;
	UnsafeArray<UTF8Char> nnslowLayer;
	if (maxName.SetTo(nnmaxName) && maxSpd && slowLayer.SetTo(nnslowLayer) && slowTime.IsNotNull())
	{
		slowTime.SetNoCheck(maxSpd);
		retPtr = Text::StrConcat(nnslowLayer, nnmaxName);
	}
...
Bool Map::MapConfig2::SupportMCC(Int32 mcc)
{
	if (mcc == 0)
		return true;
	UIntOS i = this->areaList.GetCount();
	while (i-- > 0)
	{
		if (this->areaList.GetItemNoCheck(i)->mcc == mcc)
			return true;
	}
	return false;
}
...
Int32 Map::MapConfig2::QueryMCC(Math::Coord2DDbl pos)
{
	...
	{
		NN<Map::MapConfig2::MapArea> area;
		NN<Map::MapLayerData> areaData;
		area = this->areaList.GetItemNoCheck(i);
		if (area->data.SetTo(areaData))
		{
			if (areaData->GetPGLabel(sb, pos, 0, 0))
			{
