...
Optional<Math::Geometry::Vector2D> Map::OruxDBLayer::GetNewVectorById(NN<GetObjectSess> session, Int64 id)
{
	...
	if (imgList.SetTo(nnimgList))
	{
		...
		NEW_CLASSNN(vimg, Math::Geometry::VectorImage(4326, shImg, Math::Coord2DDbl(x1, y2), Math::Coord2DDbl(x1 + (lyr->mapMax.x - lyr->mapMin.x) / lyr->max.x, y1), Math::Coord2DDbl((lyr->mapMax.x - lyr->mapMin.x) / lyr->max.x, (y1 - y2)), false, Text::CString(nullptr), 0, 0));
		shImg.Delete();
		return vimg;
	}
	else
