...
Math::Vector2D *Map::OruxDBLayer::GetVectorById(void *session, Int64 id)
{
	...
	if (lyr == 0)
		return 0;
	DB::SQLBuilder sql(DB::DBUtil::SVR_TYPE_SQLITE, 0);
	Int32 x;
	Int32 y;
...
DB::DBReader *Map::OruxDBLayer::GetTableData(const UTF8Char *name, UOSInt maxCnt, void *ordering, void *condition)
{
	return this->db->GetTableData(name, maxCnt, ordering, condition);
}
...
void Map::OruxDBLayer::GetErrorMsg(Text::StringBuilderUTF *str)
{
