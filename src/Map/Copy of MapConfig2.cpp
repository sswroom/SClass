...
void Map::MapConfig2::DrawChars(Media::DrawImage *img, const WChar *str1, Int32 scnPosX, Int32 scnPosY, Int32 scaleW, Int32 scaleH, Data::ArrayList<MapFontStyle*> *fontStyle, Bool isAlign)
{
	WChar wbuff[256];
...
//				Media::DrawPen *p = 0;
			...
			{
				img->DrawStringWB(scnPosX - (size[0]  >> 1), scnPosY - (size[1] >> 1), str1, font->font, (Media::DrawBrush*)font->other, font->thick);
				...
				while (cnt--)
				{
					...
					if (type)
					{
						...
						else
						{
							...
							l[0] = lbl[0];
							l[1] = 0;
							img->DrawStringWB(startX + currX - (szThis[0] >> 1), startY + currY, l, font->font, (Media::DrawBrush*)font->other, font->thick);
...
/*							WChar l[2];
					...
					else
					{
						...
						else
						{
							...
							l[0] = lbl[0];
							l[1] = 0;
							img->DrawStringWB(startX + currX, startY + currY, l, font->font, (Media::DrawBrush*)font->other, font->thick);
...
/*							WChar l[2];
			...
			//	img->DrawStringRotW(scnPosX, scnPosY, str1, font->font, (Media::DrawBrush*)font->other, (Int32)(degD * 180 / PI));
				...
				{
					img->DrawStringRotWB(scnPosX, scnPosY, str1, font->font, (Media::DrawBrush*)font->other, (Int32)(degD * 180 / PI), font->thick);
/*					Int32 j = font->thick;
					Int32 k;
					Int32 px = scnPosX;
					Int32 py = scnPosY;
...
void Map::MapConfig2::GetCharsSize(Media::DrawImage *img, Int32 *size, const WChar *label, Data::ArrayList<MapFontStyle*> *fontStyle, Int32 scaleW, Int32 scaleH)
{
	Int32 xSizeAdd = 0;
	Int32 ySizeAdd = 0;
	...
	while (i < fntCount)
	{
		...
		else if (font->fontType == 4)
		{
			if (maxSize < font->fontSize + (font->thick << 1))
			{
				...
				maxIndex = i;
				xSizeAdd = font->thick << 1;
				ySizeAdd = font->thick << 1;
			}
		}
...
Int32 Map::MapConfig2::ToColor(WChar *str)
{
...
void Map::MapConfig2::DrawPoints(Media::DrawImage *img, MapLayerStyle *lyrs, Map::MapView *view, Bool *isLayerEmpty, Map::MapScheduler *sch, Int32 osSize, Media::DrawEngine *eng, Media::IImgResizer *resizer, Int32 *objBounds, Int32 *objCnt, Int32 maxObjCnt)
{
	...
	if (osSize > 1)
	{
		...
		Media::GDIEngine *geng = (Media::GDIEngine *)eng;
		Media::GDIImage *gimg = (Media::GDIImage*)geng->CreateImage32(imgW * osSize, imgH * osSize);
		Media::GDIImage *gimg2 = (Media::GDIImage*)lyrs->img;
		resizer->Resize((UInt8*)gimg2->bmpBits, imgW << 2, imgW, imgH, (UInt8*)gimg->bmpBits, imgW * osSize << 2, imgW * osSize, imgH * osSize);
		dimg = gimg;
		imgW = (imgW * osSize) >> 1;
		imgH = (imgH * osSize) >> 1;
#ifndef NOSCH
		sch->SetDrawType(lyrs->lyr, Map::MapScheduler::MSDT_POINTS, 0, 0, dimg, isLayerEmpty);
#endif
	}
	else
	{
	...
	while (i-- > 0)
	{
		...
		while (j-- > 0)
		{
			view->MapToScnXY(&dobj->points[j << 1], pts, 1, 0, 0);
...
void Map::MapConfig2::DrawString(Media::DrawImage *img, MapLayerStyle *lyrs, Map::MapView *view, Data::ArrayList<MapFontStyle*> **fonts, MapLabels2 *labels, Int32 maxLabels, Int32 *labelCnt, Bool *isLayerEmpty)
{
	void *arr;
	Data::ArrayListInt *arri;
	OSInt i;
...
//			wptr = (WChar*)arr->GetItem(i);
			if (AddLabel(labels, maxLabels, labelCnt, wptr, maxSize, &dobj->points[maxPos << 1], lyrs->priority, lyrs->lyr->GetLayerType(), lyrs->style, lyrs->bkColor, view, imgWidth * view->GetOSSize(), imgHeight * view->GetOSSize()))
			{
				lyrs->lyr->ReleaseObject(session, dobj);
			}
			else
...
Int32 Map::MapConfig2::NewLabel(MapLabels2 *labels, Int32 maxLabel, Int32 *labelCnt, Int32 priority)
{
	Int32 minPriority;
	Int32 i;
	Int32 j;
...
Bool Map::MapConfig2::AddLabel(MapLabels2 *labels, Int32 maxLabel, Int32 *labelCnt, WChar *labelt, Int32 nPoint, Int32 *points, Int32 priority, Int32 recType, Int32 fontStyle, Int32 flags, Map::MapView *view, Int32 xOfst, Int32 yOfst)
{
	Int32 size;
	Int32 visibleSize;
...
//						wprintf(L"Shape: %s merged (%d + %d)\n", labelt, labels[i].nPoints, nPoint);
						...
						MemCopy(newArr, points, nPoint << 3);
						MemCopy(&newArr[nPoint << 1], &labels[i].points[2], (labels[i].nPoints - 1) << 3);
...
//						wprintf(L"Shape: %s merged (%d + %d)\n", labelt, labels[i].nPoints, nPoint);
						...
						MemCopy(newArr, labels[i].points, labels[i].nPoints << 3);
						MemCopy(&newArr[labels[i].nPoints << 1], &points[2], (nPoint - 1) << 3);
...
//						wprintf(L"Shape: %s inverse merged (%d + %d)\n", labelt, labels[i].nPoints, nPoint);
						...
						MemCopy(&newArr[l], points, nPoint << 3);
...
Int32 Map::MapConfig2::LabelOverlapped(Int32 *points, Int32 nPoints, Int32 tlx, Int32 tly, Int32 brx, Int32 bry)
{
	while (nPoints--)
	{
...
void Map::MapConfig2::DrawLabels(Media::DrawImage *img, MapLabels2 *labels, Int32 maxLabel, Int32 *labelCnt, Map::MapView *view, Data::ArrayList<MapFontStyle*> **fonts, Media::DrawEngine *drawEng, Int32 *objBounds, Int32 *objCnt)
{
	Int32 i;
	Int32 j;
	WChar *dest;
	...
	if (*labelCnt)
	{
		...
		//	labels[i].shapeType = 0;
			if (labels[i].shapeType == 1)
			{
				...
				if (j)
				{
					tlx = scnPtX - szThis[0] - 1 - (labels[i].xOfst >> 1);
					brx = tlx + szThis[0];
					tly = scnPtY - (szThis[1] >> 1);
					bry = tly + szThis[1];
				...
				if (j)
				{
					tlx = scnPtX - (szThis[0] >> 1);
					brx = tlx + szThis[0];
					tly = scnPtY - szThis[1] - 1 - (labels[i].yOfst >> 1);
					bry = tly + szThis[1];
...
Map::MapConfig2::MapConfig2(WChar *fileName, Media::DrawEngine *eng, Data::ArrayList<Map::IMapDrawLayer*> *layerList, Parser::ParserList *parserList, WChar *forceBase, IO::Writer *errWriter, Int32 maxScale, Int32 minScale)
{
	...
	Media::Resizer::LanczosResizer8_8 *resizer;
	...
	else
	{
		...
		NEW_CLASS(resizer, Media::Resizer::LanczosResizer8_8(3));
		if (forceBase)
		{
...
Media::DrawPen *Map::MapConfig2::CreatePen(Media::DrawImage *img, Int32 lineStyle, Int32 lineLayer, Int32 osSize)
{
	...
	{
		return img->NewPenARGB(thisLine->color, thisLine->lineWidth * osSize, 0, 0);
	}
...
//		lines[index].pen = map->img->NewPenARGB(lines[index].color, lines[index].width, 0, 0);
	...
	else if (thisLine->lineType == 2)
	{
		...
		Media::DrawPen *pen = img->NewPenARGB(thisLine->color, thisLine->lineWidth * osSize, pattern, i);
		MemFree(pattern);
		return pen;
	}
	return 0;
...
WChar *Map::MapConfig2::DrawMap(Media::DrawImage *img, Map::MapView *view, Bool *isLayerEmpty, Map::MapScheduler *mapSch, Int32 osSize, Media::IImgResizer *resizer, WChar *slowLayer, Double *slowTime)
{
	...
	*isLayerEmpty = true;
	...
	Int32 *objBounds = MemAlloc(Int32, this->nStr * 8);
	Int32 objCnt = 0;
	view->SetOSSize(osSize);
		...
		if (fontArr)
		{
			...
			while (j < k)
			{
				...
				if (fnt->fontType == 0)
				{
					...
					if (fnt->thick > 0)
						s = s | DFS_BOLD;
					fnt2->font = img->NewFontW(fnt->fontName, fnt->fontSize * osSize, s);
				...
				{
					fnt2->font = img->NewFontW(fnt->fontName, fnt->fontSize * osSize, 0);
					fnt2->other = img->NewBrushARGB(fnt->color);
					fnt2->thick = fnt2->thick * osSize;
				}
	...
	while (index < layerCnt)
	{
		...
		if (thisScale > lyrs->minScale && thisScale <= lyrs->maxScale)
		{
...
#endif
						*isLayerEmpty = false;
						mapSch->SetDrawType(lyr, Map::MapScheduler::MSDT_POLYGON, pen = CreatePen(img, lyrs->style, 0, osSize), brush = img->NewBrushARGB(lyrs->bkColor), 0, isLayerEmpty);
						...
						while (i-- > 0)
						{
							...
							if (thisId != lastId)
							{
								...
								mapSch->Draw(dobj);
#else
								view->MapToScnXY(dobj->points, dobj->points, dobj->nPoints, 0, 0);
								drawArr->Add(dobj);
#endif
							}
						}
						...
						{
							index2 = 1;
							while ((pen = CreatePen(img, lyrs->style, index2++, osSize)) != 0)
							{
			...
			else if (lyrs->drawType == 6)
			{
				...
				if (lyr)
				{
					...
					if ((i = arr->GetCount()) > 0)
					{
						...
						*isLayerEmpty = false;
#ifndef NOSCH
						mapSch->SetDrawType(lyr, Map::MapScheduler::MSDT_POLYLINE, pen = CreatePen(img, lyrs->style, 0, osSize), 0, 0, isLayerEmpty);
#endif
			...
			{
				DrawPoints(img, lyrs, view, isLayerEmpty, mapSch, osSize, drawEng, resizer, objBounds, &objCnt, maxLabel);
			}
		}
	}
