...
//			sptr = (WChar*)arr->GetItem(i);
			if (AddLabel(labels, maxLabels, labelCnt, sptr, maxSize, &dobj->points[maxPos << 1], lyrs->priority, lyrs->lyr->GetLayerType(), lyrs->style, lyrs->bkColor, view, imgWidth * view->GetOSSize(), imgHeight * view->GetOSSize()))
			{
				lyrs->lyr->ReleaseObject(session, dobj);
			}
			else
