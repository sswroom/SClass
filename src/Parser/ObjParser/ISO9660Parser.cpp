	...
	if (!err)
	{
		...
		while (sizeLeft > 32)
		{
			...
			if (recBuff[25] & 2)
			{
				...
				else
				{
					...
					*sptr++ = IO::Path::PATH_SEPERATOR;
					...
					pkgFile->AddPack(pf2, CSTRP(&fileNameEnd[1], sptr), Data::Timestamp(dt.ToTicks(), 0), 0, 0);
			...
			else
			{
				...
				pkgFile->AddData(fd, 0, fileSize, CSTRP(fileNameEnd, sptr), Data::Timestamp(dt.ToTicks(), 0), 0, 0);
				fd.Delete();
			}
