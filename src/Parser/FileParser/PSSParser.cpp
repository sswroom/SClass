...
void Parser::FileParser::PSSParser::PrepareSelector(IO::IFileSelector *selector, IO::ParsedObject::ParserType t)
{
...
IO::ParsedObject::ParserType Parser::FileParser::PSSParser::GetParserType()
{
	return IO::ParsedObject::PT_VIDEO_PARSER;
}

IO::ParsedObject *Parser::FileParser::PSSParser::ParseFile(IO::IStreamData *fd, IO::PackageFile *pkgFile, IO::ParsedObject::ParserType targetType)
{
	UInt8 buff[256];
	UTF8Char sbuff[512];
	UTF8Char *sptr;
					...
					if (*(Int32*)&buff[0x17] == 0x64685353)
					{
						if (formats[stmId]->formatId == 0)
						{
							...
							else if (buff[0x14] == 0xa1)
							{
								...
								formats[stmId]->align = formats[stmId]->nChannels * *(Int32*)&buff[0x2b];
			...
			else if ((buff[3] & 0xe0) == 0xc0) //Audio stream
			{
				...
				if (formats[stmId]->formatId == 0)
				{
					...
					else if (buff[j] == 0xff && (buff[j + 1] & 0xfe) == 0xfc)
					{
						...
						formats[stmId]->bitRate = 0;
						NEW_CLASS(stmData[stmId], IO::StmData::BlockStreamData(fd));
						stmData[stmId]->Append(currOfst + j, (Int32)(i - j + 6));
						audDelay[stmId] = (Int32)((pts - initScr) / 90);
...
//								last_dts = dts;
					...
					if (vstm && v1 && !isFrame)
					{
						...
						OSInt currPtr;
						Int32 srchByte;
						OSInt frameSize = i - stmHdrSize - 3;
						...
						while (currPtr < frameSize - 4)
						{
							if (*(Int32*)&frameBuff[currPtr] == srchByte)
							{
								...
								{
									isFrame = true;
									vstm->AddFramePart(currOfst + 9 + stmHdrSize, (Int32)currPtr);
									stmHdrSize += currPtr;
									break;
								}
							}
				...
				if (vstm != 0)
				{
					if (isFrame)
					{
						...
						vstm->AddNewFrame(currOfst + 9 + stmHdrSize, (Int32)(i - stmHdrSize - 3), true, frameTime);
					}
					else
					{
						vstm->AddFramePart(currOfst + 9 + stmHdrSize, (Int32)(i - stmHdrSize - 3));
					}
				}
	...
	while (i < 4)
	{
		if (formats[i]->formatId)
		{
			if (formats[i]->formatId == 1)
			{
				...
				NEW_CLASS(as, Media::LPCMSource(stmData[i], 0, (Int32)stmData[i]->GetDataSize(), formats[i], fd->GetFullName()));
