...
void Parser::FileParser::QTParser::PrepareSelector(IO::IFileSelector *selector, IO::ParsedObject::ParserType t)
{
...
IO::ParsedObject::ParserType Parser::FileParser::QTParser::GetParserType()
{
...
Media::MediaFile *Parser::FileParser::QTParser::ParseMoovAtom(IO::IStreamData *fd, Int64 ofst, Int64 size)
{
	...
	while (i < size)
	{
		if (fd->GetRealData(ofst + i, 8, hdr) != 8)
			return 0;
...
//				timeScale = 10000000;
		...
		{
			fd->GetRealData(ofst + i + 8, 12, buff);
...
//			Int32 descSize;
			if (atomSize > 1032)
			{
				buff = MemAlloc(UInt8, atomSize - 8);
				fd->GetRealData(ofst + i + 8, atomSize - 8, buff);
			}
			else
			{
				fd->GetRealData(ofst + i + 8, atomSize - 8, dataBuff);
				buff = dataBuff;
			}
...
//			Int32 nstco;
			...
			stcBuff = MemAlloc(UInt8, stcAtomSize);
			stcoBuff = MemAlloc(UInt8, stcoAtomSize);
			stszBuff = MemAlloc(UInt8, stszAtomSize);
...
//			Int32 nstco;
			...
			Data::Integer32Map<Int32> keyMap;
			keyMap.Put(0, 1);
...
//			nstco = ReadMInt32(&stcoBuff[12]);
			...
			if (avccOfst != 0)
			{
				...
				{
					Int32 nsps;
					stszBuff = MemAlloc(UInt8, avccAtomSize);
...
*/
				...
				else if (frInfo.fourcc == *(UInt32*)"HEVC")
				{
					...
					fsrc->SetProp(*(Int32*)"HEVC", stszBuff, avccAtomSize - 8);
					MemFree(stszBuff);
