...
IO::ParsedObject *Parser::FileParser::IPACParser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UInt32 recCnt;
	UInt32 startOfst;
	UInt32 currOfst;
	...
	while (i < recCnt)
	{
		...
		pf->AddData(fd, currOfst, currSize, IO::PackFileItem::HeaderType::No, 0, CSTRP(name, sptr), 0, 0, 0, 0);
		currOfst += currSize;
		hdrOfst += 44;
		i++;
	}
