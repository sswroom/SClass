...
IO::ParsedObject *Parser::FileParser::LinkArcParser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UInt64 currOfst;
	UInt8 nameSize;
	UInt8 recBuff[256];
	...
	while (currOfst < fileSize)
	{
		...
		pf->AddData(fd, currOfst + 15 + fnameSize, recSize - fnameSize - 15, IO::PackFileItem::HeaderType::No, 0, {fileName, (UOSInt)fnameSize >> 1}, Data::Timestamp(dt.ToInstant(), 0), 0, 0, 0);
