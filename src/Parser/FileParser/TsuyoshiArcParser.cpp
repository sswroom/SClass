...
IO::ParsedObject *Parser::FileParser::TsuyoshiArcParser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	...
	while (i < recCnt)
	{
		...
		if (recSize == decSize)
		{
			pf->AddData(fd, recOfst, recSize, IO::PackFileItem::HeaderType::No, 0, CSTRP(fileName, sptr), 0, 0, 0, 0);
		}
		else
		{
			...
			pf->AddCompData(fd, recOfst, recSize, IO::PackFileItem::HeaderType::No, 0, &compInfo, CSTRP(fileName, sptr), 0, 0, 0, 0);
		}
