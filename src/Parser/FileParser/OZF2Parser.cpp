...
IO::ParsedObject *Parser::FileParser::OZF2Parser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UInt8 tmpBuff[1036];
	UOSInt i;
	UOSInt j;
	...
	while (i < scaleCnt)
	{
		...
		{
			NEW_CLASS(outImg, Media::StaticImage(Math::Size2D<UOSInt>(thisImgWidth, thisImgHeight), 0, 8, Media::PF_PAL_8, 0, 0, Media::ColorProfile::YUVT_UNKNOWN, Media::AT_NO_ALPHA, Media::YCOFST_C_CENTER_LEFT));
			if (outImg)
			{
