...
IO::ParsedObject *Parser::FileParser::RAR5Parser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	...
	/*		sb->AppendC(UTF8STRC("\r\nCompression version = "));
			...
			{
				pf->AddData(fd, currOfst + headerSize, dataSize, CSTRP(sbuff, sptr), Data::Timestamp(dt.ToInstant(), dt.GetTimeZoneQHR()), 0, 0, 0);
			}
			else
			{
				...
				pf->AddCompData(fd, currOfst + headerSize, dataSize, &cinfo, CSTRP(sbuff, sptr), Data::Timestamp(dt.ToInstant(), dt.GetTimeZoneQHR()), 0, 0, 0);
			}
		}
