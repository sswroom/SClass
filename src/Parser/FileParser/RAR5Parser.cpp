...
IO::ParsedObject *Parser::FileParser::RAR5Parser::ParseFileHdr(IO::StreamData *fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UTF8Char sbuff[512];
	UTF8Char *sptr;
	UInt8 buff[512];
	...
	/*		sb->AppendC(UTF8STRC("\r\nCompression version = "));
			...
			{
				pf->AddData(fd, currOfst + headerSize, dataSize, CSTRP(sbuff, sptr), Data::Timestamp(dt.ToTicks(), dt.GetTimeZoneQHR()));
			}
			else
			{
				...
				pf->AddCompData(fd, currOfst + headerSize, dataSize, &cinfo, CSTRP(sbuff, sptr), Data::Timestamp(dt.ToTicks(), dt.GetTimeZoneQHR()));
			}
		}
