...
void Parser::FileParser::TILParser::PrepareSelector(IO::IFileSelector *selector, IO::ParserType t)
{
...
IO::ParsedObject *Parser::FileParser::TILParser::ParseFile(IO::IStreamData *fd, IO::PackageFile *pkgFile, IO::ParserType targetType)
{
	...
	if (flags & 2)
	{
		...
		while (i < indexSize)
		{
			...
			pf->AddData(fd, ReadUInt64(&indexBuff[i + 16]), ReadUInt64(&indexBuff[i + 24]), CSTRP(fileName, srcPtr), timeTicks);
			i += 32;
		}
		MemFree(indexBuff);
	}
	else if (flags & 1)
	{
		...
		while (dirOfst < fileSize)
		{
			...
			pf->AddData(fd, fileOfst, ReadUInt64(&indexBuff[24]), CSTRP(fileName, srcPtr), timeTicks);
		}
	}
	else
	{
