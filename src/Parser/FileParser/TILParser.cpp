...
void Parser::FileParser::TILParser::PrepareSelector(NotNullPtr<IO::FileSelector> selector, IO::ParserType t)
{
...
IO::ParsedObject *Parser::FileParser::TILParser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UInt64 dirOfst;
	UInt64 fileSize;
	Int32 flags;
	...
	if (flags & 2)
	{
		...
		while (i < indexSize)
		{
			...
			pf->AddData(fd, ReadUInt64(&indexBuff[i + 16]), ReadUInt64(&indexBuff[i + 24]), IO::PackFileItem::HeaderType::No, 0, CSTRP(fileName, srcPtr), Data::Timestamp(timeTicks, 0), 0, 0, 0);
			i += 32;
		}
	}
	else if (flags & 1)
	{
		...
		while (dirOfst < fileSize)
		{
			...
			pf->AddData(fd, fileOfst, ReadUInt64(&indexBuff[24]), IO::PackFileItem::HeaderType::No, 0, CSTRP(fileName, srcPtr), Data::Timestamp(timeTicks, 0), 0, 0, 0);
		}
	}
	else
	{
