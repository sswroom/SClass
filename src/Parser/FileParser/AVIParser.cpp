...
//	hdrlSize = *(Int32*)&chunkBuffer[4];
	...
	avih = (MyAVIHeader*)MAlloc(i = *(Int32*)&chunkBuffer[4]);
	...
	{
		buffer = MemAlloc(UInt8, i = *(Int32*)&chunkBuffer[4] - 4);
		...
		MemCopyNO(&strl[l].strh, &buffer[8], *(Int32*)&buffer[4]);
		...
		strl[l].strf = MemAlloc(UInt8, strl[l].strfSize = *(Int32*)&buffer[j + 4]);
		MemCopyNO(strl[l].strf, &buffer[j + 8], strl[l].strfSize);
		j += 8 + strl[l].strfSize;
		if (*(UInt32*)&chunkBuffer[4] - 4 > j)
			if (*(Int32*)&buffer[j] == *(Int32*)"JUNK")
				j += *(Int32*)&buffer[j + 4] + 8;
		if (*(UInt32*)&chunkBuffer[4] - 4 > j)
		{
			strl[l].otherSize = *(Int32*)&chunkBuffer[4] - j - 4;
	...
	while (offset <= *(UInt32*)&headBuffer[4] + 4)
	{
		...
		{
			offset -= 8;
			idx1 = MemAlloc(UInt8, i = *(Int32*)&chunkBuffer[4] + 4);
		...
		else if (*(Int32*)chunkBuffer == *(Int32*)"LIST")
		{
			if (*(Int32*)&chunkBuffer[8] == *(Int32*)"INFO")
			{
				if (info)
					MemFree(info);
				info = MemAlloc(UInt8, i = *(Int32*)&chunkBuffer[4]);
				*(Int32*)info = i;
...
//						frameCnt = *(Int64*)&buffer[l + 8];
		...
		{
			offset += *(Int32*)&chunkBuffer[4] - 4;
		}
		else if (*(Int32*)chunkBuffer == *(Int32*)"SMCH")
		{
			offset -= 4;
			chap = MemAlloc(UInt8, i = *(Int32*)&chunkBuffer[4]);
		...
		offset += (i = (Int32)fd->GetRealData(offset, 12, chunkBuffer));
		if (i == 0)
			break;
	}
...
//			Int32 ccCoord = 0;
			...
			Data::ArrayListInt64 *ofsts;
			Data::ArrayListInt32 *sizes;
			NEW_CLASS(ofsts, Data::ArrayListInt64());
			NEW_CLASS(sizes, Data::ArrayListInt32());
			...
			while (j < strl[i].otherSize)
			{
				...
				else if (*(Int32*)&strl[i].others[j] == *(Int32*)"indx")
				{
					...
					while (k < *(UInt32*)&indx[4])
					{
						...
						base = *(Int64*)&buffer[20];
						l = 32;
						while (l < *(UInt32*)&indx[(k << 4) + 32])
						{
							if (*(UInt32*)&buffer[l] || *(Int32*)&buffer[l + 4])
							{
								ofsts->Add(base + *(UInt32*)&buffer[l]);
								sizes->Add(*(Int32*)&buffer[l + 4]);
							}
							l += 8;
						}
						MemFree(buffer);
...
//					ccCoord = strl[i].others[j + 10];
				...
				{
					arTop = *(Int16*)&strl[i].others[j + 8];
					arBottom = *(Int16*)&strl[i].others[j + 10];
					arLeft = *(Int16*)&strl[i].others[j + 12];
			...
			if (!error)
			{
				...
				Int32 scale = strl[i].strh.dwScale;
				Int32 rate = strl[i].strh.dwRate;
				k = (Int32)ofsts->GetCount();
				j = 0;
				while (j < k)
				{
					Int32 size = sizes->GetItem(j);
					...
					*(Char*)&cmpTmp += i / 10;
					while (k < l)
					{
						if (*(Int32*)&idx1[k] == cmpTmp)
							blkData->Append(base + *(Int32*)&idx1[k + 8] + 4, *(Int32*)&idx1[k + 12]);
						k += 16;
					}
					NEW_CLASS(lpcmData, Media::LPCMSource(blkData, 0,  blkData->GetDataSize(), &fmt, audsName));
					DEL_CLASS(blkData);
				...
				else
				{
					...
					*(Char*)&cmpTmp += i / 10;
					while (k < l)
					{
						if (*(Int32*)&idx1[k] == cmpTmp)
						{
							totalSize += *(Int32*)&idx1[k + 12];
							audsData->AddBlock(base + *(Int32*)&idx1[k + 8] + 4, *(Int32*)&idx1[k + 12], EstimateDecodeSize(&fmt, totalSize, *(Int32*)&idx1[k + 12]));
						}
						k += 16;
					}
				}
...
/*			Data::ArrayList<DataSegment*> *dsList = new Data::ArrayList<DataSegment*>();
	...
	if (chap)
	{
		...
		while (i < dataCnt)
		{
			frameNum = *(Int32*)&chap[i * 8 + 4];
			chapOfst = *(Int32*)&chap[i * 8 + 8];
			
			chapTime = MulDiv32(frameNum, scale * 1000, rate);
...
Int32 Parser::FileParser::AVIParser::EstimateDecodeSize(Media::AudioFormat *fmt, Int64 totalSize, OSInt frameSize)
{
	if (fmt->formatId == 1)
	{
		return (Int32)(frameSize / fmt->align);
	}
	if (fmt->formatId == 255)
	{
		return 1024;
	}
	Int32 lastSize = (Int32)((totalSize - frameSize) * fmt->frequency / (fmt->bitRate >> 3));
	return (Int32)(totalSize * fmt->frequency / (fmt->bitRate >> 3)) - lastSize;
}
