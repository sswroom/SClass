...
//	isUncompress = ReadUInt32(&hdr[44]);
	...
	while (i < fileCnt)
	{
		...
		{
			pf->AddData(fd, fileInfo[i].startOfst, fileInfo[i].decompSize, CSTRP(sbuff, sptr), 0, 0, 0, 0);
		}
		else
		{
			...
			pf->AddCompData(fd, fileInfo[i].startOfst, fileInfo[i].storeSize, &compInfo, CSTRP(sbuff, sptr), 0, 0, 0, 0);
		}
		i++;
	}
	MemFree(fileInfo);
