...
{
	if (hdr.ReadNU64(0) != *(UInt64*)"ElfFile" || ReadLUInt32(&hdr[32]) != 128 || ReadLUInt16(&hdr[40]) != 4096)
	{
		return nullptr;
	}
	...
	while (fd->GetRealData(ofst, 65536, buff) == 65536)
	{
		...
		{
//			UInt64 firstEvtRecNum = ReadLUInt64(&buff[8]);
//			UInt64 lastEvtRecNum = ReadLUInt64(&buff[16]);
//			UInt64 firstEvtRecId = ReadLUInt64(&buff[24]);
//			UInt64 lastEvtRecId = ReadLUInt64(&buff[32]);
//			UInt32 hdrSize = ReadLUInt32(&buff[40]);
			dataOfst = 512;
			UInt32 freeSpaceOfst = ReadLUInt32(&buff[48]);
			while (dataOfst < freeSpaceOfst)
			{
				if (ReadLUInt32(&buff[dataOfst]) != 0x2A2A)
				{
					break;
				}
				UInt32 recSize = ReadLUInt32(&buff[dataOfst + 4]);
				UInt64 evtRecId = ReadLUInt64(&buff[dataOfst + 8]);
...
#endif
			//UInt16 depId = ReadLUInt16(&chunk[ofst + 1]);
			dataSize = ReadLUInt32(&chunk[ofst + 3]);
			ofst += 7;
...
#endif
			nChar = ReadLUInt16(&chunk[ofst + 2]);
...
#endif
			dataOfst = ReadLUInt32(&chunk[ofst + 1]);
			ofst += 5;
			sb->AppendUTF8Char(' ');
			if (dataOfst == ofst)
			{
				UInt16 nChar = ReadLUInt16(&chunk[ofst + 6]);
				sb->AppendUTF16((const UTF16Char*)&chunk[ofst + 8], nChar);
				ofst += 8 + (UIntOS)(nChar + 1) * 2;
			}
			else
			{
				UInt16 nChar = ReadLUInt16(&chunk[dataOfst + 6]);
				sb->AppendUTF16((const UTF16Char*)&chunk[dataOfst + 8], nChar);
			}
			if (chunk[ofst] == 5 && chunk[ofst + 1] == 1)
			{
				UInt16 nChar = ReadLUInt16(&chunk[ofst + 2]);
				sb->AppendUTF8Char('=');
				sb->AppendUTF8Char('"');
				sb->AppendUTF16((const UTF16Char*)&chunk[ofst + 4], nChar);
				sb->AppendUTF8Char('"');
...
#endif
			dataOfst = ReadLUInt32(&chunk[ofst + 6]);
			ofst += 10;
			if (dataOfst == ofst)
			{
				ofst += 20;
