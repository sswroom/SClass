...
IO::ParsedObject *Parser::FileParser::NOAParser::ParseFileHdr(NN<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UInt8 tagBuff[16];
	UInt32 tagSize;
	UInt32 dataOfst;
	...
	while (i < recCnt)
	{
		...
		pf->AddData(fd, fileOfst + (UInt64)dataOfst, fileSize, IO::PackFileItem::HeaderType::No, CSTRP(fileName, sptr), Data::Timestamp(dt.ToInstant(), 0), 0, 0, 0);
