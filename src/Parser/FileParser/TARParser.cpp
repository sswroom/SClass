...
IO::ParsedObject *Parser::FileParser::TARParser::ParseFileHdr(NotNullPtr<IO::StreamData> fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	...
	while (true)
	{
				...
				while (true)
				{
					...
					if (i != INVALID_INDEX)
					{
						...
						if (pf3 == 0)
						{
							...
							pf2->AddPack(pf3, {sptr, i}, Data::Timestamp(t * 1000LL, 0), 0, 0);
						}
						pf2 = pf3;
						sptr = &sptr[i + 1];
					}
					else 
					{
						if (sptr[0] != 0)
						{
							...
							{
								NEW_CLASS(pf3, IO::PackageFile(sb.ToCString()));
								pf2->AddPack(pf3, CSTRP(sptr, sptrEnd), Data::Timestamp(t * 1000LL, 0), 0, 0);
							}
						}
						break;
					}
		...
		pf2->AddData(fd, currOfst, itemSize, CSTRP(sptr, sptrEnd), Data::Timestamp(t * 1000LL, 0), 0, 0);
		if (itemSize & 511)
		{
