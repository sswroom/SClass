...
{
	if (*(Int32*)&hdr[0] != *(Int32*)"3DLT" || ReadLInt32(&hdr[4]) != 1 || ReadLInt32(&hdr[84]) != 0)
	{
		return nullptr;
	}
	UInt32 inputBpp = ReadLUInt32(&hdr[48]);
	UInt32 outputBpp = ReadLUInt32(&hdr[64]);
	UInt32 paramOfst = ReadLUInt32(&hdr[72]);
	UInt32 paramSize = ReadLUInt32(&hdr[76]);
	UInt32 lutOfst = ReadLUInt32(&hdr[80]);
	UInt32 lutSize = ReadLUInt32(&hdr[92]);
