...
//						OSInt decBpl = (((imgWidth * bpp) & ~7) + 7)/ 8;
	...
	if (imgList->GetCount() == 1 && targetType != IO::ParsedObject::PT_IMAGE_LIST_PARSER)
	{
		...
		if (fd->IsFullFile())
		{
			...
			if (IO::Path::GetPathType(fileNameBuff) == IO::Path::PT_FILE)
			{
				IO::FileStream *fs;
				...
				Double xCoord;
				Double yCoord;
				NEW_CLASS(fs, IO::FileStream(fileNameBuff, IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_SEQUENTIAL));
