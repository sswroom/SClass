...
void Parser::FileParser::GIFParser::PrepareSelector(NN<IO::FileSelector> selector, IO::ParserType t)
{
...
Optional<IO::ParsedObject> Parser::FileParser::GIFParser::ParseFileHdr(NN<IO::StreamData> fd, Optional<IO::PackageFile> pkgFile, IO::ParserType targetType, Data::ByteArrayR hdr)
{
//	UInt8 ver;
			...
			while (true)
			{
				...
				if (blockType < 0x20)
				{
					...
					if (scnImg32.GetSize() > 0)
					{
						...
						MemCopyANC(simg->data.Ptr(), scnImg32.Ptr(), scnWidth * scnHeight * 4);
					}
					else
					{
						...
						MemCopyANC(simg->data.Ptr(), scnImg.Arr().Ptr(), scnWidth * scnHeight);
						if (localColorTable.GetSize() > 0)
						{
