...
IO::ParsedObject *Parser::FileParser::ZWEIParser::ParseFileHdr(IO::StreamData *fd, IO::PackageFile *pkgFile, IO::ParserType targetType, const UInt8 *hdr)
{
	UInt32 extCnt;
	UInt32 extOfst;
	UInt32 fileOfst;
	...
	while (i < extCnt)
	{
		...
		while (j < recCnt)
		{
			...
			*sptr++ = '.';
			...
			pf->AddData(fd, fileOfst, fileSize, CSTRP(name, sptr), Data::Timestamp(0));
			fileOfst += fileSize;
