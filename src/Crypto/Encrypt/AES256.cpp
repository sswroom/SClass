...
Crypto::Encrypt::AES256::AES256(const UInt8 *key) : Crypto::Encrypt::BlockCipher(16)
{
	this->SetKey(key);
}
...
OSInt Crypto::Encrypt::AES256::EncryptBlock(const UInt8 *inBlock, UInt8 *outBlock, void *encParam)
{
	UInt32 s0;
	UInt32 s1;
	UInt32 s2;
	...
	s3 =
		...
		this->encRK[59];
	WriteMInt32(outBlock + 12, s3);
	return 16;
}

OSInt Crypto::Encrypt::AES256::DecryptBlock(const UInt8 *inBlock, UInt8 *outBlock, void *decParam)
{
	UInt32 s0;
	UInt32 s1;
	UInt32 s2;
