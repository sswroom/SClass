...
const UInt32 Crypto::Encrypt::Blowfish::origP[18] = {
...
const UInt32 Crypto::Encrypt::Blowfish::origS[1024] = {
...
void Crypto::Encrypt::Blowfish::EncryptInt(NN<EncryptParam> param) const
{
	UInt32 Xl;
	UInt32 Xr;
	UInt32 temp;
	...
	ENCINTSTEP(13);
	ENCINTSTEP(15);
	param->xl = Xr ^ this->p[N + 1];
	param->xr = Xl;
}

void Crypto::Encrypt::Blowfish::DecryptInt(NN<EncryptParam> param) const
{
	UInt32 Xl;
	UInt32 Xr;
	UInt32 temp;
...
void Crypto::Encrypt::Blowfish::Key(UnsafeArray<const UInt8> password, UOSInt pwdLen)
{
	...
	while (i < 4)
	{
		...
		{
			this->EncryptInt(param);
			this->s[i][j + 0] = param.xl;
			this->s[i][j + 1] = param.xr;
			j += 2;
		}
		i++;
	}
	...
	else
	{
		...
		while (i < 4)
		{
			...
			while (j < 256)
			{
				...
				param.xr = param.xr ^ salt4;
				this->EncryptInt(param);
				this->s[i][j + 2] = param.xl;
				this->s[i][j + 3] = param.xr;
				j += 4;
			}
			i++;
		}
...
Crypto::Encrypt::Blowfish::~Blowfish()
{
...
UOSInt Crypto::Encrypt::Blowfish::EncryptBlock(UnsafeArray<const UInt8> inBlock, UnsafeArray<UInt8> outBlock) const
{
	...
	param.xr = ReadMUInt32(&inBlock[4]);
	this->EncryptInt(param);
	WriteMUInt32(&outBlock[0], param.xl);
	WriteMUInt32(&outBlock[4], param.xr);
	return 8;
}
...
void Crypto::Encrypt::Blowfish::EncryptBlk(UnsafeArray<UInt32> lr)
{
	EncryptParam param;
	param.xl = lr[0];
	param.xr = lr[1];
