...
UnsafeArray<UTF8Char> Crypto::Hash::SHA224::GetName(UnsafeArray<UTF8Char> sbuff) const
{
	return Text::StrConcatC(sbuff, UTF8STRC("SHA-224"));
}

NN<Crypto::Hash::HashAlgorithm> Crypto::Hash::SHA224::Clone() const
{
	NN<Crypto::Hash::SHA224> sha224;
	NEW_CLASSNN(sha224, Crypto::Hash::SHA224());
...
void Crypto::Hash::SHA224::Calc(UnsafeArray<const UInt8> buff, UIntOS buffSize)
{
	...
	while (buffSize >= 64)
	{
		SHA256_CalcBlock(this->intermediateHash, buff.Ptr());
		buff += 64;
		buffSize -= 64;
	}
	if (buffSize > 0)
...
UIntOS Crypto::Hash::SHA224::GetBlockSize() const
{
	return 64;
}

UIntOS Crypto::Hash::SHA224::GetResultSize() const
{
	return 28;
}
