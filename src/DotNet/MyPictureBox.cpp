...
void DotNet::MyPictureBox::SetImage(Media::Image *currImage)
{
	...
	if (this->currImage)
	{
		this->csconv = Media::CS::CSConverter::NewConverter(this->currImage->info->fourcc, this->currImage->info->bpp, this->currImage->info->rgbType, 0, 32, Media::CS::TRANodeType::Unknown, this->currImage->info->rgbGamma, this->currImage->info->yuvType, this->dnColorMgr->GetColorManager());
		if (this->csconv)
		{
...
void DotNet::MyPictureBox::YUVParamChanged(NN<const Media::ColorHandler::YUVPARAM> yuvParam)
{
...
void DotNet::MyPictureBox::RGBParamChanged(const Media::ColorHandler::RGBPARAM *rgbParam)
{
	if (this->currImage && this->csconv)
	{
		if (this->currImage->GetImageType() == Media::Image::ImageType::Static)
