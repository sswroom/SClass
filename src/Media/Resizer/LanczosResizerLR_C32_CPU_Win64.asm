...
LanczosResizerLR_C32_CPU_horizontal_filter:
	...
	punpckldq xmm7,xmm7
	mov rdx,r8 ;width
	lea rcx,[rdx*8]				; IntOS dAdd = dstep - width * 8;
...
LanczosResizerLR_C32_CPU_vertical_filter_na:
	...
	mov rax,qword [rsp+72] ;tap
	lea rcx,[rdx*4]
	sub qword [rsp+104],rcx ;dAdd			;IntOS dAdd = dstep - width * 4;
	test rdx,1						;if (width & 1)
	jz vfnastart2
	cmp rax,6
	jz vf6na_1start
...
LanczosResizerLR_C32_CPU_collapse:
	...
	lea rcx,[r8*8] ;width
	lea rdx,[r8*4] ;width
	sub qword [rsp+72],rcx ;sAdd		IntOS sAdd = sstep - width * 8;
	sub qword [rsp+80],rdx ;dAdd		IntOS dAdd = dstep - width * 4;
	test r8,3
	jnz colstart
	test r11,15 ;outPt
	jnz colstart
