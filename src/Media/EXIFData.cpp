...
Media::EXIFData::EXIFInfo Media::EXIFData::defInfos[] = {
	...
	{42112, CSTR("GdalMetadata")},
	{42113, CSTR("GdalNoData")},
...
void Media::EXIFData::FreeItem(NN<Media::EXIFData::EXIFItem> item)
{
	if (item->dataBuff.NotNull())
	{
		...
		{
			MemFree(item->dataBuff.GetOpt<UInt8>().OrNull());
		}
	}
	MemFreeNN(item);
}
...
void Media::EXIFData::ToExifBuffImpl(UnsafeArray<UInt8> buff, NN<const Data::ReadingListNN<Media::EXIFData::EXIFItem>> exifList, InOutParam<UInt32> startOfst, InOutParam<UInt32> otherOfst) const
{
	...
	while (i < exifList->GetCount())
	{
		...
		case Media::EXIFData::ET_BYTES:
			WriteLInt16(&buff[j], exif->id);
			WriteLInt16(&buff[j + 2], 1);
			WriteLUInt32(&buff[j + 4], (UInt32)exif->cnt);
			if (exif->cnt <= 4)
			{
				WriteLInt32(&buff[j + 8], 0);
				MemCopyNO(&buff[j + 8], &exif->value, (UIntOS)exif->cnt);
				j += 12;
			}
			else
			{
				WriteLUInt32(&buff[j + 8], (UInt32)k);
	...
	WriteLInt32(&buff[j], 0);
	WriteLInt16(&buff[startOfst.Get()], objCnt);
	j += 4;
	...
	while (i < exifList->GetCount())
	{
		...
		{
			WriteLUInt32(&buff[exif->value], j);
			exif->dataBuff.GetNN<Media::EXIFData>()->ToExifBuff(buff, j, k32);
		}
		i++;
	}
	startOfst.Set(j);
...
void Media::EXIFData::GetExifBuffSize(NN<const Data::ReadingListNN<EXIFItem>> exifList, OutParam<UInt64> size, OutParam<UInt64> endOfst) const
{
	...
	while (k-- > 0)
	{
		...
		else if (exif->type == Media::EXIFData::ET_SUBEXIF)
		{
			i += 12;
			j += 12;
			exif->dataBuff.GetNN<Media::EXIFData>()->GetExifBuffSize(l, m);
			i += m;
			j += l;
		}
...
NN<Media::EXIFData> Media::EXIFData::Clone() const
{
	...
	while (i < j)
	{
		...
		case ET_BYTES:
			...
			{
				newExif->AddBytes(item->id, item->cnt, item->dataBuff.GetOpt<UInt8>().OrNull());
			}
			break;
		case ET_STRING:
			newExif->AddString(item->id, item->cnt, item->dataBuff.GetOpt<Char>().OrNull());
			break;
		case ET_UINT16:
			if (item->cnt <= 2)
			{
			...
			{
				newExif->AddUInt16(item->id, item->cnt, item->dataBuff.GetOpt<UInt16>().OrNull());
			}
			break;
		case ET_UINT32:
			if (item->cnt == 1)
			...
			{
				newExif->AddUInt32(item->id, item->cnt, item->dataBuff.GetOpt<UInt32>().OrNull());
			}
			break;
		case ET_RATIONAL:
			newExif->AddRational(item->id, item->cnt, item->dataBuff.GetOpt<UInt32>().OrNull());
			break;
		case ET_OTHER:
			newExif->AddOther(item->id, item->cnt, item->dataBuff.GetOpt<UInt8>().OrNull());
			break;
		case ET_INT16:
			if (item->cnt <= 2)
			{
			...
			{
				newExif->AddInt16(item->id, item->cnt, item->dataBuff.GetOpt<Int16>().OrNull());
			}
			break;
		case ET_INT32:
			if (item->cnt <= 1)
			...
			{
				newExif->AddInt32(item->id, item->cnt, item->dataBuff.GetOpt<Int32>().OrNull());
			}
			break;
		case ET_SUBEXIF:
			newExif->AddSubEXIF(item->id, item->dataBuff.GetNN<Media::EXIFData>()->Clone());
			break;
		case ET_SRATIONAL:
			newExif->AddSRational(item->id, item->cnt, item->dataBuff.GetOpt<Int32>().OrNull());
			break;
		case ET_DOUBLE:
			newExif->AddDouble(item->id, item->cnt, item->dataBuff.GetOpt<Double>().OrNull());
			break;
		case ET_UINT64:
			newExif->AddUInt64(item->id, item->cnt, item->dataBuff.GetOpt<UInt64>().OrNull());
			break;
		case ET_INT64:
			newExif->AddInt64(item->id, item->cnt, item->dataBuff.GetOpt<Int64>().OrNull());
			break;
		case ET_UNKNOWN:
		default:
			break;
...
void Media::EXIFData::AddBytes(UInt32 id, UInt64 cnt, UnsafeArray<const UInt8> buff)
{
	...
	else
	{
		...
		item->dataBuff = MemAllocArr(UInt8, cnt);
...
void Media::EXIFData::AddUInt16(UInt32 id, UInt64 cnt, UnsafeArray<const UInt16> buff)
{
	...
	else
	{
		...
		item->dataBuff = MemAllocArr(UInt16, cnt);
...
void Media::EXIFData::AddUInt32(UInt32 id, UInt64 cnt, UnsafeArray<const UInt32> buff)
{
	...
	{
		item->value = (Int32)*buff;
		item->dataBuff = MemAllocArr(UInt32, cnt);
...
void Media::EXIFData::AddInt16(UInt32 id, UInt64 cnt, UnsafeArray<const Int16> buff)
{
	...
	{
		item->value = ReadNInt32((const UInt8*)&buff[0]);
		item->dataBuff = MemAllocArr(Int16, cnt);
...
void Media::EXIFData::AddInt32(UInt32 id, UInt64 cnt, UnsafeArray<const Int32> buff)
{
	...
	{
		item->value = ReadNInt32((const UInt8*)&buff[0]);
		item->dataBuff = MemAllocArr(Int32, cnt);
...
void Media::EXIFData::AddRational(UInt32 id, UInt64 cnt, UnsafeArray<const UInt32> buff)
{
	...
	item->cnt = cnt;
	item->value = 0;
	item->dataBuff = MemAllocArr(UInt32, cnt << 1);
...
void Media::EXIFData::AddSRational(UInt32 id, UInt64 cnt, UnsafeArray<const Int32> buff)
{
	...
	item->cnt = cnt;
	item->value = 0;
	item->dataBuff = MemAllocArr(Int32, cnt << 1);
...
void Media::EXIFData::AddDouble(UInt32 id, UInt64 cnt, UnsafeArray<const Double> buff)
{
	...
	item->cnt = cnt;
	item->value = 0;
	item->dataBuff = MemAllocArr(Double, cnt);
...
void Media::EXIFData::AddUInt64(UInt32 id, UInt64 cnt, UnsafeArray<const UInt64> buff)
{
	...
	item->cnt = cnt;
	item->value = 0;
	item->dataBuff = MemAllocArr(UInt64, cnt);
...
void Media::EXIFData::AddInt64(UInt32 id, UInt64 cnt, UnsafeArray<const Int64> buff)
{
	...
	item->cnt = cnt;
	item->value = 0;
	item->dataBuff = MemAllocArr(Int64, cnt);
...
Bool Media::EXIFData::RemoveLargest()
{
	UInt32 largestId = 0;
	...
	while (i < j)
	{
		...
		if (item->type == ET_SUBEXIF)
		{
			...
			while (k < l)
			{
				item = data->exifMap.GetItemNoCheck(k);
				if (item->cnt > largestSize)
				{
					largestSize = item->cnt;
					largestId = id;
				}
				k++;
			}
	...
	if (largestSize == 0 || !this->exifMap.Get(largestId).SetTo(item))
	{
		return false;
	}
	if (item->type != ET_SUBEXIF)
	{
		this->exifMap.Remove(largestId);
		FreeItem(item);
		return true;
	}
	NN<Media::EXIFData> data = item->dataBuff.GetNN<Media::EXIFData>();
...
Bool Media::EXIFData::GetPhotoDate(NN<Data::DateTime> dt) const
{
	...
	if (this->exifMaker == EM_STANDARD)
	{
		if (this->exifMap.Get(36867).SetTo(item))
		{
			...
			{
				dt->SetValue(Text::CStringNN(item->dataBuff.GetOpt<const UTF8Char>().OrNull(), item->cnt - 1));
				return true;
			}
		}
...
Bool Media::EXIFData::GetPhotoDate(OutParam<Data::Timestamp> ts) const
{
	...
	if (this->exifMaker == EM_STANDARD)
	{
		if (this->exifMap.Get(36867).SetTo(item))
		{
			if (item->type == ET_STRING)
			{
				ts.Set(Data::Timestamp::FromStr(Text::CStringNN(item->dataBuff.GetOpt<const UTF8Char>().OrNull(), item->cnt - 1), Data::DateTimeUtil::GetLocalTzQhr()));
				return true;
			}
		}
		if (this->exifMap.Get(36868).SetTo(item))
...
Text::CString Media::EXIFData::GetPhotoMake() const
{
	...
	if (this->exifMaker == EM_STANDARD)
	{
		if (this->exifMap.Get(271).SetTo(item))
		{
			if (item->type == ET_STRING)
			{
				return Text::CString(item->dataBuff.GetOpt<const UTF8Char>().OrNull(), item->cnt - 1);
			}
		}
	}
	return nullptr;
...
Double Media::EXIFData::GetPhotoFNumber() const
{
	...
	if (this->exifMaker == EM_STANDARD)
	{
		if (this->exifMap.Get(33437).SetTo(item))
		{
			...
			{
				UInt32 *dataBuff = item->dataBuff.GetOpt<UInt32>().OrNull();
	...
	{7, L"GPSTimeStamp"},*/
		...
		if (subExif->GetExifItem(1).SetTo(item1) && subExif->GetExifItem(2).SetTo(item2))
		{
			if (item2->type == Media::EXIFData::ET_RATIONAL)
			{
				if (item2->cnt == 3)
				{
					val = ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[0]) / (Double)ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[4]);
					val += ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[8]) / (Double)ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[12]) / 60.0;
					val += ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[16]) / (Double)ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[20]) / 3600.0;
				}
				else if (item2->cnt == 1)
				{
					val = ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[0]) / (Double)ReadLInt32(&item2->dataBuff.GetOpt<UInt8>().OrNull()[4]);
		...
		if (subExif->GetExifItem(3).SetTo(item1) && subExif->GetExifItem(4).SetTo(item2))
		{
			if (item2->type == Media::EXIFData::ET_RATIONAL)
			{
				if (item2->cnt == 3)
				{
					val = ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[0]) / (Double)ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[4]);
					val += ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[8]) / (Double)ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[12]) / 60.0;
					val += ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[16]) / (Double)ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[20]) / 3600.0;
				}
				else if (item2->cnt == 1)
				{
					val = ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[0]) / (Double)ReadLInt32(&item2->dataBuff.GetArray<UInt8>()[4]);
		...
		if (subExif->GetExifItem(7).SetTo(item1) && subExif->GetExifItem(29).SetTo(item2))
		{
			...
			{
				if (ReadLInt32(&item1->dataBuff.GetOpt<UInt8>().OrNull()[4]) != 1 || ReadLInt32(&item1->dataBuff.GetOpt<UInt8>().OrNull()[12]) != 1)
				{
					succ = false;
				}
				else
				{
					hh = ReadLInt32(&item1->dataBuff.GetOpt<UInt8>().OrNull()[0]);
					mm = ReadLInt32(&item1->dataBuff.GetOpt<UInt8>().OrNull()[8]);
					val = ReadLInt32(&item1->dataBuff.GetOpt<UInt8>().OrNull()[16]) / (Double)ReadLInt32(&item1->dataBuff.GetOpt<UInt8>().OrNull()[20]);
					ss = (Int32)val;
...
Bool Media::EXIFData::GetGeoBounds(Math::Size2D<UIntOS> imgSize, OutParam<UInt32> srid, OutParam<Double> minX, OutParam<Double> minY, OutParam<Double> maxX, OutParam<Double> maxY) const
{
	...
	if (this->exifMap.Get(33922).SetTo(item) && this->exifMap.Get(33550).SetTo(item2))
	{
		...
		ptr = item2->dataBuff.GetOpt<Double>().OrNull();
		mppX = ptr[0];
		mppY = ptr[1];
	...
	if (this->exifMap.Get(34264).SetTo(item))
	{
		if (item->cnt == 16)
		{
			...
			Double *transfMatrix = item->dataBuff.GetOpt<Double>().OrNull();
			minX.Set(transfMatrix[3]);
			maxY.Set(transfMatrix[7]);
			maxX.Set(transfMatrix[3] + transfMatrix[0] * dimgSize.x + transfMatrix[1] * dimgSize.y);
			minY.Set(transfMatrix[7] + transfMatrix[4] * dimgSize.x + transfMatrix[5] * dimgSize.y);
			srid.Set(fileSRID);
			return true;
		}
	}
...
Double Media::EXIFData::GetHDPI() const
{
	...
	Int32 *arr = item->dataBuff.GetOpt<Int32>().OrNull();
...
Bool Media::EXIFData::ToString(NN<Text::StringBuilderUTF8> sb, Text::CString linePrefix) const
{
	...
	while (i < j)
	{
		...
		if (this->GetExifItem(v).SetTo(exItem))
		{
				...
				while (i2 < j2)
				{
					...
					if (subExif->GetExifItem(v2).SetTo(subExItem))
					{
						if (subExItem->type == Media::EXIFData::ET_STRING)
						{
							...
							if (subExItem->dataBuff.GetOpt<Char>().OrNull()[subExItem->cnt - 1])
							{
								sb->AppendC(subExItem->dataBuff.GetOpt<UTF8Char>().OrNull(), subExItem->cnt);
							}
							else
							{
								sb->AppendSlow(subExItem->dataBuff.GetOpt<UTF8Char>().OrNull());
							}
						}
						else if (subExItem->type == Media::EXIFData::ET_DOUBLE)
						{
							Double *valBuff = subExItem->dataBuff.GetOpt<Double>().OrNull();
							k = 0;
						...
						else if (subExItem->type == Media::EXIFData::ET_UINT16)
						{
							...
							{
								valBuff = subExItem->dataBuff.GetOpt<UInt16>().OrNull();
							}
							k = 0;
							while (k < subExItem->cnt)
							{
						...
						else if (subExItem->type == Media::EXIFData::ET_UINT32)
						{
							...
							{
								valBuff = subExItem->dataBuff.GetOpt<UInt32>().OrNull();
							}
							k = 0;
							while (k < subExItem->cnt)
							{
						...
						{
							Int32 *valBuff;
							valBuff = subExItem->dataBuff.GetOpt<Int32>().OrNull();
							k = 0;
							while (k < subExItem->cnt)
							{
								if (k == 0)
						...
						else if (subExItem->type == Media::EXIFData::ET_INT16)
						{
							...
							{
								valBuff = subExItem->dataBuff.GetOpt<Int16>().OrNull();
							}
							k = 0;
							while (k < subExItem->cnt)
							{
			...
			{
				sb->AppendC(UTF8STRC(", value = "));
				if (exItem->dataBuff.GetOpt<Char>().OrNull()[exItem->cnt - 1])
				{
					sb->AppendC(exItem->dataBuff.GetOpt<UTF8Char>().OrNull(), exItem->cnt);
				}
				else
				{
					sb->AppendSlow(exItem->dataBuff.GetOpt<UTF8Char>().OrNull());
				}
			}
			else if (exItem->type == Media::EXIFData::ET_DOUBLE)
			{
				Double *valBuff = exItem->dataBuff.GetOpt<Double>().OrNull();
				k = 0;
				while (k < exItem->cnt)
				{
					if (k == 0)
			...
			else if (exItem->type == Media::EXIFData::ET_BYTES)
			{
				...
				{
					valBuff = exItem->dataBuff.GetOpt<UInt8>().OrNull();
				}
				sb->AppendC(UTF8STRC(", value = "));
			...
			else if (exItem->type == Media::EXIFData::ET_UINT16)
			{
				...
				{
					valBuff = exItem->dataBuff.GetOpt<UInt16>().OrNull();
				}
			...
			else if (exItem->type == Media::EXIFData::ET_UINT32)
			{
				...
				{
					valBuff = exItem->dataBuff.GetOpt<UInt32>().OrNull();
				}
				k = 0;
				while (k < exItem->cnt)
				{
			...
			{
				Int32 *valBuff;
				valBuff = exItem->dataBuff.GetOpt<Int32>().OrNull();
				k = 0;
				while (k < exItem->cnt)
				{
					if (k == 0)
			...
			else if (exItem->type == Media::EXIFData::ET_INT16)
			{
				...
				{
					valBuff = exItem->dataBuff.GetOpt<Int16>().OrNull();
				}
				k = 0;
				while (k < exItem->cnt)
				{
...
Optional<Media::EXIFData> Media::EXIFData::ParseIFD(UnsafeArray<const UInt8> buff, UIntOS buffSize, NN<Data::ByteOrder> bo, OptOut<UInt64> nextOfst, EXIFMaker exifMaker, UnsafeArrayOpt<const UInt8> optbasePtr)
{
	...
	while (i < ifdCnt)
	{
		...
		case 3:
		{
			...
			{
				tmpBuff = MemAllocArr(UInt8, fcnt << 1);
				MemCopyNO(&tmpBuff[0], &basePtr[bo->GetUInt32(&ifdEntries[ifdOfst + 8])], fcnt << 1);
				j = fcnt << 1;
				while (j > 0)
				{
					j -= 2;
				...
				exif->AddUInt16(tag, fcnt, UnsafeArray<UInt16>::ConvertFrom(tmpBuff));
				MemFreeArr(tmpBuff);
			}
			break;
		}
		case 4:
		{
			...
			{
				tmpBuff = MemAllocArr(UInt8, fcnt << 2);
				MemCopyNO(&tmpBuff[0], &basePtr[bo->GetUInt32(&ifdEntries[ifdOfst + 8])], fcnt << 2);
				j = fcnt << 2;
				while (j > 0)
				{
					j -= 4;
				...
				exif->AddUInt32(tag, fcnt, UnsafeArray<UInt32>::ConvertFrom(tmpBuff));
				MemFreeArr(tmpBuff);
			}
			break;
		}
		case 5:
			tmpBuff = MemAllocArr(UInt8, fcnt << 3);
			MemCopyNO(&tmpBuff[0], &basePtr[bo->GetUInt32(&ifdEntries[ifdOfst + 8])], fcnt << 3);
			j = fcnt << 3;
			while (j > 0)
			{
				j -= 8;
			...
			exif->AddRational(tag, fcnt, UnsafeArray<UInt32>::ConvertFrom(tmpBuff));
			MemFreeArr(tmpBuff);
			break;
		case 7:
			if (fcnt <= 4)
		...
		case 8:
		{
			...
			else
			{
				...
				exif->AddInt16(tag, fcnt, UnsafeArray<Int16>::ConvertFrom(tmpBuff));
				MemFreeArr(tmpBuff);
			}
			break;
		}
		case 9:
			...
			else
			{
				...
				exif->AddInt32(tag, fcnt, UnsafeArray<Int32>::ConvertFrom(tmpBuff));
				MemFreeArr(tmpBuff);
			}
			break;
		case 10:
			...
			exif->AddSRational(tag, fcnt, UnsafeArray<Int32>::ConvertFrom(tmpBuff));
			MemFreeArr(tmpBuff);
			break;
		case 12:
			tmpBuff = MemAllocArr(UInt8, fcnt << 3);
			...
			exif->AddDouble(tag, fcnt, UnsafeArray<Double>::ConvertFrom(tmpBuff));
			MemFreeArr(tmpBuff);
			break;
	...
	nextOfst.Set(bo->GetUInt32(&ifdEntries[ifdCnt * 12]));
	return exif;
}
...
Bool Media::EXIFData::ParseFrame(NN<IO::FileAnalyse::FrameDetailHandler> frame, UIntOS frameOfst, NN<IO::StreamData> fd, UInt64 ofst, NN<Data::ByteOrder> bo, OptOut<UInt32> nextOfst, UInt32 ifdId, UInt64 readBase)
{
	...
	while (i < ifdCnt)
	{
		...
		case 12:
		{
			...
			{
				j -= 8;
				frame->AddFloat((UIntOS)(fofst + readBase + j - ofst + frameOfst), 8, GetEXIFName(EM_STANDARD, ifdId, tag), ReadLDouble(&tmpBuff[j]));
			}
			break;
		}
		default:
