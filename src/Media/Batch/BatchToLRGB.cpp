	...
	while (i < j)
	{
		if (Optional<Media::StaticImage>::ConvertFrom(imgList->GetImage(i, 0)).SetTo(simg) && simg->info.fourcc != *(UInt32*)"LRGB")
		{
			...
			if (this->csconv.SetTo(csconv))
			{
				...
				csconv->ConvertV2(&simg->data, dimg->data, simg->info.dispSize.x, simg->info.dispSize.y, simg->info.storeSize.x, simg->info.storeSize.y, (IntOS)dimg->GetDataBpl(), Media::FT_NON_INTERLACE, Media::YCOFST_C_CENTER_LEFT);
