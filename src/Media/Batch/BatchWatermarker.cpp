...
void Media::Batch::BatchWatermarker::ImageOutput(NN<Media::ImageList> imgList, Text::CStringNN fileId, Text::CStringNN subId)
{
	...
	if (this->deng->CreateImage32(Math::Size2D<UOSInt>(16, 16), Media::AT_ALPHA_ALL_FF).SetTo(tmpImg))
	{
		i = 0;
		j = imgList->GetCount();
		while (i < j)
			...
			if (Optional<Media::StaticImage>::ConvertFrom(imgList->GetImage(i, 0)).SetTo(simg))
			{
				...
				while (true)
				{
					...
					if (sz.x <= UOSInt2Double(simg->info.dispSize.x) && sz.y <= UOSInt2Double(simg->info.dispSize.y))
					{
						...
						if (this->deng->CreateImage32(Math::Size2D<UOSInt>(iWidth, iHeight), Media::AT_ALPHA_ALL_FF).SetTo(gimg2))
						{
