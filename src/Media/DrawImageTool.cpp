	...
	while (true)
	{
		...
		while (true)
		{
			if (i >= j)
			{
				...
				{
					dimg->GetTextSizeUTF8(f, currTxt, Text::StrCharCnt(currTxt), sz);
					if (sz[0] <= width)
					{
						outStr->Add(Text::StrCopyNew(currTxt));
					}
			...
			if (c == ' ')
			{
				...
				dimg->GetTextSizeUTF8(f, tmpBuff, i, sz);
				if (sz[0] > width)
				{
					if (k == 0)
					{
			...
			else if (c == '-')
			{
				...
				dimg->GetTextSizeUTF8(f, tmpBuff, i + 1, sz);
				if (sz[0] > width)
				{
					if (k == 0)
					{
...
void Media::DrawImageTool::SplitString(Media::DrawImage *dimg, const WChar *txt, Data::ArrayList<const WChar *> *outStr, Media::DrawFont *f, Double width)
{
	OSInt i;
	OSInt j;
	OSInt k;
	Double sz[2];
	const WChar *currTxt = txt;
	WChar *tmpBuff;
	OSInt txtLen;
	txtLen = Text::StrCharCnt(currTxt);
	tmpBuff = MemAlloc(WChar, txtLen + 1);
	WChar c;
	WChar c2;
	while (true)
	{
