...
//		wprintf(L"Error in founding %ls\r\n", sbuff);
	...
	// hdmi
	NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/sys/class/hdmi/hdmi/attr/edid", IO::FileStream::FileMode::ReadOnly, IO::FileStream::FileShare::DenyNone, IO::FileStream::BufferType::Normal));
	if (!fs->IsError())
	{
		edidSize = fs->Read(edid, 1024);
		if (edidSize > 0)
	...
	if (ret == 0)
	{
		NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/sys/class/amhdmitx/amhdmitx0/rawedid", IO::FileStream::FileMode::ReadOnly, IO::FileStream::FileShare::DenyNone, IO::FileStream::BufferType::Normal));
		if (!fs->IsError())
		{
			edidSize = fs->Read(edid, 1024);
			if (edidSize > 0)
