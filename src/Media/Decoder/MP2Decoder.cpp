...
Media::Decoder::MP2Decoder::~MP2Decoder()
{
	if (this->context)
	{
...
Bool Media::Decoder::MP2Decoder::Start(Sync::Event *evt, UOSInt blkSize)
{
	if (this->sourceAudio)
	{
...
UOSInt Media::Decoder::MP2Decoder::ReadBlock(Data::ByteArray blk)
{
	...
	while (nBlk-- > 0)
	{
		...
		{
			kjmp2_decode_frame((kjmp2_context_t*)this->context, srcBuff, (Int16*)blk.Arr().Ptr());
			blk += 4608;
			outSize += 4608;
		}
	}
