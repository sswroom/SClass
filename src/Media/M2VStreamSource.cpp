			...
			if (nextIndex == this->playBuffStart)
			{
				if (this->playToStop || (!this->playing && clk.GetTimeDiff() >= 3))
				{
					...
					if (this->debugLog)
					{
						...
						sb.Append((const UTF8Char*)"Add to Play buff ");
...
UInt32 __stdcall Media::M2VStreamSource::PlayThread(void *userObj)
{
	...
	while (!me->playToStop)
	{
		...
		else
		{
			...
			{
				Text::StringBuilderUTF8 sb;
				sb.Append((const UTF8Char*)"Output frame ");
...
Media::M2VStreamSource::M2VStreamSource(Media::IStreamControl *pbc)
{
	...
	NEW_CLASS(this->debugFS, IO::FileStream((const UTF8Char*)"M2VDebug.txt", IO::FileStream::FILE_MODE_APPEND, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
...
void Media::M2VStreamSource::WriteFrameStream(UInt8 *buff, UOSInt buffSize)
{
	...
	{
		Text::StringBuilderUTF8 sb;
		sb.Append((const UTF8Char*)"WriteFrameStream ");
