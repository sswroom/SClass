...
Media::TimedImageList::TimedImageList(Text::CStringNN fileName)
{
	...
	else if (fileSize == 0)
	{
		...
		WriteInt64(&hdr[8], 16);
		this->fs->Write(Data::ByteArrayR(hdr, 16));
		this->flags = 1;
...
Bool Media::TimedImageList::AddImage(Int64 captureTimeTicks, const UInt8 *imgBuff, UOSInt imgSize, ImageFormat imgFmt)
{
	...
	if (!this->changed)
	{
		...
		this->fs->Write(Data::ByteArrayR(indexBuff, 16));
		this->fs->SeekFromBeginning(this->currFileOfst);
		this->changed = true;
	}
	...
	succ = succ && (this->fs->Write(Data::ByteArrayR(indexBuff, 32)) == 32);
	this->indexStm.Write(Data::ByteArrayR(indexBuff, 32));
	succ = succ && (this->fs->Write(Data::ByteArrayR(imgBuff, imgSize)) == imgSize);
