...
Media::CS::CSYUV_LRGBC::CSYUV_LRGBC(NN<const Media::ColorProfile> srcProfile, NN<const Media::ColorProfile> destProfile, Media::ColorProfile::YUVType yuvType, Optional<Media::ColorManagerSess> colorSess) : Media::CS::CSConverter(colorSess), srcProfile(srcProfile), destProfile(destProfile)
{
...
void Media::CS::CSYUV_LRGBC::YUVParamChanged(NN<const Media::ColorHandler::YUVPARAM> yuv)
{
	MemCopyNO(&this->yuvParam, yuv.Ptr(), sizeof(YUVPARAM));
	this->yuvUpdated = true;
}

void Media::CS::CSYUV_LRGBC::RGBParamChanged(NN<const Media::ColorHandler::RGBPARAM2> rgb)
{
	this->rgbParam.Set(rgb);
	this->rgbUpdated = true;
}

UIntOS Media::CS::CSYUV_LRGBC::GetDestFrameSize(UIntOS width, UIntOS height)
{
