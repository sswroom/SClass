...
Media::CS::CSYV12_LRGBC::CSYV12_LRGBC(NN<const Media::ColorProfile> srcProfile, NN<const Media::ColorProfile> destProfile, Media::ColorProfile::YUVType yuvType, Optional<Media::ColorManagerSess> colorSess) : Media::CS::CSYUV420_LRGBC(srcProfile, destProfile, yuvType, colorSess)
{
}

Media::CS::CSYV12_LRGBC::~CSYV12_LRGBC()
{
}

void Media::CS::CSYV12_LRGBC::ConvertV2(UnsafeArray<const UnsafeArray<UInt8>> srcPtr2, UnsafeArray<UInt8> destPtr, UOSInt dispWidth, UOSInt dispHeight, UOSInt srcStoreWidth, UOSInt srcStoreHeight, OSInt destRGBBpl, Media::FrameType ftype, Media::YCOffset ycOfst)
{
	this->UpdateTable();
	UInt32 isLast = 1;
	UInt32 isFirst = 0;
	...
	{
		UnsafeArray<UInt8> vStart = srcPtr + srcStoreWidth * srcStoreHeight;
		UnsafeArray<UInt8> uStart = vStart + ((srcStoreWidth * srcStoreHeight) >> 2);
		...
		else
		{
			...
			else
			{
				...
				while (i-- > j)
				{
					...
					stats[i].yPtr = srcPtr + srcStoreWidth * currHeight;
					stats[i].yBpl = srcStoreWidth << 1;
					stats[i].vPtr = srcPtr + srcStoreWidth * srcStoreHeight + ((srcStoreWidth >> 1) * (currHeight >> 1));
				...
				while (i-- > 0)
				{
					...
					stats[i].yPtr = srcPtr + srcStoreWidth * currHeight;
					stats[i].yBpl = srcStoreWidth << 1;
					stats[i].vPtr = srcPtr + srcStoreWidth * srcStoreHeight + ((srcStoreWidth >> 1) * ((currHeight >> 1) + 1));
					stats[i].uPtr = stats[i].vPtr + ((srcStoreWidth * srcStoreHeight) >> 2);
					stats[i].uvBpl = srcStoreWidth;
					stats[i].dest = destPtr + destRGBBpl * (OSInt)currHeight;
					stats[i].isFirst = isFirst;
...
//			OSInt yvSize = srcStoreWidth * dispHeight;
/*			if (yvSize > yvBuffSize)
			{
				yvBuffSize = yvSize;
				if (this->uBuff)
				{
				...
				this->uBuff = MemAllocA64(UInt8, yvSize);
				this->vBuff = MemAllocA64(UInt8, yvSize);
			}*/
...
/*			while (i-- > 0)
			{
				currHeight = MulDiv(i, height, this->nThread) & ~1;
				stats[i].uPtr = vStart;
				stats[i].vPtr = this->vBuff + (srcStoreWidth * currHeight);
				stats[i].width = srcStoreWidth >> 1;
				stats[i].height = (lastHeight - currHeight);
				stats[i].uvBpl = currHeight;
				stats[i].dbpl = srcStoreWidth;
				stats[i].yvParam = &this->yvParamO;
				stats[i].status = 7;
				stats[i].evt->Set();
				lastHeight = currHeight;
			}
			this->WaitForWorker(7);
			i = this->nThread;
			lastHeight = height;
			while (i-- > 0)
			{
				currHeight = MulDiv(i, height, this->nThread) & ~1;
				stats[i].uPtr = uStart;
				stats[i].vPtr = this->uBuff + (srcStoreWidth * currHeight);
				stats[i].width = srcStoreWidth >> 1;
				stats[i].height = (lastHeight - currHeight);
				stats[i].uvBpl = currHeight;
				stats[i].dbpl = srcStoreWidth;
			...
			while (i-- > 0)
			{
				...
				stats[i].vPtr = this->vBuff + (srcStoreWidth * currHeight);
				stats[i].uPtr = this->uBuff + (srcStoreWidth * currHeight);
				stats[i].uvBpl = srcStoreWidth;
				...
				if (stats[i].csLineSize < width)
				{
					if (stats[i].csLineBuff)
						MemFreeA64(stats[i].csLineBuff);
					if (stats[i].csLineBuff2)
						MemFreeA64(stats[i].csLineBuff2);
					stats[i].csLineSize = width;
					stats[i].csLineBuff = MemAllocA64(UInt8, (cSize << 1));
					stats[i].csLineBuff2 = MemAllocA64(UInt8, (cSize << 1));
				}
				stats[i].status = 8;
		...
		else
		{
			while (i-- > 0)
			{
				...
				stats[i].yPtr = srcPtr + srcStoreWidth * currHeight;
				stats[i].yBpl = srcStoreWidth;
				stats[i].vPtr = srcPtr + srcStoreWidth * srcStoreHeight + ((srcStoreWidth * currHeight) >> 2);
				stats[i].uPtr = stats[i].vPtr + ((srcStoreWidth * srcStoreHeight) >> 2);
