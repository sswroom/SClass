...
Media::CS::CSBI42_LRGBC::CSBI42_LRGBC(NN<const Media::ColorProfile> srcProfile, NN<const Media::ColorProfile> destProfile, Media::ColorProfile::YUVType yuvType, Optional<Media::ColorManagerSess> colorSess) : Media::CS::CSYUV420_LRGBC(srcProfile, destProfile, yuvType, colorSess)
{
}

Media::CS::CSBI42_LRGBC::~CSBI42_LRGBC()
{
}

void Media::CS::CSBI42_LRGBC::ConvertV2(UnsafeArray<const UnsafeArray<UInt8>> srcPtr, UnsafeArray<UInt8> destPtr, UIntOS dispWidth, UIntOS dispHeight, UIntOS srcStoreWidth, UIntOS srcStoreHeight, IntOS destRGBBpl, Media::FrameType ftype, Media::YCOffset ycOfst)
{
	this->UpdateTable();
	UInt32 isLast = 1;
	UInt32 isFirst = 0;
	...
	if (srcStoreWidth & 31)
	{
		srcStoreWidth = srcStoreWidth + 32 - (srcStoreWidth & 31);
	}
	if (srcStoreHeight & 15)
	{
		srcStoreHeight = srcStoreHeight + 16 - (srcStoreHeight & 15);
	}
