...
void Media::CS::CSYUV10_RGB8::SetupYUV_RGB13()
{
	...
	if (updateY)
	{
		...
		while (i--)
		{
			y = (long)(0x7fff * (Math::Pow(sumy[i] * currSum, this->yuvParam.YGamma) * this->yuvParam.Contrast + this->yuvParam.Brightness - 1) / 4.0);
			if (y < -0x8000)
				y = 0x8000;
			else if (y > 0x7fff)
				y = 0x7fff;
...
//		Bool fullRange = (this->yuvType & Media::ColorProfile::YUVT_FLAG_YUV_0_255) != 0;
		...
		while (i--)
		{
			...
			if (i > 512)
				c = Math::Pow(sumc[i - 512] * currSum, this->yuvParam.CGamma) * 16383;
			else if (i == 512)
				c = 0;
			else
				c = Math::Pow(sumc[-i + 512] * currSum, this->yuvParam.CGamma) * (-16383);
