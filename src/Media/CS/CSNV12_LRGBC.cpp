...
extern "C"
{
	void CSNV12_LRGBC_VerticalFilterLRGB(UInt8 *inYPt, UInt8 *inUVPt, UInt8 *outPt, UIntOS width, UIntOS height, UIntOS tap, IntOS *index, Int64 *weight, UIntOS isFirst, UIntOS isLast, UInt8 *csLineBuff, UInt8 *csLineBuff2, UIntOS ystep, IntOS dstep, Int64 *yuv2rgb, Int64 *rgbGammaCorr);
	void CSNV12_LRGBC_do_yv12rgb8(UInt8 *yPtr, UInt8 *uvPtr, UInt8 *dest, UIntOS width, UIntOS height, IntOS dbpl, UIntOS isFirst, UIntOS isLast, UInt8 *csLineBuff, UInt8 *csLineBuff2, UIntOS yBpl, UIntOS uvBpl, Int64 *yuv2rgb, Int64 *rgbGammaCorr);
	void CSNV12_LRGBC_do_yv12rgb2(UInt8 *yPtr, UInt8 *uvPtr, UInt8 *dest, UIntOS width, UIntOS height, IntOS dbpl, UIntOS isFirst, UIntOS isLast, UInt8 *csLineBuff, UInt8 *csLineBuff2, UIntOS yBpl, UIntOS uvBpl, Int64 *yuv2rgb, Int64 *rgbGammaCorr);
}
...
void Media::CS::CSNV12_LRGBC::SetupInterpolationParameter(UIntOS source_length, UIntOS result_length, NN<YVPARAMETER> out, UIntOS indexSep, Double offsetCorr)
{
	UIntOS i;
	UIntOS j;
	IntOS n;
...
UInt32 Media::CS::CSNV12_LRGBC::WorkerThread(AnyType obj)
{
	...
	while (true)
	{
		...
		{
#if LANCZOS_NTAP == 4
			CSNV12_LRGBC_VerticalFilterLRGB(ts->yPtr.Ptr(), ts->uvPtr.Ptr(), ts->dest.Ptr(), ts->width, ts->height, ts->yvParam->tap, ts->yvParam->index.Ptr() + ts->uvBpl, ts->yvParam->weight.Ptr() + ts->uvBpl * 6, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, ts->yBpl, ts->dbpl, converter->yuv2rgb14.Ptr(), converter->rgbGammaCorr.Ptr());
#else
			CSNV12_LRGBC_VerticalFilterLRGB(ts->yPtr.Ptr(), ts->uvPtr.Ptr(), ts->dest.Ptr(), ts->width, ts->height, ts->yvParam->tap, ts->yvParam->index.Ptr() + ts->uvBpl * LANCZOS_NTAP, ts->yvParam->weight.Ptr() + ts->uvBpl * LANCZOS_NTAP, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, ts->yBpl, ts->dbpl, converter->yuv2rgb14.Ptr(), converter->rgbGammaCorr.Ptr());
#endif
			ts->status = 4;
			converter->evtMain.Set();
		}
...
void Media::CS::CSNV12_LRGBC::ConvertV2(UnsafeArray<const UnsafeArray<UInt8>> srcPtr2, UnsafeArray<UInt8> destPtr, UIntOS dispWidth, UIntOS dispHeight, UIntOS srcStoreWidth, UIntOS srcStoreHeight, IntOS destRGBBpl, Media::FrameType ftype, Media::YCOffset ycOfst)
{
	this->UpdateTable();
	UInt32 isLast = 1;
	UInt32 isFirst = 0;
...
UIntOS Media::CS::CSNV12_LRGBC::GetSrcFrameSize(UIntOS width, UIntOS height)
{
