...
UInt32 Media::CS::CSUYVY_LRGB::WorkerThread(void *obj)
{
	CSUYVY_LRGB *converter = (CSUYVY_LRGB*)obj;
	...
	while (true)
	{
		...
		{
			CSUYVY_LRGB_do_uyvy2rgb(ts->yPtr, ts->dest, ts->width, ts->height, ts->dbpl, converter->yuv2rgb, converter->rgbGammaCorr);
			ts->status = 4;
			converter->evtMain->Set();
		}
	}
...
Media::CS::CSUYVY_LRGB::CSUYVY_LRGB(Media::ColorProfile *srcColor, Media::ColorProfile::YUVType yuvType, Media::ColorManagerSess *colorSess) : Media::CS::CSYUV_LRGB(srcColor, yuvType, colorSess)
{
	UOSInt i;
	this->nThread = Sync::Thread::GetThreadCnt();

	NEW_CLASS(evtMain, Sync::Event((const UTF8Char*)"Media.CS.CSUYVY_RGB8.evtMain"));
	stats = MemAlloc(THREADSTAT, nThread);
	i = nThread;
	while(i-- > 0)
	{
		NEW_CLASS(stats[i].evt, Sync::Event((const UTF8Char*)"Media.CS.CSUYVY_RGB8.stats.evt"));
		stats[i].status = 0;
...
void Media::CS::CSUYVY_LRGB::ConvertV2(UInt8 **srcPtr, UInt8 *destPtr, UOSInt dispWidth, UOSInt dispHeight, UOSInt srcStoreWidth, UOSInt srcStoreHeight, OSInt destRGBBpl, Media::FrameType ftype, Media::YCOffset ycOfst)
{
	this->UpdateTable();
