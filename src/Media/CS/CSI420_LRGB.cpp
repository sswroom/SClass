...
UInt32 Media::CS::CSI420_LRGB::WorkerThread(void *obj)
{
	...
	while (true)
	{
		...
		{
			CSI420_LRGB_do_yv12rgb(ts->yPtr, ts->uPtr, ts->vPtr, ts->dest, ts->width, ts->height, ts->dbpl, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, converter->yuv2rgb, converter->rgbGammaCorr);
			ts->status = 4;
			converter->evtMain->Set();
		}
	}
...
Media::CS::CSI420_LRGB::CSI420_LRGB(const Media::ColorProfile *srcColor, Media::ColorProfile::YUVType yuvType, Media::ColorManagerSess *colorSess) : Media::CS::CSYUV_LRGB(srcColor, yuvType, colorSess)
{
	...
	NEW_CLASS(evtMain, Sync::Event((const UTF8Char*)"Media.CS.CSI420_LRGB.evtMain"));
	stats = MemAlloc(THREADSTAT, nThread);
	i = nThread;
	while(i-- > 0)
	{
		NEW_CLASS(stats[i].evt, Sync::Event((const UTF8Char*)"Media.CS.CSI420_LRGB.stats.evt"));
		stats[i].status = 0;
...
///////////////////////////////////////////////////////
void Media::CS::CSI420_LRGB::ConvertV2(UInt8 **srcPtr, UInt8 *destPtr, UOSInt dispWidth, UOSInt dispHeight, UOSInt srcStoreWidth, UOSInt srcStoreHeight, OSInt destRGBBpl, Media::FrameType ftype, Media::YCOffset ycOfst)
{
	this->UpdateTable();
	UOSInt isLast = 1;
	UOSInt isFirst = 0;
...
UOSInt Media::CS::CSI420_LRGB::GetSrcFrameSize(UOSInt width, UOSInt height)
{
