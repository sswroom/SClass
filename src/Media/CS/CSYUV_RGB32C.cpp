...
void Media::CS::CSYUV_RGB32C::SetupRGB13_LR()
{
	Int32 i;
	Double thisV;
	Double cV;
...
void Media::CS::CSYUV_RGB32C::SetupYUV_RGB13()
{
	Double c;
	...
	if (fullRange)
	{
		...
		while (i--)
		{
			y = Math::Double2Int32(0x7fff * (Math::Pow(i / 255.0, this->yuvParam.YGamma) * this->yuvParam.Contrast + this->yuvParam.Brightness - 1) / 4.0);
			if (y < -0x8000)
				y = 0x8000;
			else if (y > 0x7fff)
				y = 0x7fff;
	...
	while (i--)
	{
		...
		{
			if (i > 128)
				c = Math::Pow((i - 128) / 127.0, this->yuvParam.CGamma) * 16383;
			else if (i == 128)
				c = 0;
			else
				c = Math::Pow((-i + 128) / 127.0, this->yuvParam.CGamma) * (-16383);
...
void Media::CS::CSYUV_RGB32C::SetupYUV14_RGB13()
{
	Double c;
	...
	while (i--)
	{
		...
		{
			if (i >= 32768)
				c = Math::Pow((-i + 8192 + 65536) / 8160.0, this->yuvParam.CGamma) * (-16383);
			else if (i > 8192)
				c = Math::Pow((i - 8192) / 8160.0, this->yuvParam.CGamma) * 16383;
			else if (i == 8192)
				c = 0;
			else
				c = Math::Pow((-i + 8192) / 8160.0, this->yuvParam.CGamma) * (-16383);
...
Media::CS::CSYUV_RGB32C::CSYUV_RGB32C(const Media::ColorProfile *srcProfile, const Media::ColorProfile *destProfile, Media::ColorProfile::YUVType yuvType, Media::ColorManagerSess *colorSess, Media::PixelFormat destPF) : Media::CS::CSConverter(colorSess)
{
...
void Media::CS::CSYUV_RGB32C::YUVParamChanged(const Media::IColorHandler::YUVPARAM *yuv)
{
	MemCopyNO(&this->yuvParam, yuv, sizeof(YUVPARAM));
	this->yuvUpdated = true;
}

void Media::CS::CSYUV_RGB32C::RGBParamChanged(const Media::IColorHandler::RGBPARAM2 *rgb)
{
	this->rgbParam->Set(rgb);
	this->rgbUpdated = true;
}

UOSInt Media::CS::CSYUV_RGB32C::GetDestFrameSize(UOSInt width, UOSInt height)
{
