...
void Media::CS::CSNV12_RGB32C::SetupInterpolationParameter(UOSInt source_length, UOSInt result_length, YVPARAMETER *out, UOSInt indexSep, Double offsetCorr)
{
	UOSInt i;
	UOSInt j;
	OSInt n;
...
UInt32 Media::CS::CSNV12_RGB32C::WorkerThread(void *obj)
{
	CSNV12_RGB32C *converter = (CSNV12_RGB32C*)obj;
	...
	while (true)
	{
		...
		else if (ts->status == 3)
		{
			if (ts->width & 3)
			{
				CSNV12_RGB32C_do_yv12rgb2(ts->yPtr, ts->uvPtr, ts->dest, ts->width, ts->height, ts->dbpl, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, ts->yBpl, ts->uvBpl, converter->yuv2rgb, converter->rgbGammaCorr);
			}
			else
			{
				CSNV12_RGB32C_do_yv12rgb8(ts->yPtr, ts->uvPtr, ts->dest, ts->width, ts->height, ts->dbpl, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, ts->yBpl, ts->uvBpl, converter->yuv2rgb, converter->rgbGammaCorr);
			}
			ts->status = 4;
			converter->evtMain->Set();
		}
		...
		{
#if LANCZOS_NTAP == 4
			CSNV12_RGB32C_VerticalFilterLRGB(ts->yPtr, ts->uvPtr, ts->dest, ts->width, ts->height, ts->yvParam->tap, ts->yvParam->index + ts->uvBpl, ts->yvParam->weight + ts->uvBpl * 6, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, ts->yBpl, ts->dbpl, converter->yuv2rgb14, converter->rgbGammaCorr);
#else
			CSNV12_RGB32C_VerticalFilterLRGB(ts->yPtr, ts->uvPtr, ts->dest, ts->width, ts->height, ts->yvParam->tap, ts->yvParam->index + ts->uvBpl * LANCZOS_NTAP, ts->yvParam->weight + ts->uvBpl * LANCZOS_NTAP, ts->isFirst, ts->isLast, ts->csLineBuff, ts->csLineBuff2, ts->yBpl, ts->dbpl, converter->yuv2rgb14, converter->rgbGammaCorr);
#endif
			ts->status = 4;
			converter->evtMain->Set();
		}
...
Media::CS::CSNV12_RGB32C::CSNV12_RGB32C(const Media::ColorProfile *srcProfile, const Media::ColorProfile *destProfile, Media::ColorProfile::YUVType yuvType, Media::ColorManagerSess *colorSess, Media::PixelFormat destPF) : Media::CS::CSYUV_RGB32C(srcProfile, destProfile, yuvType, colorSess, destPF)
{
	UOSInt i;
	...
	NEW_CLASS(evtMain, Sync::Event((const UTF8Char*)"Media.CS.CSNV12_RGB32C.evtMain"));
	stats = MemAlloc(THREADSTAT, nThread);
	i = nThread;
	while(i-- > 0)
	{
		NEW_CLASS(stats[i].evt, Sync::Event((const UTF8Char*)"Media.CS.CSNV12_RGB32C.stats.evt"));
		stats[i].status = 0;
