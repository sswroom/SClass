...
extern "C" void CSYUY2_RGB8_do_yuy2rgb(UInt8 *src, UInt8 *dest, OSInt width, OSInt height, OSInt dbpl, Int64 *yuv2rgb, UInt8 *rgbGammaCorr)
{
	...
	while (height-- > 0)
	{
		cval = vqadd_s16(*(int16x4_t*)&yuv2rgb[src[1] + 256], *(int16x4_t*)&yuv2rgb[src[3] + 512]);
		rgbval = vqadd_s16(*(int16x4_t*)&yuv2rgb[src[0]], cval);
		...
		dest[3] = 0xff;
		rgbval = vqadd_s16(*(int16x4_t*)&yuv2rgb[src[2]], cval);
		src += 4;
		dest += 8;
		...
		{
			cval = vqadd_s16(*(int16x4_t*)&yuv2rgb[src[1] + 256], *(int16x4_t*)&yuv2rgb[src[3] + 512]);
			rgbval2 = vqadd_s16(*(int16x4_t*)&yuv2rgb[src[0]], cval);
			cval = vshr_n_s16(cval, 1);
		...
		dest[2] = rgbGammaCorr[((UInt16)vget_lane_s16(rgbval2, 2))];
		dest[3] = 0xff;
		rgbval2 = vqadd_s16(*(int16x4_t*)&yuv2rgb[src[2]], cval);
