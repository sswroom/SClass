...
extern "C" void AudioUtil_Amplify16(UInt8 *audioBuff, UIntOS buffSize, Int32 vol)
{
	...
	while (buffSize > 0)
	{
		v = (ReadLInt16(audioBuff) * (Int64)vol) >> 16;
		if (v < -32768)
			WriteLInt16(audioBuff, -32768);
		else if (v > 32767)
			WriteLInt16(audioBuff, 32767);
		else
			WriteLInt16(audioBuff, v);
		audioBuff += 2;
		buffSize -= 2;
	}
}
...
extern "C" void AudioUtil_ConvI16_F32(const UInt8 *srcBuff, UInt8 *destBuff, UIntOS nSample)
{
	...
	while (nSample-- > 0)
	{
		WriteLFloat(destBuff, (Single)(ReadLInt16(srcBuff) * mul));
		srcBuff += 2;
		destBuff += 4;
	}
}
...
extern "C" void AudioUtil_ConvF32_I16(const UInt8 *srcBuff, UInt8 *destBuff, UIntOS nSample)
{
	while (nSample-- > 0)
	{
		WriteLInt16(destBuff, Math::SDouble2Int16(ReadLFloat(srcBuff) * 32767.0));
		srcBuff += 4;
		destBuff += 2;
	}
}
