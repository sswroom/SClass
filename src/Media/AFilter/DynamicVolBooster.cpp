...
void Media::AFilter::DynamicVolBooster::GetFormat(NN<AudioFormat> format)
{
	Media::AudioFormat fmt;
...
Data::Duration Media::AFilter::DynamicVolBooster::SeekToTime(Data::Duration time)
{
	this->ResetStatus();
...
void Media::AFilter::DynamicVolBooster::SetEnabled(Bool enabled)
{
	Sync::MutexUsage mutUsage(this->mut);
...
void Media::AFilter::DynamicVolBooster::SetBGLevel(Double bgLevel)
{
	if (bgLevel > 0)
	{
		Sync::MutexUsage mutUsage(this->mut);
