#include "Stdafx.h"
#include "MyMemory.h"
#include "Core/ByteTool_C.h"
#include "Media/AFilter/AudioCaptureFilter.h"
#include "Sync/MutexUsage.h"
#include "Sync/SimpleThread.h"
#include "Sync/ThreadUtil.h"
#define BUFFSIZE 1048576

UInt32 __stdcall Media::AFilter::AudioCaptureFilter::CaptureThread(AnyType userObj)
{
	NN<Media::AFilter::AudioCaptureFilter> me = userObj.GetNN<Media::AFilter::AudioCaptureFilter>();
	UInt8 *tmpBuff;
	UIntOS buffSize;
	NN<IO::FileStream> waveStm;
	me->running = true;
	while (!me->toStop)
	{
		Sync::MutexUsage writeMutUsage(me->writeMut);
		while (me->readBuffSize > 0)
		{
			Sync::MutexUsage readMutUsage(me->readMut);
			tmpBuff = me->writeBuff;
			me->writeBuff = me->readBuff;
			me->readBuff = tmpBuff;
			buffSize = me->readBuffSize;
			me->readBuffSize = 0;
			readMutUsage.EndUse();

			if (me->waveStm.SetTo(waveStm))
			{
				waveStm->Write(Data::ByteArrayR(me->writeBuff, buffSize));
				me->dataSize += buffSize;
				me->fileSize += buffSize;
			}
		}
		writeMutUsage.EndUse();
		
		me->evt.Wait(1000);
	}
	me->running = false;
	return 0;
}

Media::AFilter::AudioCaptureFilter::AudioCaptureFilter(NN<Media::AudioSource> sourceAudio) : Media::AudioFilter(sourceAudio)
{
	this->waveStm = nullptr;
	this->readBuff = MemAlloc(UInt8, BUFFSIZE);
	this->writeBuff = MemAlloc(UInt8, BUFFSIZE);
	this->readBuffSize = 0;
	this->writing = false;
	this->running = false;
	this->toStop = false;
	Sync::ThreadUtil::Create(CaptureThread, this);
}

Media::AFilter::AudioCaptureFilter::~AudioCaptureFilter()
{
	StopCapture();
	this->toStop = true;
	this->evt.Set();
	while (this->running)
	{
		Sync::SimpleThread::Sleep(10);
	}
	MemFree(this->readBuff);
	MemFree(this->writeBuff);
}

Data::Duration Media::AFilter::AudioCaptureFilter::SeekToTime(Data::Duration time)
{
	return this->sourceAudio->SeekToTime(time);
}

UIntOS Media::AFilter::AudioCaptureFilter::ReadBlock(Data::ByteArray blk)
{
	UIntOS readSize = this->sourceAudio->ReadBlock(blk);
	Sync::MutexUsage mutUsage(this->readMut);
	if (this->writing)
	{
		if (this->readBuffSize >= BUFFSIZE)
		{
		}
		else if (this->readBuffSize + blk.GetSize() > BUFFSIZE)
		{
			MemCopyNO(&this->readBuff[this->readBuffSize], blk.Arr().Ptr(), BUFFSIZE - this->readBuffSize);
			this->readBuffSize = BUFFSIZE;
			this->evt.Set();
		}
		else
		{
			MemCopyNO(&this->readBuff[this->readBuffSize], blk.Arr().Ptr(), blk.GetSize());
			this->readBuffSize += blk.GetSize();
			this->evt.Set();
		}
	}
	return readSize;
}

Bool Media::AFilter::AudioCaptureFilter::StartCapture(Text::CStringNN fileName)
{
	this->StopCapture();
	Media::AudioFormat format;
	this->sourceAudio->GetFormat(format);
	UInt8 buff[74];
	*(Int32*)&buff[0] = *(Int32*)"RIFF";
	*(UInt32*)&buff[4] = 0;
	*(Int32*)&buff[8] = *(Int32*)"WAVE";
	*(Int32*)&buff[12] = *(Int32*)"JUNK";
	*(UInt32*)&buff[16] = 28;
	MemClear(&buff[20], 28);
	*(Int32*)&buff[48] = *(Int32*)"fmt ";
	WriteLUInt32(&buff[52], 18 + format.extraSize);
	WriteLUInt16(&buff[56], (UInt16)format.formatId);
	WriteLUInt16(&buff[58], format.nChannels);
	WriteLUInt32(&buff[60], format.frequency);
	WriteLUInt32(&buff[64], format.bitRate >> 3);
	WriteLUInt16(&buff[68], (UInt16)format.align);
	WriteLUInt16(&buff[70], format.bitpersample);
	WriteLUInt16(&buff[72], (UInt16)format.extraSize);
	Sync::MutexUsage mutUsage(this->writeMut);
	NN<IO::FileStream> waveStm;
	NEW_CLASSNN(waveStm, IO::FileStream(fileName, IO::FileMode::Create, IO::FileShare::DenyNone, IO::FileStream::BufferType::Normal));
	this->waveStm = waveStm;
	waveStm->Write(Data::ByteArrayR(buff, 74));
	if (format.extraSize > 0)
	{
		this->dataOfst = 78 + format.extraSize;
		waveStm->Write(Data::ByteArrayR(format.extra, format.extraSize));
	}
	else
	{
		this->dataOfst = 78;
	}
	*(Int32*)&buff[0] = *(Int32*)"data";
	*(Int32*)&buff[4] = 0;
	waveStm->Write(Data::ByteArrayR(buff, 8));
	this->dataSize = 0;
	this->fileSize = this->dataOfst - 4;
	this->writing = true;
	return true;
}

void Media::AFilter::AudioCaptureFilter::StopCapture()
{
	this->writing = false;
	Sync::MutexUsage mutUsage(this->writeMut);
	NN<IO::FileStream> waveStm;
	if (this->waveStm.SetTo(waveStm))
	{
		if (this->fileSize >= 0x100000000LL)
		{
			UInt8 buff[48];
			*(Int32*)&buff[0] = *(Int32*)"RF64";
			*(Int32*)&buff[4] = -1;
			*(Int32*)&buff[12] = *(Int32*)"ds64";
			WriteLUInt64(&buff[20], this->fileSize);
			WriteLUInt64(&buff[28], this->dataSize);
			*(Int64*)&buff[36] = 0;
			*(Int32*)&buff[44] = 0;
			waveStm->SeekFromBeginning(0);
			waveStm->Write(Data::ByteArrayR(buff, 48));

			waveStm->SeekFromBeginning(dataOfst);
			waveStm->Write(Data::ByteArrayR(&buff[4], 4));
		}
		else
		{
			waveStm->SeekFromBeginning(4);
			waveStm->Write(Data::ByteArrayR((const UInt8*)&this->fileSize, 4));
			waveStm->SeekFromBeginning(dataOfst);
			waveStm->Write(Data::ByteArrayR((const UInt8*)&this->dataSize, 4));
		}
		this->waveStm.Delete();
	}
}
