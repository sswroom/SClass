...
{
	cv::Mat *fr = (cv::Mat *)this->frame;
	UIntOS w = this->GetWidth();
	UIntOS h = this->GetHeight();
	IntOS bpl = this->GetBpl();
	IntOS lineAdd = bpl - (IntOS)w;
	UInt8 *ptr = fr->data;
	UIntOS i = h;
	UIntOS j;
	while (i-- > 0)
