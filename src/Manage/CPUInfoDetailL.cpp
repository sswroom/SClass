...
Manage::CPUInfoDetail::CPUInfoDetail()
{
	...
	if (!fs->IsError())
	{
		...
		if (this->cpuModel == 0)
		{
			...
			else if (IO::Path::GetPathType((const UTF8Char*)"/sys/rk3368_thermal") == IO::Path::PT_DIRECTORY)
			{
				this->cpuModel = (const UTF8Char*)"RK3368";
			}
			else if (IO::Path::GetPathType((const UTF8Char*)"/sys/devices/platform/rk3026-codec") == IO::Path::PT_DIRECTORY)
			{
				this->cpuModel = (const UTF8Char*)"RK3026";
			}
			else if (IO::Path::GetPathType((const UTF8Char*)"/sys/bus/platform/drivers/bcm2835_thermal") == IO::Path::PT_DIRECTORY)
			{
				if (cpuPart == 0xd03)
				{
			...
			else if (IO::Path::GetPathType((const UTF8Char*)"/sys/bus/platform/drivers/rtk129x-cpufreq") == IO::Path::PT_DIRECTORY)
			{
				...
				{
					if (IO::Path::GetPathType((const UTF8Char*)"/sys/bus/platform/drivers/[RTD129x PCIE Slot2]") == IO::Path::PT_UNKNOWN)
					{
						this->cpuModel = (const UTF8Char*)"RTD1294";
					}
					else if (IO::Path::GetPathType((const UTF8Char*)"/sys/bus/platform/devices/9803f000.sata:sata-port@2") == IO::Path::PT_UNKNOWN)
					{
			...
			else if (this->brand == Manage::CPUVendor::CB_TI)
			{
				...
				if (!fs2->IsError())
				{
					...
					{
						Bool hasCAN = (IO::Path::GetPathType((const UTF8Char*)"/proc/net/can") == IO::Path::PT_DIRECTORY);
...
//						Bool hasEtherCAT = false;
			...
			else if (this->brand == Manage::CPUVendor::CB_AMLOGIC)
			{
				...
				{
					if (IO::Path::GetPathType((const UTF8Char*)"/sys/class/amaudio") != IO::Path::PT_DIRECTORY)
					{
						this->cpuModel = (const UTF8Char*)"Amlogic S905";
					}
					else if (IO::Path::GetPathType((const UTF8Char*)"/sys/class/tsdemux") != IO::Path::PT_DIRECTORY)
					{
			...
			{
				if (IO::Path::GetPathType((const UTF8Char*)"/sys/bus/platform/drivers/cpufreq-ipq40xx") == IO::Path::PT_DIRECTORY)
				{
					Bool hasSDCard = (IO::Path::GetPathType((const UTF8Char*)"/sys/class/mmc_host") == IO::Path::PT_DIRECTORY);
