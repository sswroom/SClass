...
Text::CStringNN IO::ED538Sim::GetName()
{
	return CSTR("ED538");
}

UIntOS IO::ED538Sim::GetValueCount()
{
	return 20;
}

Text::CString IO::ED538Sim::GetValueName(UIntOS index)
{
	switch (index)
	{
	case 0:
...
Bool IO::ED538Sim::GetValue(UIntOS index, NN<Text::StringBuilderUTF8> sb)
{
	if (index < 8)
	{
...
Bool IO::ED538Sim::ToggleValue(UIntOS index)
{
	if (index < 8)
	{
...
Bool IO::ED538Sim::ReadCoil(UInt16 coilAddr, OutParam<Bool> value)
{
	if (coilAddr < 4)
	{
...
Bool IO::ED538Sim::ReadDescreteInput(UInt16 inputAddr, OutParam<Bool> value)
{
	if (inputAddr < 8)
	{
...
Bool IO::ED538Sim::ReadHoldingRegister(UInt16 regAddr, OutParam<UInt16> value)
{
	if (regAddr < 8)
	{
...
Bool IO::ED538Sim::ReadInputRegister(UInt16 regAddr, OutParam<UInt16> value)
{
	if (regAddr < 8)
	{
		value.Set(this->diCounts[regAddr]);
