...
IO::PcapngWriter::PcapngWriter(Text::CStringNN fileName, IO::PacketAnalyse::LinkType linkType, Text::CStringNN appName) : fs(fileName, IO::FileMode::Create, IO::FileShare::DenyNone, IO::FileStream::BufferType::Normal)
{
	...
	WriteLUInt32(&buff[0], 0x0a0d0d0a); //block type = SHB
	WriteLInt32(&buff[4], 2); //block length
	WriteLInt32(&buff[8], 0x1A2B3C4D); //byte order magic
	WriteLInt16(&buff[12], 1); //major version
	WriteLInt16(&buff[14], 0); //minor version
	WriteLInt64(&buff[16], -1); //section length
	...
	{
		i = sptr - buff - 28;
		WriteLInt16(&buff[24], 2);
		WriteLInt16(&buff[26], i);
		if (i & 3)
		{
			i = 4 - (i & 3);
			sptr += i;
	...
	*sptr2++ = ' ';
	...
	i = sptr2 - sptr - 4;
	WriteLInt16(&sptr[0], 3);
	WriteLInt16(&sptr[2], i);
	sptr = sptr2;
	if (i & 3)
	{
		i = 4 - (i & 3);
	...
	WriteLInt32(&sptr[0], 0);
	sptr += 4;
	i = sptr - buff + 4;
	WriteLInt32(&sptr[0], (Int32)i);
	WriteLInt32(&buff[4], (Int32)i);

	this->fs.Write(Data::ByteArrayR(buff, (UIntOS)i));

	WriteLInt32(&buff[0], 1); //Block Type = IDB
	WriteLInt32(&buff[4], 0); //Block Size
	WriteLInt32(&buff[8], (Int32)linkType); //Link Type + Reserved
	WriteLInt32(&buff[12], 65536); //snap len
	WriteLInt16(&buff[16], 2);
	WriteLInt16(&buff[18], 3);
	Text::StrConcatC(&buff[20], UTF8STRC("any"));
	WriteLInt16(&buff[24], 9);
	WriteLInt16(&buff[26], 1);
	WriteLInt32(&buff[28], 9);
	sptr = &buff[32];
	sptr2 = IO::OS::GetDistro(sptr + 4).Or(sptr + 4);
	*sptr2++ = ' ';
	sptr2 = IO::OS::GetVersion(sptr2).Or(sptr2);
...
Bool IO::PcapngWriter::WritePacket(Data::ByteArrayR packet)
{
	...
	WriteLInt32(&buff[0], 6);
	WriteLInt32(&buff[4], (Int32)pSize);
	WriteLInt32(&buff[8], 0);
	WriteLInt32(&buff[12], (Int32)(t >> 32));
	WriteLInt32(&buff[16], (Int32)(t & 0xffffffff));
	WriteLInt32(&buff[20], (Int32)packet.GetSize());
	WriteLInt32(&buff[24], (Int32)packet.GetSize());
