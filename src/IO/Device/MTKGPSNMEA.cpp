...
void IO::Device::MTKGPSNMEA::ParseUnknownCmd(UnsafeArray<const UTF8Char> cmd, UIntOS cmdLen)
{
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK101"), buff);
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK102"), buff);
	this->stm->Write(Data::ByteArrayR(buff, cmdSize));
}
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK103"), buff);
	this->stm->Write(Data::ByteArrayR(buff, cmdSize));
}
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK104"), buff);
	this->stm->Write(Data::ByteArrayR(buff, cmdSize));
}
...
Bool IO::Device::MTKGPSNMEA::IsMTKDevice()
{
	UInt8 buff[64];
	Bool succ = false;
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK000"), buff);
...
Bool IO::Device::MTKGPSNMEA::QueryFirmware()
{
	...
	UIntOS i;
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK605"), buff);
	NN<Text::String> result;
	...
	result->Release();
	Text::StrSplitP(sarr2, 2, {sbuff, (UIntOS)(sptr - sbuff)}, '*');
...
Bool IO::Device::MTKGPSNMEA::IsLogEnabled()
{
	UInt8 buff[64];
	UnsafeArray<UTF8Char> sptr;
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,2,7"), buff);
	UTF8Char sbuff[128];
	NN<Text::String> result;
	if (!SendMTKCommand(buff, cmdSize, UTF8STRC("$PMTK182,3,7,"), 2000).SetTo(result))
		return false;
	sptr = result->ConcatTo(sbuff);
	UIntOS i = Text::StrIndexOfCharC(sbuff, (UIntOS)(sptr - sbuff), '*');
	if (i == INVALID_INDEX)
	{
		result->Release();
		return false;
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,5"), buff);
	NN<Text::String> result;
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,4"), buff);
	NN<Text::String> result;
...
UIntOS IO::Device::MTKGPSNMEA::CalLogBlockCount(UIntOS logSize)
{
	if (logSize & 65535)
	{
		return (logSize >> 16) + 1;
...
Bool IO::Device::MTKGPSNMEA::ReadLogPart(UIntOS addr, UnsafeArray<UInt8> buff)
{
	UTF8Char sbuff[64];
	UnsafeArray<UTF8Char> sptr;
	UIntOS i;
	...
	i = GenNMEACommand(sbuff, (UIntOS)(sptr - sbuff), cbuff);
...
Bool IO::Device::MTKGPSNMEA::ReadLogBlock(UIntOS addr, UnsafeArray<UInt8> buff)
{
	UIntOS retryCnt = 0;
	UIntOS ofst = 0;
	Bool succ;
...
Bool IO::Device::MTKGPSNMEA::ParseLog(NN<Map::GPSTrack> gps)
{
	...
	this->DisableLog();
	UIntOS logSize = this->GetLogSize();
...
{
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,6,1"), buff);
	NN<Text::String> result;
...
Bool IO::Device::MTKGPSNMEA::SetLogFormat(LogFormat lf)
{
	...
	UIntOS cmdSize = GenNMEACommand(sbuff, (UIntOS)(sptr - sbuff), buff);
	NN<Text::String> result;
...
Bool IO::Device::MTKGPSNMEA::SetLogMode(LogMode lm)
{
	...
	UIntOS cmdSize = GenNMEACommand(cmd.v, cmd.leng, buff);
	NN<Text::String> result;
	if (!SendMTKCommand(buff, cmdSize, UTF8STRC("$PMTK001,182,1,3"), 2000).SetTo(result))
		return false;
	result->Release();
...
IO::Device::MTKGPSNMEA::LogFormat IO::Device::MTKGPSNMEA::GetLogFormat()
{
	...
	UnsafeArray<UTF8Char> sptr;
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,2,2"), buff);
	NN<Text::String> result;
...
UInt32 IO::Device::MTKGPSNMEA::GetLogDistance()
{
	UTF8Char sbuff[128];
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,2,4"), buff);
	NN<Text::String> result;
...
IO::Device::MTKGPSNMEA::LogMode IO::Device::MTKGPSNMEA::GetLogMode()
{
	...
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,2,6"), buff);
	NN<Text::String> result;
...
UIntOS IO::Device::MTKGPSNMEA::GetLogSize()
{
	UTF8Char sbuff[128];
	UInt8 buff[64];
	UIntOS cmdSize = GenNMEACommand(UTF8STRC("$PMTK182,2,8"), buff);
	NN<Text::String> result;
...
Optional<Text::String> IO::Device::MTKGPSNMEA::SendMTKCommand(UnsafeArray<const UInt8> cmdBuff, UIntOS cmdSize, UnsafeArray<const UTF8Char> resultStart, UIntOS resultStartLen, Data::Duration timeout)
{
	Data::DateTime dt;
	Data::DateTime dt2;
	NN<Text::String> cmdRes;
...
UIntOS IO::Device::MTKGPSNMEA::GetMTKSerialPort()
{
	UIntOS port = IO::SerialPort::GetUSBPort();
	if (port)
		return port;
