#include "Stdafx.h"
#include "Crypto/Encrypt/RC4Cipher.h"
#include "Core/ByteTool_C.h"
#include "IO/FileStream.h"
#include "IO/FileAnalyse/MDBFileAnalyse.h"
#include "Sync/SimpleThread.h"
#include "Text/MyStringFloat.h"
#include "Text/MyStringW.h"
#include "Text/XLSUtil.h"

void __stdcall IO::FileAnalyse::MDBFileAnalyse::ParseThread(NN<Sync::Thread> thread)
{
	NN<IO::FileAnalyse::MDBFileAnalyse> me = thread->GetUserObj().GetNN<IO::FileAnalyse::MDBFileAnalyse>();
	UInt8 readBuff[4096];
	UInt64 readOfst;
	UIntOS readSize;
	NN<IO::FileAnalyse::MDBFileAnalyse::PackInfo> pack;
	NN<IO::StreamData> fd;
	if (!me->fd.SetTo(fd))
	{
		return;
	}
	readOfst = 0;
	while (!thread->IsStopping())
	{
		if (me->pauseParsing)
		{
			Sync::SimpleThread::Sleep(100);
		}
		else
		{
			readSize = fd->GetRealData(readOfst, 4096, BYTEARR(readBuff));
			if (readSize != 4096)
			{
				break;
			}
			pack = MemAllocNN(PackInfo);
			pack->fileOfst = readOfst;
			pack->packSize = 4096;
			pack->packType = ReadLUInt16(readBuff);
			me->packs.Add(pack);
			readOfst += 4096;
		}
	}
}

IO::FileAnalyse::MDBFileAnalyse::MDBFileAnalyse(NN<IO::StreamData> fd) : thread(ParseThread, this, CSTR("MDBFileAnalyse"))
{
	UInt8 buff[256];
	this->fd = nullptr;
	this->pauseParsing = false;
	fd->GetRealData(0, 256, BYTEARR(buff));
	if (ReadLInt32(buff) != 0x00000100)
	{
		return;
	}
	if (Text::StrStartsWithC(&buff[4], 252, UTF8STRC("Standard Jet DB")))
	{
		this->fileFormat = 0;
	}
	else if (Text::StrStartsWithC(&buff[4], 252, UTF8STRC("Standard ACE DB")))
	{
		this->fileFormat = 1;
	}
	else
	{
		return;
	}
	this->fileVer = ReadLUInt32(&buff[20]);
	this->fd = fd->GetPartialData(0, fd->GetDataSize()).Ptr();
	this->thread.Start();
}

IO::FileAnalyse::MDBFileAnalyse::~MDBFileAnalyse()
{
	this->thread.Stop();
	this->fd.Delete();
	this->packs.MemFreeAll();
}

Text::CStringNN IO::FileAnalyse::MDBFileAnalyse::GetFormatName()
{
	return CSTR("MDB");
}

UIntOS IO::FileAnalyse::MDBFileAnalyse::GetFrameCount()
{
	return this->packs.GetCount();
}

Bool IO::FileAnalyse::MDBFileAnalyse::GetFrameName(UIntOS index, NN<Text::StringBuilderUTF8> sb)
{
	NN<IO::FileAnalyse::MDBFileAnalyse::PackInfo> pack;
	if (!this->packs.GetItem(index).SetTo(pack))
		return false;
	sb->AppendU64(pack->fileOfst);
	sb->AppendC(UTF8STRC(": Num="));
	sb->AppendUIntOS(index);
	sb->AppendC(UTF8STRC(", Type=0x"));
	sb->AppendHex16(pack->packType);
	switch (pack->packType)
	{
	case 0x100:
		sb->AppendC(UTF8STRC(" (File Header)"));
		break;
	case 0x101:
		sb->AppendC(UTF8STRC(" (Data Page)"));
		break;
	case 0x102:
		sb->AppendC(UTF8STRC(" (Table Definition)"));
		break;
	case 0x103:
		sb->AppendC(UTF8STRC(" (Intermediate Index pages)"));
		break;
	case 0x104:
		sb->AppendC(UTF8STRC(" (Leaf Index pages)"));
		break;
	case 0x105:
		sb->AppendC(UTF8STRC(" (Page Usage Bitmaps)"));
		break;
	}
	return true;
}

UIntOS IO::FileAnalyse::MDBFileAnalyse::GetFrameIndex(UInt64 ofst)
{
	UIntOS index = (UIntOS)(ofst >> 12);
	if (index >= this->packs.GetCount())
	{
		return INVALID_INDEX;
	}
	return index;
}

Optional<IO::FileAnalyse::FrameDetail> IO::FileAnalyse::MDBFileAnalyse::GetFrameDetail(UIntOS index)
{
	NN<IO::FileAnalyse::FrameDetail> frame;
	NN<IO::FileAnalyse::MDBFileAnalyse::PackInfo> pack;
	UTF8Char sbuff[64];
	UTF8Char sbuff2[256];
	UnsafeArray<UTF8Char> sptr;
	UnsafeArray<UTF8Char> sptr2;
	UInt8 packBuff[4096];
	UInt8 decBuff[128];
	NN<IO::StreamData> fd;
	if (!this->packs.GetItem(index).SetTo(pack))
		return nullptr;
	if (!this->fd.SetTo(fd))
		return nullptr;

	NEW_CLASSNN(frame, IO::FileAnalyse::FrameDetail(pack->fileOfst, pack->packSize));
	fd->GetRealData(pack->fileOfst, pack->packSize, BYTEARR(packBuff));
	frame->AddHex16(0, CSTR("Frame Type"), ReadLUInt16(&packBuff[0]));
	frame->AddUInt(2, 2, CSTR("Free Space"), ReadLUInt16(&packBuff[2]));
	switch (pack->packType)
	{
	case 0x100:
		frame->AddStrS(4, 16, CSTR("File Format"), &packBuff[4]);
		frame->AddHex32(20, CSTR("File Version"), ReadLUInt32(&packBuff[20]));
		{
			WriteLUInt32(decBuff, 0x6b39dac7);
			Crypto::Encrypt::RC4Cipher rc4(decBuff, 4);
			if (this->fileVer == 0)
			{
				rc4.Decrypt(&packBuff[24], 126, decBuff);
				Double ddays = ReadLDouble(&decBuff[0x5A]);
				frame->AddHexBuff(24, 0x22, CSTR("Unknown"), decBuff, true);
				frame->AddUInt(24 + 0x22, 2, CSTR("System Collation"), ReadLUInt16(&decBuff[0x22]));
				frame->AddUInt(24 + 0x24, 2, CSTR("System Codepage"), ReadLUInt16(&decBuff[0x24]));
				frame->AddHex32(24 + 0x26, CSTR("Database Key"), ReadLUInt32(&decBuff[0x26]));
				frame->AddStrS(24 + 0x2A, 20, CSTR("Database password"), &decBuff[0x2A]);
				frame->AddHexBuff(24 + 0x3E, 24, CSTR("Unknown"), &decBuff[0x3E], true);
				frame->AddUInt(24 + 0x56, 2, CSTR("Unknown"), ReadLUInt16(&decBuff[0x56]));
				Data::DateTime dt;
				Text::XLSUtil::Number2Date(dt, ddays);
				dt.ToLocalTime();
				sptr = Text::StrConcatC(dt.ToString(Text::StrConcatC(Text::StrDouble(sbuff, ddays), UTF8STRC(" (")), "yyyy-MM-dd HH:mm:ss.fff"), UTF8STRC(")"));
				frame->AddField(24 + 0x5A, 8, CSTR("Creation date"), CSTRP(sbuff, sptr));
				frame->AddHexBuff(24 + 0x62, 28, CSTR("Unknown"), &decBuff[0x62], true);
			}
			else
			{
				rc4.Decrypt(&packBuff[24], 128, decBuff);
				Double ddays = ReadLDouble(&decBuff[0x5A]);
				UInt32 ndays = (UInt32)ddays;
				frame->AddHexBuff(24, 0x24, CSTR("Unkonwn"), decBuff, true);
				frame->AddUInt(24 + 0x24, 2, CSTR("System Codepage"), ReadLUInt16(&decBuff[0x24]));
				frame->AddHex32(24 + 0x26, CSTR("Database Key"), ReadLUInt32(&decBuff[0x26]));
				WriteLUInt32(&decBuff[0x2A], ReadLUInt32(&decBuff[0x2A]) ^ ndays);
				WriteLUInt32(&decBuff[0x2E], ReadLUInt32(&decBuff[0x2E]) ^ ndays);
				WriteLUInt32(&decBuff[0x32], ReadLUInt32(&decBuff[0x32]) ^ ndays);
				WriteLUInt32(&decBuff[0x36], ReadLUInt32(&decBuff[0x36]) ^ ndays);
				WriteLUInt32(&decBuff[0x3A], ReadLUInt32(&decBuff[0x3A]) ^ ndays);
				WriteLUInt32(&decBuff[0x3E], ReadLUInt32(&decBuff[0x3E]) ^ ndays);
				WriteLUInt32(&decBuff[0x42], ReadLUInt32(&decBuff[0x42]) ^ ndays);
				WriteLUInt32(&decBuff[0x46], ReadLUInt32(&decBuff[0x46]) ^ ndays);
				WriteLUInt32(&decBuff[0x4A], ReadLUInt32(&decBuff[0x4A]) ^ ndays);
				WriteLUInt32(&decBuff[0x4E], ReadLUInt32(&decBuff[0x4E]) ^ ndays);
				sptr = Text::StrUTF16_UTF8C(sbuff, (const UTF16Char*)&decBuff[0x2A], 20);
				sptr[0] = 0;
				frame->AddField(24 + 0x2A, 40, CSTR("Database Password"), CSTRP(sbuff, sptr));
				frame->AddHex32(24 + 0x52, CSTR("Unknown"), ReadLUInt32(&decBuff[0x52]));
				frame->AddUInt(24 + 0x56, 4, CSTR("System Collation"), ReadLUInt32(&decBuff[0x56]));
				Data::DateTime dt;
				Text::XLSUtil::Number2Date(dt, ddays);
				dt.ToLocalTime();
				sptr = Text::StrConcatC(dt.ToString(Text::StrConcatC(Text::StrDouble(sbuff, ddays), UTF8STRC(" (")), "yyyy-MM-dd HH:mm:ss.fff"), UTF8STRC(")"));
				frame->AddField(24 + 0x5A, 8, CSTR("Creation date"), CSTRP(sbuff, sptr));
				frame->AddHexBuff(24 + 0x62, 30, CSTR("Unknown"), &decBuff[0x62], true);

				frame->AddUInt(152, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[152]));
				frame->AddStrS(156, 4, CSTR("Unknown"), &packBuff[156]);
			}
		}
		break;
	case 0x101:
		frame->AddUInt(4, 4, CSTR("Data Owner"), ReadLUInt32(&packBuff[4]));
		frame->AddUInt(8, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[8]));
		frame->AddUInt(12, 2, CSTR("Record Count"), ReadLUInt16(&packBuff[12]));
		{
			UIntOS i = 0;
			UIntOS j = ReadLUInt16(&packBuff[12]);
			UIntOS thisOfst;
			UIntOS lastOfst = 4096;
			while (i < j)
			{
				thisOfst = ReadLUInt16(&packBuff[14 + i * 2]);
				sptr = Text::StrUIntOS(sbuff, thisOfst & 0xFFF);
				if (thisOfst & 0x4000)
				{
					sptr = Text::StrConcatC(sptr, UTF8STRC(" (overflow)"));
				}
				if (thisOfst & 0x8000)
				{
					sptr = Text::StrConcatC(sptr, UTF8STRC(" (deleted)"));
				}
				frame->AddField(14 + i * 2, 2, CSTR("Record Offset"), CSTRP(sbuff, sptr));
				sptr = Text::StrUIntOS(Text::StrConcatC(sbuff, UTF8STRC("Record ")), i);
				thisOfst &= 0xfff;
				frame->AddHexBuff(thisOfst, lastOfst - thisOfst, CSTRP(sbuff, sptr), &packBuff[thisOfst], true);
				lastOfst = thisOfst;
				i++;
			}
			frame->AddHexBuff(14 + j * 2, ReadLUInt16(&packBuff[2]), CSTR("Free Space"), &packBuff[14 + j * 2], true);
		}
		break;
	case 0x102:
		if (this->fileVer == 0)
		{
			frame->AddUInt(4, 4, CSTR("Next Page"), ReadLUInt32(&packBuff[4]));
			frame->AddUInt(8, 4, CSTR("Table Length"), ReadLUInt32(&packBuff[8]));
			frame->AddUInt(12, 4, CSTR("Number of records"), ReadLUInt32(&packBuff[12]));
			frame->AddUInt(16, 4, CSTR("Next Autonunber"), ReadLUInt32(&packBuff[16]));

			UIntOS freeSpace = ReadLUInt16(&packBuff[2]);
			frame->AddHexBuff(4096 - freeSpace, freeSpace, CSTR("Free Space"), &packBuff[4096 - freeSpace], true);
		}
		else //1
		{
			frame->AddUInt(4, 4, CSTR("Next Page"), ReadLUInt32(&packBuff[4]));
			frame->AddUInt(8, 4, CSTR("Table Length"), ReadLUInt32(&packBuff[8]));
			frame->AddUInt(12, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[12]));
			frame->AddUInt(16, 4, CSTR("Number of records"), ReadLUInt32(&packBuff[16]));
			frame->AddUInt(20, 4, CSTR("Next Autonunber"), ReadLUInt32(&packBuff[20]));
			frame->AddHex32(24, CSTR("Autonunber Flags"), ReadLUInt32(&packBuff[24]));
			frame->AddUInt(28, 4, CSTR("CT Autonunber"), ReadLUInt32(&packBuff[28]));
			frame->AddUInt64(32, CSTR("Unknown"), ReadLUInt64(&packBuff[32]));
			frame->AddStrC(40, 1, CSTR("Table Type"), &packBuff[40]);
			frame->AddUInt(41, 2, CSTR("Maximum column"), ReadLUInt16(&packBuff[41]));
			frame->AddUInt(43, 2, CSTR("Number of Variable Columns"), ReadLUInt16(&packBuff[43]));
			frame->AddUInt(45, 2, CSTR("Number of columns"), ReadLUInt16(&packBuff[45]));
			frame->AddUInt(47, 4, CSTR("Number of logical index"), ReadLUInt32(&packBuff[47]));
			frame->AddUInt(51, 4, CSTR("Number of index"), ReadLUInt32(&packBuff[51]));
			frame->AddHex8(55, CSTR("Usage Flags"), packBuff[55]);
			frame->AddUInt(56, 3, CSTR("Usage Page"), ReadLUInt24(&packBuff[56]));
			frame->AddHex8(59, CSTR("Free Flags"), packBuff[59]);
			frame->AddUInt(60, 3, CSTR("Free Page"), ReadLUInt24(&packBuff[60]));

			UIntOS ofst = 63;
			UIntOS i = 0;
			UIntOS j = ReadLUInt32(&packBuff[51]);
			while (i < j)
			{
				frame->AddUInt(ofst, 4, CSTR("Index Unknown"), ReadLUInt32(&packBuff[ofst]));
				frame->AddUInt64(ofst + 4, CSTR("Number of index rows"), ReadLUInt64(&packBuff[ofst + 4]));
				ofst += 12;
				i++;
			}

			i = 0;
			j = ReadLUInt16(&packBuff[45]);
			while (i < j)
			{
				frame->AddUIntName(ofst, 1, CSTR("Column Type"), packBuff[ofst], ColumnTypeGetName(packBuff[ofst]));
				frame->AddUInt(ofst + 1, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[ofst + 1]));
				frame->AddUInt(ofst + 5, 2, CSTR("Total Column Number"), ReadLUInt16(&packBuff[ofst + 5]));
				frame->AddUInt(ofst + 7, 2, CSTR("Offset of Variable Length Column"), ReadLUInt16(&packBuff[ofst + 7]));
				frame->AddUInt(ofst + 9, 2, CSTR("Column Number"), ReadLUInt16(&packBuff[ofst + 9]));
				frame->AddHex32(ofst + 11, CSTR("Misc"), ReadLUInt32(&packBuff[ofst + 11]));
				frame->AddHex8(ofst + 15, CSTR("Flags1"), packBuff[ofst + 15]);
				frame->AddHex8(ofst + 16, CSTR("Flags2"), packBuff[ofst + 16]);
				frame->AddUInt(ofst + 17, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[ofst + 17]));
				frame->AddUInt(ofst + 21, 2, CSTR("Offset of Fixed Column"), ReadLUInt16(&packBuff[ofst + 21]));
				frame->AddUInt(ofst + 23, 2, CSTR("Column Length"), ReadLUInt16(&packBuff[ofst + 23]));
				ofst += 25;
				i++;
			}
			i = 0;
			while (i < j)
			{
				UIntOS colSize = ReadLUInt16(&packBuff[ofst]);
				sptr = Text::StrUIntOS(Text::StrConcatC(sbuff, UTF8STRC("Column Name Size ")), i);
				frame->AddUInt(ofst, 2, CSTRP(sbuff, sptr), colSize);
				sptr = Text::StrUIntOS(Text::StrConcatC(sbuff, UTF8STRC("Column Name ")), i);
				sptr2 = Text::StrUTF16_UTF8C(sbuff2, (const UTF16Char*)&packBuff[ofst + 2], colSize >> 1);
				sptr2[0] = 0;
				frame->AddField(ofst + 2, colSize, CSTRP(sbuff, sptr), CSTRP(sbuff2, sptr2));
				ofst += 2 + colSize;
				i++;
			}
			i = 0;
			j = ReadLUInt32(&packBuff[51]);
			while (i < j)
			{
				frame->AddUInt(ofst, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[ofst]));
				frame->AddUInt(ofst + 4, 2, CSTR("Column number0"), ReadLUInt16(&packBuff[ofst + 4]));
				frame->AddUInt(ofst + 6, 1, CSTR("Column order0"), packBuff[ofst + 6]);
				frame->AddUInt(ofst + 7, 2, CSTR("Column number1"), ReadLUInt16(&packBuff[ofst + 7]));
				frame->AddUInt(ofst + 9, 1, CSTR("Column order1"), packBuff[ofst + 9]);
				frame->AddUInt(ofst + 10, 2, CSTR("Column number2"), ReadLUInt16(&packBuff[ofst + 10]));
				frame->AddUInt(ofst + 12, 1, CSTR("Column order2"), packBuff[ofst + 12]);
				frame->AddUInt(ofst + 13, 2, CSTR("Column number3"), ReadLUInt16(&packBuff[ofst + 13]));
				frame->AddUInt(ofst + 15, 1, CSTR("Column order3"), packBuff[ofst + 15]);
				frame->AddUInt(ofst + 16, 2, CSTR("Column number4"), ReadLUInt16(&packBuff[ofst + 16]));
				frame->AddUInt(ofst + 18, 1, CSTR("Column order4"), packBuff[ofst + 18]);
				frame->AddUInt(ofst + 19, 2, CSTR("Column number5"), ReadLUInt16(&packBuff[ofst + 19]));
				frame->AddUInt(ofst + 21, 1, CSTR("Column order5"), packBuff[ofst + 21]);
				frame->AddUInt(ofst + 22, 2, CSTR("Column number6"), ReadLUInt16(&packBuff[ofst + 22]));
				frame->AddUInt(ofst + 24, 1, CSTR("Column order6"), packBuff[ofst + 24]);
				frame->AddUInt(ofst + 25, 2, CSTR("Column number7"), ReadLUInt16(&packBuff[ofst + 25]));
				frame->AddUInt(ofst + 27, 1, CSTR("Column order7"), packBuff[ofst + 27]);
				frame->AddUInt(ofst + 28, 2, CSTR("Column number8"), ReadLUInt16(&packBuff[ofst + 28]));
				frame->AddUInt(ofst + 30, 1, CSTR("Column order8"), packBuff[ofst + 30]);
				frame->AddUInt(ofst + 31, 2, CSTR("Column number9"), ReadLUInt16(&packBuff[ofst + 31]));
				frame->AddUInt(ofst + 33, 1, CSTR("Column order9"), packBuff[ofst + 33]);
				frame->AddHex8(ofst + 34, CSTR("Page Flag"), packBuff[ofst + 34]);
				frame->AddUInt(ofst + 35, 3, CSTR("Used Page"), ReadLUInt24(&packBuff[ofst + 35]));
				frame->AddUInt(ofst + 38, 4, CSTR("First Index"), ReadLUInt32(&packBuff[ofst + 38]));
				frame->AddHex8(ofst + 42, CSTR("Index Flag"), packBuff[ofst + 42]);
				frame->AddHexBuff(ofst + 43, 9, CSTR("Unknown"), &packBuff[ofst + 43], false);
				ofst += 52;
				i++;
			}
			i = 0;
			j = ReadLUInt32(&packBuff[47]);
			while (i < j)
			{
				frame->AddUInt(ofst, 4, CSTR("Unknown"), ReadLUInt32(&packBuff[ofst]));
				frame->AddUInt(ofst + 4, 4, CSTR("Index Number"), ReadLUInt32(&packBuff[ofst + 4]));
				frame->AddUInt(ofst + 8, 4, CSTR("Index Number2"), ReadLUInt32(&packBuff[ofst + 8]));
				frame->AddUInt(ofst + 12, 1, CSTR("Rel Table Type"), packBuff[ofst + 12]);
				frame->AddInt(ofst + 13, 4, CSTR("Rel Index Num"), ReadLInt32(&packBuff[ofst + 13]));
				frame->AddInt(ofst + 17, 4, CSTR("Rel Table Page"), ReadLInt32(&packBuff[ofst + 17]));
				frame->AddUInt(ofst + 21, 1, CSTR("Cascade Updates"), packBuff[ofst + 21]);
				frame->AddUInt(ofst + 22, 1, CSTR("Cascade Deletes"), packBuff[ofst + 22]);
				frame->AddUInt(ofst + 23, 1, CSTR("Index Type"), packBuff[ofst + 23]);
				ofst += 28;
				i++;
			}
			i = 0;
			while (i < j)
			{
				UIntOS colSize = ReadLUInt16(&packBuff[ofst]);
				sptr = Text::StrUIntOS(Text::StrConcatC(sbuff, UTF8STRC("Index Name Size ")), i);
				frame->AddUInt(ofst, 2, CSTRP(sbuff, sptr), colSize);
				sptr = Text::StrUIntOS(Text::StrConcatC(sbuff, UTF8STRC("Index Name ")), i);
				sptr2 = Text::StrUTF16_UTF8C(sbuff2, (const UTF16Char*)&packBuff[ofst + 2], colSize >> 1);
				sptr2[0] = 0;
				frame->AddField(ofst + 2, colSize, CSTRP(sbuff, sptr), CSTRP(sbuff2, sptr2));
				ofst += 2 + colSize;
				i++;
			}
			UIntOS freeSpace = ReadLUInt16(&packBuff[2]);
			UIntOS endOfst = 4096 - freeSpace;
			while (ofst < endOfst)
			{
				Int16 colNum = ReadLInt16(&packBuff[ofst]);
				UInt32 usedPages = ReadLUInt32(&packBuff[ofst + 2]);
				UInt32 freePages = ReadLUInt32(&packBuff[ofst + 6]);
				frame->AddInt(ofst, 2, CSTR("Column Number"), colNum);
				frame->AddUInt(ofst + 2, 4, CSTR("Used Pages"), usedPages);
				frame->AddUInt(ofst + 6, 4, CSTR("Free Pages"), freePages);
				if (colNum == -1)
				{
					break;
				}
				ofst += 10;
			}
			frame->AddHexBuff(4096 - freeSpace, freeSpace, CSTR("Free Space"), &packBuff[4096 - freeSpace], true);
		}
		break;
	}
	return frame;
}

Bool IO::FileAnalyse::MDBFileAnalyse::IsError()
{
	return this->fd.IsNull();
}

Bool IO::FileAnalyse::MDBFileAnalyse::IsParsing()
{
	return this->thread.IsRunning();
}

Bool IO::FileAnalyse::MDBFileAnalyse::TrimPadding(Text::CStringNN outputFile)
{
	return false;
}

Text::CString IO::FileAnalyse::MDBFileAnalyse::ColumnTypeGetName(UInt8 colType)
{
	switch (colType)
	{
	case 1:
		return CSTR("BOOL");
	case 2:
		return CSTR("BYTE");
	case 3:
		return CSTR("INT");
	case 4:
		return CSTR("LONGINT");
	case 5:
		return CSTR("MONEY");
	case 6:
		return CSTR("FLOAT");
	case 7:
		return CSTR("DOUBLE");
	case 8:
		return CSTR("DATETIME");
	case 9:
		return CSTR("BINARY");
	case 0xA:
		return CSTR("TEXT");
	case 0xB:
		return CSTR("OLE"); //Long Binary
	case 0xC:
		return CSTR("MEMO"); //Long Text
	case 0xF:
		return CSTR("REPID");
	case 0x10:
		return CSTR("NUMERIC");
	default:
		return CSTR("Unknown");
	}
}
