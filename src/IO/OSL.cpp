...
UTF8Char *IO::OS::GetDistro(UTF8Char *sbuff)
{
	...
	if (IO::Path::GetPathType((const UTF8Char*)"/usr/sbin/ENG/stringlist_ENG.txt") == IO::Path::PT_FILE)
	{
		...
		NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/usr/sbin/ENG/stringlist_ENG.txt", IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
		if (!fs->IsError())
		{
			NEW_CLASS(reader, Text::UTF8Reader(fs));
			sb.ClearStr();
...
UTF8Char *IO::OS::GetVersion(UTF8Char *sbuff)
{
	...
	if (IO::Path::GetPathType((const UTF8Char*)"/etc/debian_version") == IO::Path::PT_FILE)
	{
		...
		Text::UTF8Reader *reader;
		IO::FileStream *fs;
		NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/etc/debian_version", IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
		if (!fs->IsError())
		{
			NEW_CLASS(reader, Text::UTF8Reader(fs));
	...
	if (IO::Path::GetPathType((const UTF8Char*)"/etc/openwrt_version") == IO::Path::PT_FILE)
	{
		...
		Text::UTF8Reader *reader;
		IO::FileStream *fs;
		NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/etc/openwrt_version", IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
		if (!fs->IsError())
		{
			NEW_CLASS(reader, Text::UTF8Reader(fs));
			ret = reader->ReadLine(sbuff, 128);
	...
	if (IO::Path::GetPathType((const UTF8Char*)"/etc/version.txt") == IO::Path::PT_FILE) //Bovine
	{
		...
		UTF8Char *sptr;
		IO::FileStream *fs;
		NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/etc/version.txt", IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
		NEW_CLASS(reader, Text::UTF8Reader(fs));
	...
	if (IO::Path::GetPathType((const UTF8Char*)"/etc/mlinux-version") == IO::Path::PT_FILE) //mLinux
	{
		...
		UTF8Char *sptr;
		IO::FileStream *fs;
		NEW_CLASS(fs, IO::FileStream((const UTF8Char*)"/etc/mlinux-version", IO::FileStream::FILE_MODE_READONLY, IO::FileStream::FILE_SHARE_DENY_NONE, IO::FileStream::BT_NORMAL));
		NEW_CLASS(reader, Text::UTF8Reader(fs));
		sptr = reader->ReadLine(sbuff, 512);
		DEL_CLASS(reader);
		DEL_CLASS(fs);
