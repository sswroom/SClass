...
UIntOS Exporter::CURExporter::BuildBuff(UnsafeArray<UInt8> buff, NN<Media::ImageList> imgList, Bool hasHotSpot)
{
	...
	while (i < j)
	{
		...
		else if (img->info.pf == Media::PF_PAL_1_A1)
		{
			...
			WriteLInt32(&imgPtr[24], 0);
			WriteLInt32(&imgPtr[28], 0);
			WriteLInt32(&imgPtr[32], 2);
			WriteLInt32(&imgPtr[36], 2);
			imgPtr += 40;
			retSize += 40;
		...
		else if (img->info.pf == Media::PF_PAL_4_A1)
		{
			...
			WriteLInt32(&indexPtr[8], (Int32)(40 + 64 + (imgSize + maskSize) * img->info.dispSize.y));
			...
			WriteLInt32(&imgPtr[24], 0);
			WriteLInt32(&imgPtr[28], 0);
			WriteLInt32(&imgPtr[32], 16);
			WriteLInt32(&imgPtr[36], 16);
			imgPtr += 40;
			retSize += 40;
		...
		else if (img->info.pf == Media::PF_PAL_8_A1)
		{
			...
			WriteLInt32(&indexPtr[8], (Int32)(40 + 1024 + (imgSize + maskSize) * img->info.dispSize.y));
			...
			WriteLInt32(&imgPtr[24], 0);
			WriteLInt32(&imgPtr[28], 0);
			WriteLInt32(&imgPtr[32], 256);
			WriteLInt32(&imgPtr[36], 256);
			imgPtr += 40;
			retSize += 40;
