...
Bool Exporter::GIFExporter::GetOutputName(OSInt index, UTF8Char *nameBuff, UTF8Char *fileNameBuff)
{
	if (index == 0)
	{
		Text::StrConcat(nameBuff, (const UTF8Char*)"GIF Image");
...
Bool Exporter::GIFExporter::ExportFile(IO::SeekableStream *stm, const UTF8Char *fileName, IO::ParsedObject *pobj, void *param)
{
	...
	if (img->info->pf == Media::PF_PAL_8 || img->info->pf == Media::PF_PAL_W8)
	{
		...
		buff[10] = 0xf7;
		buff[11] = 0;
		buff[12] = (UInt8)(Math::Double2Int32(img->info->par2 * 64.0) - 15);
		stm->Write(buff, 13);
