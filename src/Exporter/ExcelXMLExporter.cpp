...
Bool Exporter::ExcelXMLExporter::GetOutputName(UIntOS index, UnsafeArray<UTF8Char> nameBuff, UnsafeArray<UTF8Char> fileNameBuff)
{
	if (index == 0)
	{
...
Bool Exporter::ExcelXMLExporter::ExportFile(NN<IO::SeekableStream> stm, Text::CStringNN fileName, NN<IO::ParsedObject> pobj, Optional<ParamData> param)
{
	...
	if (wb->HasWindowInfo() != 0)
	{
		...
		if (wb->GetActiveSheet() != 0)
		{
			...
			sb.AppendUIntOS(wb->GetActiveSheet());
			sb.AppendC(UTF8STRC("</ActiveSheet>"));
			writer.WriteLine(sb.ToCString());
		}
		...
		while (k < l)
		{
			if (wb->GetStyle(k).SetTo(style))
			{
				...
				if ((style->GetFillColor() & 0xffffff) != 0xffffff)
				{
					...
					sb.AppendC(&sbuff[2], (UIntOS)(sptr - &sbuff[2]));
	...
	while (it.HasNext())
	{
		...
		if (ws->GetCount() > 0)
		{
				...
				if (ws->GetColWidthPt(k) != lastColWidth)
				{
					if (lastColWidth >= 0)
					{
						...
						if (lastColIndex + 1 != k)
						{
							...
							sb.AppendUIntOS((k - lastColIndex - 1));
							sb.AppendC(UTF8STRC("\""));
							needIndex = true;
						}
						else
			...
			if (lastColWidth >= 0)
			{
				...
				{
					sb.AppendC(UTF8STRC(" ss:Span=\""));
					sb.AppendUIntOS((l - lastColIndex - 1));
					sb.AppendC(UTF8STRC("\""));
				}
				sb.AppendC(UTF8STRC("/>"));
				writer.WriteLine(sb.ToCString());
